...
export async function GET(request: Request, { params }: { params: { id: string } }) {
  const rateLimited = enforceRateLimit(request, "cards-by-id", { limit: 240, windowMs: 60_000 });
  if (rateLimited) return rateLimited;

  const cardId = safeIdentifier(params.id, 160);
  if ("error" in cardId) return cardId.error;
  ...
  try {
    const response = await fetch(`${API_URL}/${encodeURIComponent(cardId.value)}`, {
