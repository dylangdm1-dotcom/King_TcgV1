...
async function resolveChineseSet(setCode: string, apiKey: string) {
  ...
  if (index.ok) {
    ...
    const exact = rows.find((raw: any) =>
      normalizeCode(raw?.set_code) === wanted &&
  ...
  const direct = await getJson(
    `${POKEWALLET}/sets/${encodeURIComponent(setCode)}?language=chn&page=1&limit=200`,
    apiKey
  );
...
export async function GET(request: NextRequest) {
  ...
  // V79: Simplified Chinese catalogue display is QUOTA-INDEPENDENT.
  // Never call the authenticated PokéWallet API merely to list/open a CN set.
  if (language === "zh-tw" && !setCode) {
    ...
    return NextResponse.json({ success: true, sets, source: "local-cn-v79" });
  }

  const apiKey = process.env.POKEWALLET_API_KEY;

  // V80: CN set opening uses the documented authenticated set endpoint first.
  // This avoids the expensive global /sets lookup and directly resolves the
  // requested Simplified-Chinese code. The public catalogue is only a degraded
  // fallback so a temporary provider failure never hides the whole set.
  if (language === "zh-tw" && setCode) {
    if (apiKey) {
      let privateResult = await getJson(
        `${POKEWALLET}/sets/${encodeURIComponent(setCode)}?language=chn&page=1&limit=200`,
        apiKey
      );

      if (privateResult.ok && privateResult.data?.disambiguation && Array.isArray(privateResult.data?.matches)) {
        const exact = privateResult.data.matches.find(
          (match: any) => String(match?.language || "").toLowerCase() === "chn"
        );
        const numericId = String(exact?.set_id || "").trim();
        if (numericId) privateResult = await fetchSetByNumericId(numericId, apiKey);
      } else if (privateResult.ok) {
        const numericId = String(privateResult.data?.set?.set_id || "").trim();
        const firstCards = Array.isArray(privateResult.data?.cards) ? privateResult.data.cards : [];
        const total = Number(privateResult.data?.set?.total_cards || firstCards.length || 0);
        // If the set is paginated, re-fetch by unique numeric id so every page is collected.
        if (numericId && total > firstCards.length) {
          const paged = await fetchSetByNumericId(numericId, apiKey);
          if (paged.ok) privateResult = paged;
        }
      }
      ...
      if (privateResult.ok && Array.isArray(privateResult.data?.cards) && privateResult.data.cards.length > 0) {
        ...
        return NextResponse.json({
          ...
          source: "pokewallet-private-cn-v80",
        });
      }
    }

    const publicCatalogue = await fetchPublicChineseCards(setCode);
    if (publicCatalogue.ok) {
      ...
      return NextResponse.json({
        ...
        cards: publicCards,
        source: "pokewallet-public-cn-v80",
        degraded: true,
      });
    }
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Chinese set unavailable from private and public PokéWallet paths",
        public_upstream: publicCatalogue.status,
        set: normalizeCode(setCode),
      },
      { status: 502 }
    );
  }
  ...
  if (!setCode) {
    ...
    if (language === "zh-tw") {
      ...
      const sets = CHINESE_SET_CATALOG.map((entry) => {
        const raw: any = byCode.get(normalizeCode(entry.code));
        return {
          id: entry.code,
      ...
      const known = new Set(sets.map((set) => normalizeCode(set.id)));
