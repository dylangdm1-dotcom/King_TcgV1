...
import {
  CHINESE_SET_CATALOG,
  CHINESE_AUDITED_AVAILABLE_SET_COUNTS,
...
export async function GET(request: NextRequest) {
  const rateLimited = enforceRateLimit(request, "catalog-sets", { limit: 120, windowMs: 60_000 });
  ...
  const rawSetCode = params.get("set");
  const parsedSetCode = rawSetCode ? safeIdentifier(rawSetCode, 80) : null;
  if (parsedSetCode && "error" in parsedSetCode) return parsedSetCode.error;
  const setCode = parsedSetCode?.value || null;
  ...
  if (language === "zh-tw" && !setCode && !apiKey) {
    const sets = CHINESE_SET_CATALOG.map((entry) => {
      ...
      return {
        ...
        total: auditedCount || Number(entry.officialCount || 0),
        printedTotal: auditedCount || Number(entry.officialCount || 0),
  ...
  if (!setCode) {
    ...
    if (language === "zh-tw") {
      ...
      const sets = CHINESE_SET_CATALOG.map((entry) => {
        ...
        const providerCount = Number(raw?.card_count || 0);
        const auditedCount = CHINESE_AUDITED_AVAILABLE_SET_COUNTS[entry.code] || 0;
        const total = providerCount || auditedCount || Number(entry.officialCount || 0);
        return {
          id: entry.code,
