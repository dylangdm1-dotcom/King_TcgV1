...
export async function GET(request: NextRequest) {
  ...
  // V72 CN fallback: /search accepts a set code directly and returns card rows
  // with the same unified Cardmarket/TCGPlayer payload. This bypasses both the
  // global /sets listing and a transient /sets/:code miss without inventing IDs.
  if (!result.ok && language === "zh-tw") {
    const collected: any[] = [];
    let totalPages = 1;
