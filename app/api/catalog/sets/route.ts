...
export async function GET(request: NextRequest) {
  ...
  if (language === "zh-tw" && !setCode && !apiKey) {
    const sets = CHINESE_SET_CATALOG.map((entry) => ({
      ...
      total: 0, printedTotal: 0, releaseDate: "", images: {}, availability: "unknown",
    }));
  ...
  if (language === "zh-tw" && setCode) {
    ...
    if (!apiKey) {
      ...
      if (announced) {
        return NextResponse.json({
          success: true,
          set: { id: normalizeCode(setCode), name: announced.name, total: 0 },
          cards: [],
    ...
    if (language === "zh-tw") {
      ...
      const sets = CHINESE_SET_CATALOG.map((entry) => {
        ...
        return {
          ...
          images: {},
          availability: total > 0 ? "available" : "announced",
        };
      });
  ...
  if (!result.ok) {
    if (language === "zh-tw" && CHINESE_SET_CATALOG.some((entry) => normalizeCode(entry.code) === normalizeCode(setCode))) {
      ...
      return NextResponse.json({
        success: true,
        set: { id: normalizeCode(setCode), name: announced?.name || setCode, total: 0 },
        cards: [],
        source: "local-cn-announced",
        availability: "announced",
      }, { headers: CATALOG_CACHE_HEADERS });
