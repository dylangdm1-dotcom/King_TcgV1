...
export async function GET(request: NextRequest) {
  ...
  if (!result.ok) {
    if (language === "zh-tw") {
      const publicFallback = await fetchPublicChineseCards(setCode);
      if (publicFallback.ok) {
        const publicCards = normalizePublicChineseCards(setCode, publicFallback.data);
        return NextResponse.json({
          success: true,
          set: {
            id: normalizeCode(setCode),
          ...
          cards: publicCards,
          source: "pokewallet-public-cn-fallback",
          degraded: true,
