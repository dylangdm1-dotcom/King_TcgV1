...
async function searchSetCards(apiKey: string, setName: string, setCode: string, language: RegionalLanguage) {
  ...
  // PokéWallet officially supports searching directly by set code (for example
  // CBB3C). Prefer that for CN because CardMarket-only rows keep the canonical
  // set_code even when set_name is absent. Use the curated name only as a
  // secondary discovery query.
  const queries = Array.from(new Set([setCode, setName].map((value) => String(value || "").trim()).filter(Boolean)));
  const allRows: any[] = [];
  for (const query of queries) {
    const rows = await fetchSearchPages(apiKey, query);
    allRows.push(...rows);
    if (wantedCode && rows.some((raw: any) => normalizeCode(raw?.card_info?.set_code || raw?.set_code) === wantedCode)) {
      break;
    }
  }
  ...
  return allRows.filter((raw: any) => {
    ...
    // Exact canonical set code is the strongest identity for PokéWallet.
    const codeMatch = Boolean(wantedCode && code === wantedCode);
    const nameMatch = Boolean(wantedName && name && (name === wantedName || name.includes(wantedName) || wantedName.includes(name)));
