    ...
    const response = await fetch(
      `https://www.pokewallet.io/api/catalog/sets/${encodeURIComponent(setCode)}/cards?game=pokemon`,
      {
        ...
        headers: {
          ...
          Referer: "https://www.pokewallet.io/catalog",
          Origin: "https://www.pokewallet.io",
        },
      }
    );
...
export async function GET(request: NextRequest) {
  ...
  // V76: for Simplified Chinese, use the public PokéWallet catalogue first.
  // It is the same endpoint used by pokewallet.io/catalog and does not consume
  // the authenticated API quota. This guarantees that an extension can still
  // display its card list while the private API is returning 429.
  if (language === "zh-tw") {
    ...
    if (publicCatalogue.ok) {
      ...
      return NextResponse.json({
        ...
        cards: publicCards,
        source: "pokewallet-public-cn-primary",
        degraded: true,
      });
    }
  }
