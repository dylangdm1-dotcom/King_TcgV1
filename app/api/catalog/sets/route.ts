...
async function chinesePublicAvailability(setCode: string) {
  for (const candidate of chineseProviderCodeCandidates(setCode)) {
    const result = await fetchPublicChineseCards(candidate);
    if (result.ok && Array.isArray(result.data) && result.data.length > 0) {
      return { available: true, count: result.data.length, source: "pokewallet-public" };
    }
  }
  return { available: false, count: 0, source: "metadata-only" };
}

async function enrichChineseCatalogueAvailability(sets: any[]) {
  const enriched: any[] = [];
  for (let index = 0; index < sets.length; index += 6) {
    const batch = sets.slice(index, index + 6);
    const rows = await Promise.all(batch.map(async (set) => {
      if (set?.providerSetId && Number(set?.providerCardCount || 0) > 0) {
        return {
          ...set,
          availability: "available",
          availabilitySource: "pokewallet-private",
        };
      }
      const publicStatus = await chinesePublicAvailability(String(set?.id || ""));
      if (publicStatus.available) {
        return {
          ...set,
          total: publicStatus.count || Number(set?.total || 0),
          printedTotal: publicStatus.count || Number(set?.printedTotal || 0),
          availability: "available",
          availabilitySource: publicStatus.source,
        };
      }
      return {
        ...set,
        availability: "metadata_only",
        availabilitySource: "metadata-only",
      };
    }));
    enriched.push(...rows);
  }
...
export async function GET(request: NextRequest) {
  ...
  if (!apiKey && !setCode && language === "zh-tw") {
    const baseSets = CHINESE_SET_CATALOG.map((entry) => ({
      id: entry.code,
      name: entry.name,
      series: entry.era || "Simplified Chinese",
      total: Number(entry.officialCount || 0),
    ...
    const sets = await enrichChineseCatalogueAvailability(baseSets);
    return NextResponse.json({
      success: true,
      sets,
      source: "local-cn+pokewallet-public-availability",
    }, { headers: CATALOG_CACHE_HEADERS });
  }
    ...
    if (language === "zh-tw") {
      ...
      const sets = CHINESE_SET_CATALOG.map((entry) => {
        ...
        return {
          ...
          images: {},
          providerCardCount: total,
          availability: total > 0 ? "available" : "metadata_only",
          availabilitySource: total > 0 ? "pokewallet-private" : "metadata-only",
        };
      });
      ...
      const verifiedSets = await enrichChineseCatalogueAvailability(sets);
      return NextResponse.json({
        success: true,
        sets: verifiedSets,
        source: result.ok
          ? "pokewallet-private+public+local-cn"
          : "pokewallet-public+local-cn-fallback",
      }, { headers: CATALOG_CACHE_HEADERS });
    }
