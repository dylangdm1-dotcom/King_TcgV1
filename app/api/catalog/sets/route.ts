...
export async function GET(request: NextRequest) {
  ...
  if (language === "ja") {
    try {
      if (!setCode) {
        ...
        const sets = Array.isArray(rows) ? rows.map((raw: any) => ({
          id: String(raw?.id || ""),
  ...
  if (language === "zh-tw" && !setCode && !apiKey) {
    const sets = CHINESE_SET_CATALOG.map((entry) => ({
      id: entry.code, name: entry.name, series: entry.era || "Simplified Chinese",
      total: 0, printedTotal: 0, releaseDate: "", images: {},
    }));
