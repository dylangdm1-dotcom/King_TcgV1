...
async function fromTcgdex(card: InputCard): Promise<MarketPayload> {
  ...
  for (const id of exactCardIds(card)) {
    ...
    if (cardmarket && (card.language === "fr" || card.language === "en")) {
...
async function fromJustTcg(card: InputCard): Promise<MarketPayload> {
  ...
  const providerSetId = card.marketSetId || card.setId;
  const queries = Array.from(new Set(["", providerSetId, card.setName, card.name].map((v) => String(v ?? "").trim())));
  const expectedNumber = normalizedNumber(card.number);
  const expectedSetId = normalizedSet(providerSetId);
  ...
  for (const query of queries) {
    ...
    for (const candidate of list) {
      ...
      const exactSetIdMatch = Boolean(expectedSetId && setFields.some((field) => field === expectedSetId));
      const exactSetNameMatch = Boolean(expectedSetName && textFields.some((field) => field === expectedSetName));

      // Pour JP/CN, aucun "includes" sur les codes de set : m5 ne doit jamais
      // accepter m5a, et un code chinois ne doit jamais être rapproché d'un JP.
      if ((card.language === "ja" || card.language === "zh-tw") && expectedSetId && setFields.length && !exactSetIdMatch && !exactSetNameMatch) {
        continue;
      }

      let score = 0;
      if (exactSetIdMatch) score += 8;
      if (exactSetNameMatch) score += 6;
      if (card.language !== "ja" && card.language !== "zh-tw" && expectedSetId && setFields.some((field) => field.includes(expectedSetId))) score += 2;
      if (card.language !== "ja" && card.language !== "zh-tw" && expectedSetName && textFields.some((field) => field.includes(expectedSetName) || expectedSetName.includes(field))) score += 2;
  ...
  if (!bestItem) return emptyPayload(sourceStatus(lastStatus));
...
function matchesPokeWallet(candidate: any, card: InputCard): boolean {
  ...
  const expectedSetCode = normalizedSet(card.marketSetId || card.setId);
  ...
  if (imageLanguages.length) {
    ...
    if (card.language === "ja" && !imageLanguages.some((value: string) => ["ja", "jap", "jp", "japanese"].includes(value))) return false;
    if (card.language === "zh-tw" && !imageLanguages.some((value: string) => ["zh", "zh-tw", "zh-cn", "chn", "cn", "chinese"].includes(value))) return false;
  }
  return true;
}
...
async function fromPokeWallet(card: InputCard): Promise<MarketPayload> {
  ...
  const queries = Array.from(
    new Set(
      [
        [card.marketSetId || card.setId, cleanNumber(card.number)].filter(Boolean).join(" "),
...
function calculateEstimate(payload: MarketPayload, language: CardLanguage): MarketPayload["estimate"] {
  const compatible = payload.quotes.filter((quote) => quote.compatible && quote.language === language && quote.price > 0);
  const excluded = payload.quotes.filter((quote) => !compatible.includes(quote)).map((quote) => ({ source: quote.label, reason: quote.language !== language ? `Langue ${quote.language} différente de ${language}` : "Source indicative ou comparable" }));
  if (compatible.length) {
    const center = median(compatible.map((quote) => quote.price));
    const filtered = compatible.length >= 3 ? compatible.filter((quote) => quote.price >= center * 0.25 && quote.price <= center * 4) : compatible;
    return { price: Number(median(filtered.map((quote) => quote.price)).toFixed(2)), language, currency: "EUR", condition: "Near Mint", confidence: filtered.some((quote) => quote.confidence === "high") ? "high" : filtered.length >= 2 ? "medium" : "limited", includedSources: filtered.map((quote) => quote.label), excludedSources: excluded };
  }
  const regional = payload.quotes.find((quote) => quote.metric === "trend_europe" && quote.price > 0)
    ...
    ?? payload.quotes.find((quote) => quote.metric === "lowest_europe" && quote.price > 0);
  if (!regional) return undefined;
  return { price: Number(regional.price.toFixed(2)), language, currency: "EUR", condition: "Near Mint", confidence: "limited", includedSources: [`Référence indicative : ${regional.label}`], excludedSources: excluded };
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v35-language-market",
