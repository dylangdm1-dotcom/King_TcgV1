...
async function fromJustTcg(card: InputCard): Promise<MarketPayload> {
  ...
  for (const query of queries) {
    ...
    if (item || result.status === "rate_limited") break;
  }
  ...
  const variants = allVariants.filter((variant: any) => {
    ...
    if (variantLanguage) {
      return variantLanguage.includes(normalizedText(expectedLanguage));
    }

    // Legacy variants sometimes omit language. Infer only when the game catalog
    // is unambiguous: pokemon-japan => Japanese, pokemon => English.
...
export async function POST(request: Request) {
  try {
    ...
    const includeLocalizedCardmarket =
      body?.includeLocalizedCardmarket !== false && cards.length <= 3;
    ...
    const results = await mapWithConcurrency(cards, 3, async (card) => {
      ...
      const baseValue = mergePayloads(...baseParts);
      const localCardmarket = includeLocalizedCardmarket
        ? await fromCardmarketFrance(card, baseValue.cardmarket?.url)
        : emptyPayload();
      const value = mergePayloads(localCardmarket, baseValue);
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v30-local-fr",
