...
async function fromCardmarketAsia(card: InputCard): Promise<MarketPayload> {
  ...
  const identities = marketSetIdCandidates(card)
    .map((setId) =>
      `${String(setId).replace(/[^a-z0-9]/gi, "")}${cleanNumber(card.number).replace(/[^a-z0-9]/gi, "")}`
    )
...
async function fromEbay(card: InputCard): Promise<MarketPayload> {
  ...
  const variantKeyword = card.printingVariant && card.printingVariant !== "Normal"
    ? card.printingVariant
    : "";

  const localQuery = [card.name, cleanNumber(card.number), card.setName || card.setId, variantKeyword, languageKeyword, "Pokemon"]
    .filter(Boolean)
    .join(" ");
  const englishQuery = [englishIdentity.name, cleanNumber(card.number), englishIdentity.setName, variantKeyword, languageKeyword, "Pokemon"]
    .filter(Boolean)
    .join(" ");
  ...
  const marketSetIds = marketSetIdCandidates(card);
  const compactEnglishQueries = marketSetIds.map((marketSetId) =>
    [englishIdentity.name, cleanNumber(card.number), marketSetId, variantKeyword, languageKeyword]
      .filter(Boolean)
      .join(" ")
  );
  const setNumberQueries = marketSetIds.map((marketSetId) =>
    [marketSetId, cleanNumber(card.number), variantKeyword, languageKeyword, "Pokemon"]
      .filter(Boolean)
      .join(" ")
  );
  const simpleEnglishQuery = [englishIdentity.name, cleanNumber(card.number), variantKeyword, languageKeyword, "Pokemon"]
    .filter(Boolean)
    .join(" ");
...
export async function POST(request: Request) {
  try {
    ...
    const results = await mapWithConcurrency(cards, 3, async (card) => {
      ...
      const cacheKey = [
        "price-v51-variants-tcgcsv-japan",
        card.id,
        language,
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v51-variants-tcgcsv-japan",
