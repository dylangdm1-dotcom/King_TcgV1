...
async function fromJustTcg(card: InputCard): Promise<MarketPayload> {
  ...
  // JustTCG est principalement indexé avec les noms de son propre catalogue.
  // Une recherche FR (Dracaufeu) ne doit donc pas dépendre uniquement du nom
  // quand le fournisseur connaît la carte sous son nom EN. On essaie d'abord
  // l'identité set+numéro, puis le nom comme signal supplémentaire.
  ...
  for (const query of queries) {
    const params = new URLSearchParams({ q: query, game, limit: "20" });
    ...
    item = list.find((candidate: any) => {
      const sameNumber = normalizedNumber(candidate?.number) === expectedNumber;
      if (!sameNumber) return false;
      const candidateSetName = normalizedText(candidate?.set_name ?? candidate?.set?.name ?? candidate?.set);
      const candidateSetId = normalizedSet(candidate?.set_code ?? candidate?.set_id ?? candidate?.set?.id ?? candidate?.set);
      const sameSet =
        expectedSetId && candidateSetId
          ? expectedSetId === candidateSetId
          : Boolean(expectedSet && candidateSetName && (
      ...
      // Le nom est un contrôle supplémentaire uniquement lorsqu'il est dans la
      // même langue ; set+numéro restent l'identité forte.
      const candidateName = normalizedText(candidate?.name);
      return !expectedName || !candidateName || candidateName === expectedName || sameSet;
    });
    if (item) break;
  }
  ...
  const variants = (Array.isArray(item.variants) ? item.variants : []).filter(
    (variant: any) => {
      const condition = String(variant?.condition ?? "").toLowerCase().replace(/[^a-z]/g, "");
      const language = String(variant?.language ?? "").toLowerCase().trim();
      // JustTCG possède encore des variantes historiques sans champ language.
      // On peut l'inférer uniquement lorsque le catalogue lui-même est mono-marché :
      // pokemon => EN, pokemon-japan => JP. Jamais pour FR/CN.
      const inferredLanguage = !language && card.language === "en"
        ? "english"
        : !language && card.language === "ja" && game === "pokemon-japan"
          ? "japanese"
          : language;
      return (condition === "nearmint" || condition === "nm") &&
        inferredLanguage.includes(expectedLanguage) &&
  ...
  payload.justtcg = {
    ...
    url: item.url,
    updatedAt: item.updatedAt ?? variants[0]?.lastUpdated,
  };
  addQuote(payload, {
    source: "justtcg",
...
async function fromPokeWallet(card: InputCard): Promise<MarketPayload> {
  ...
  if (!apiKey || (!card.setId && !card.setName && !card.name) || !card.number) return emptyPayload();
    ...
    .map((quote) => ({
      ...
      reason:
        quote.language !== language
          ...
          : quote.condition !== "Near Mint"
            ? "État Near Mint non vérifié"
            : quote.classification !== "exact"
              ? "Cotation indicative ou comparable"
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-final-data-v1",
