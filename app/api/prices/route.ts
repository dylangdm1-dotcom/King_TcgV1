...
type MarketEstimatePayload = {
  price: number;
  language: CardLanguage;
  currency: "EUR";
  condition: "Near Mint";
  confidence: "high" | "medium" | "limited" | "none";
...
function candidateCardIds(card: InputCard): string[] {
  ...
  // L'identifiant présent dans une carte TCGdex est la référence la plus fiable.
  // Exemple : tcgdex-fr-swsh10-098 -> swsh10-098.
  const wrappedId = String(card.id ?? "").trim();
  const tcgdexMatch = wrappedId.match(
    /^tcgdex-(?:fr|en|ja|zh-tw|zh-cn)-(.+)$/i
  );
  if (tcgdexMatch?.[1]) {
    candidates.add(tcgdexMatch[1]);
  }
  ...
  if (setId && number) {
    // Conserver le numéro tel qu'il vient de TCGdex, y compris ses zéros.
    candidates.add(`${setId}-${number}`);

    const normalizedNumber = normalizeNumber(number);
    if (normalizedNumber && normalizedNumber !== number.toUpperCase()) {
      candidates.add(`${setId}-${normalizedNumber}`);
    }

    const prefix = number.match(/^(GG|TG|SV)\s*/i)?.[1]?.toLowerCase();
    if (prefix && !normalizeSetId(setId).endsWith(prefix)) {
      candidates.add(`${setId}${prefix}-${number}`);
    }
  }
...
function buildLanguageEstimate(payload: MarketPayload, language: CardLanguage = "en"): MarketEstimatePayload {
  ...
  if (language === "fr") {
    const cardmarketFr = safeNumber(
      cm.lowPrice ?? cm.averageSellPrice ?? cm.avg1 ?? cm.avg7 ?? cm.avg30 ?? cm.trendPrice
    );
    if (cardmarketFr !== undefined) included.push({ source: "cardmarket_fr_low", value: cardmarketFr });
    const justFr = safeNumber(payload.justtcg?.medianNearMint);
    if (justFr !== undefined && payload.justtcg?.language === "french") {
      included.push({ source: "justtcg_fr_nm", value: justFr });
    }
    if (payload.tcgplayer) excluded.push({ source: "tcgplayer", reason: "Marché anglais exclu du calcul français" });
  } else if (language === "en") {
    const cmEn = safeNumber(cm.lowPrice ?? cm.trendPrice ?? cm.averageSellPrice ?? cm.avg7 ?? cm.avg30);
    if (cmEn !== undefined) included.push({ source: "cardmarket_en", value: cmEn });
    const variants = payload.tcgplayer?.prices ?? {};
    const tcgValues = Object.values(variants).flatMap((v: any) => [safeNumber(v?.market)]).filter((v): v is number => v !== undefined);
    if (tcgValues.length) included.push({ source: "tcgplayer_en", value: median(tcgValues) });
    const justEn = safeNumber(payload.justtcg?.medianNearMint);
    if (justEn !== undefined && payload.justtcg?.language === "english") included.push({ source: "justtcg_en_nm", value: justEn });
  } else {
    const expected = language === "ja" ? "japanese" : "chinese";
    const justLocal = safeNumber(payload.justtcg?.medianNearMint);
    if (justLocal !== undefined && payload.justtcg?.language === expected) {
      included.push({ source: `justtcg_${language}_nm`, value: justLocal });
    }
    if (payload.cardmarket) excluded.push({ source: "cardmarket", reason: "Source occidentale non validée pour cette langue" });
    if (payload.tcgplayer) excluded.push({ source: "tcgplayer", reason: "Marché anglais exclu pour cette langue" });
  }
  ...
  return {
    ...
    includedSources: included.map((item) => item.source),
