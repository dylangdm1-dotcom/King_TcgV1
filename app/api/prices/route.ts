...
async function fromTcgdex(card: InputCard): Promise<MarketPayload> {
  ...
  for (const id of exactCardIds(card)) {
    ...
    if (cardmarket) {
      ...
      const display = low ?? trend ?? average;
      if (display) {
        addQuote(payload, {
          source: "cardmarket",
          ...
          language: "multi",
          condition: "Near Mint",
          metric: low ? "lowest_listing" : "market",
          compatible: card.language === "en",
          confidence: "limited",
          updatedAt: cardmarket.updated,
        });
      }
...
async function fromPokemonTcg(card: InputCard): Promise<MarketPayload> {
  ...
  if (data.cardmarket?.prices) {
    ...
    const value = numberValue(
      data.cardmarket.prices.lowPrice ??
    ...
    if (value) {
      addQuote(payload, {
        source: "cardmarket",
        label: "Cardmarket Europe via Pokémon TCG API",
        price: value,
        currency: "EUR",
        language: "en",
        condition: "Near Mint",
...
async function fromPokeWallet(card: InputCard): Promise<MarketPayload> {
  ...
  const query = [card.setId, cleanNumber(card.number), card.name]
    .filter(Boolean)
    .join(" ");
  const result = await fetchJson(
  ...
  const item = results.find((candidate: any) => matchesPokeWallet(candidate, card));
  if (!item) return emptyPayload();
  ...
  if (cardmarketValue) {
    ...
    addQuote(payload, {
      source: "pokewallet",
      label: `Cardmarket via PokéWallet (${requestedLanguage})`,
      price: cardmarketValue,
      currency: "EUR",
      language: card.language ?? "en",
      condition: "Near Mint",
      metric: preferredCm?.low ? "lowest_listing" : "market",
      compatible: languageCompatible,
      confidence: languageCompatible ? "high" : "limited",
      url: item.cardmarket?.product_url,
      updatedAt: preferredCm?.updated_at,
    });
  }
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v21-pokewallet-optional",
