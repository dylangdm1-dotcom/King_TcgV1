...
function matchesPokeWallet(candidate: any, card: InputCard): boolean {
  ...
  if (imageLanguages.length) {
    ...
    if (card.language === "zh-tw" && !imageLanguages.some((v: string) => v === "zh" || v === "chn" || v === "cn")) return false;
  }
  return true;
}
...
export async function POST(request: Request) {
  try {
    ...
    const results = await mapWithConcurrency(cards, 3, async (card) => {
      ...
      const cacheKey = [
        "price-v60-jp-legacy-cn-price-identity",
        card.id,
        language,
      ...
      const settled = await Promise.allSettled([
        ...
        fromPokeWallet(card),
        // The visible CN catalogue is the mainland/Simplified-Chinese PokéWallet
        // catalogue (CSV/CBB codes). TCGdex zh-tw is a different regional
        // catalogue and must not silently become its price source.
        language === "zh-tw" ? Promise.resolve(emptyPayload()) : fromTcgdex(card),
        language === "zh-tw" ? Promise.resolve(emptyPayload()) : fromPokemonTcg(card),
        fromJustTcg(card),
        fromEbay(card),
      ]);
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v55-fr-ebay-cn-market",
