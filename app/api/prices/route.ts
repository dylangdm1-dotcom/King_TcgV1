...
async function fromJustTcg(card: InputCard): Promise<MarketPayload> {
  ...
  if (!card.number && !card.variantTcgplayerId) {
    payload.debugJustTcg.stage = "missing_card_identity";
    return payload;
  }

  // Free-plan friendly: one API request maximum per refreshed card.
  // Keep the query minimal and documented; exact identity/variant validation
  // happens locally from the returned Card/Variant objects.
  ...
  if (!selectedCard || (!card.variantTcgplayerId && bestScore < 5)) {
...
export async function POST(request: Request) {
  try {
    ...
    const results = await mapWithConcurrency(cards, 3, async (card) => {
      ...
      const cacheKey = [
        language === "zh-tw" ? "price-v101-cn-variant" : "price-v101-variant",
        card.id,
        language,
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v101-fr-proxy-justtcg-minimal",
