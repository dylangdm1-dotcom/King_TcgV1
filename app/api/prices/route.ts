...
import { buildMarketCacheKeyV275 } from "@/lib/market-cache/canonical";
import {
  MARKET_CACHE_NEGATIVE_FRESH_MS,
  MARKET_CACHE_POSITIVE_FRESH_MS,
  type ServerMarketCacheEnvelopeV275,
} from "@/lib/market-cache/server";
import {
  beginSharedMarketRefreshV276,
  endSharedMarketRefreshV276,
  readSharedMarketCacheBatchV276,
  writeSharedMarketCacheV276,
  type MarketCacheBackendV276,
} from "@/lib/market-cache/persistent";
import { appendMarketHistoryV277 } from "@/lib/market-cache/history";
import {
  inferMarketLanguageFromCardIdV279,
  normalizeMarketLanguageV279,
...
function hasUsableMarketPayload(payload: MarketPayload): boolean {
  if (Number(payload.estimate?.price || 0) > 0) return true;
  if (payload.quotes.some((quote) => Number(quote.price || 0) > 0)) return true;
  if (Number(payload.justtcg?.currentPrice || payload.justtcg?.medianNearMint || 0) > 0) return true;
  if (Number(payload.ebayListings?.median || payload.ebayListings?.average || 0) > 0) return true;
  return Boolean(
    payload.cardmarket?.prices &&
    Object.values(payload.cardmarket.prices).some((value) => Number(value || 0) > 0)
  );
}

function representativeHistoryQuote(
  payload: MarketPayload,
  source: MarketQuote["source"],
  language: CardLanguage
): { price: number; classification?: "exact" | "comparable" | "indicative" } {
  const compatible = payload.quotes.filter(
    (quote) =>
      quote.source === source &&
      quote.compatible &&
      quote.price > 0 &&
      (quote.language === language || quote.language === "multi")
  );
  const exact = compatible.filter((quote) => quote.classification === "exact");
  const selected = exact.length ? exact : compatible;
  if (!selected.length) return { price: 0 };
  const classification = selected.some((quote) => quote.classification === "exact")
    ? "exact"
    : selected.some((quote) => quote.classification === "comparable")
    ? "comparable"
    : "indicative";
  return {
    price: Number(median(selected.map((quote) => quote.price)).toFixed(2)),
    classification,
  };
}

function marketHistoryPoint(
  payload: MarketPayload,
  card: InputCard,
  language: CardLanguage,
  date = Date.now()
): MarketHistoryPoint | null {
  const average = Number(payload.estimate?.price || 0);
  if (!Number.isFinite(average) || average <= 0) return null;
  const cardmarket = representativeHistoryQuote(payload, "cardmarket", language);
  const ebay = representativeHistoryQuote(payload, "ebay", language);
  const tcgplayer = representativeHistoryQuote(payload, "tcgplayer", language);
  const justtcg = representativeHistoryQuote(payload, "justtcg", language);
  return {
    date,
    day: new Date(date).toISOString().slice(0, 10),
    cardmarket: cardmarket.price,
    ebay: ebay.price,
    tcgplayer: tcgplayer.price,
    justtcg: justtcg.price,
    average: Number(average.toFixed(2)),
    origin: "observed",
    language,
    condition: card.condition || "Near Mint",
    printingVariant: card.printingVariant || "Normal",
    confidence: payload.estimate?.confidence || "none",
    sourceCount: payload.estimate?.includedSources?.length || 0,
    sourceClassifications: {
      ...(cardmarket.classification ? { cardmarket: cardmarket.classification } : {}),
      ...(ebay.classification ? { ebay: ebay.classification } : {}),
      ...(tcgplayer.classification ? { tcgplayer: tcgplayer.classification } : {}),
      ...(justtcg.classification ? { justtcg: justtcg.classification } : {}),
    },
  };
}

function withMarketCacheMetadata(
  payload: MarketPayload,
  key: string,
  state: MarketCacheMetadata["state"],
  entry: ServerMarketCacheEnvelopeV275<MarketPayload>,
  ...
  return {
    ...
    cache: {
      ...
      state,
      backend,
      scope: backend === "redis-rest" ? "multi-instance" : "instance",
...
async function fromJustTcg(card: InputCard): Promise<MarketPayload> {
  ...
  payload.debugJustTcg.availableConditions = Array.from(
    new Set(
      allVariants
        .map((variant: any) => String(variant?.condition || "").trim())
        .filter(Boolean)
    )
  );
  ...
  let compatibleVariants = exactVariants;

  // JustTCG printing labels are provider-owned. For a generic Foil request,
  // if the exact label is absent but there is only one priced NM printing for
  // the expected language, use that sole physical printing. Never do this for
  // Reverse/Poké Ball/Master Ball/1st Edition, where the printing is identity.
  if (
    !compatibleVariants.length &&
  ...
  const selectedVariant = [...compatibleVariants].sort(
...
function calculateEstimate(payload: MarketPayload, language: CardLanguage): MarketPayload["estimate"] {
  const exactLocalQuotes = payload.quotes.filter(
    (quote) => quote.compatible && quote.language === language && quote.price > 0
  );

  // JustTCG can expose the exact physical card/condition/printing only in a
  // different language. Keep the source labelled Comparable, but allow it to
  // contribute softly to King_TCG instead of discarding a coherent market signal.
  // This rule is language-agnostic and therefore applies to FR / EN / JP / CN.
  const comparableJustTcgQuotes = payload.quotes.filter(
    (quote) =>
  ...
  const compatibleInitial = [
    ...exactLocalQuotes,
    ...comparableJustTcgQuotes,
  ];
  ...
  if (compatible.length) {
      ...
      (acc, quote) => {
        const sourceWeight =
          quote.source === "ebay"
            ...
            : quote.source === "justtcg" && quote.classification === "comparable"
              ? 0.65
              : 1;
        acc.sum += quote.price * sourceWeight;
        acc.weight += sourceWeight;
        return acc;
      },
      { sum: 0, weight: 0 }
    );
    ...
    return {
      ...
      currency: "EUR",
      condition: "Near Mint",
      confidence: filtered.some(
        (quote) => quote.confidence === "high" && quote.classification !== "comparable"
      )
        ? "high"
        : filtered.length >= 2
          ? "medium"
...
export async function POST(request: Request) {
  try {
    const rateLimited = enforceRateLimit(request, "prices", { limit: 90, windowMs: 60_000 });
    if (rateLimited) return rateLimited;

    const oversized = rejectOversizedContentLength(request, 2_000_000);
    if (oversized) return oversized;

    const parsedBody = await readJsonBodyWithLimit<any>(request, 2_000_000);
    if ("error" in parsedBody) return parsedBody.error;
    const body = parsedBody.data;
    const rawCards: any[] = Array.isArray(body?.cards) ? body.cards : [];
    const invalidLanguage = rawCards.find(
      (card: any) => card?.language !== undefined && normalizeMarketLanguageV279(card.language) === null
    );
    if (invalidLanguage) {
      return NextResponse.json(
        {
          success: false,
          error: "Langue marché invalide. Valeurs autorisées : fr, en, ja, zh-tw.",
          code: "invalid_market_language",
        },
        { status: 400 }
      );
    }

    const cards: InputCard[] = rawCards
      .filter(
        (card: any) =>
      ...
      .slice(0, 20)
      .map((card: any): InputCard => ({
        ...card,
        language:
          normalizeMarketLanguageV279(card.language) ??
          inferMarketLanguageFromCardIdV279(card.id),
      }));
    const preparedCards = cards.map((card) => {
      const language = card.language ?? "en";
      const cacheKey = buildMarketCacheKeyV275({
        id: card.id,
        language,
        setId: card.setId,
        number: card.number,
      ...
      return { card, language, cacheKey, resultKey };
    });
    const cachedByKey = await readSharedMarketCacheBatchV276<MarketPayload>(
      preparedCards.map(({ cacheKey }) => cacheKey)
    );
    const results = await mapWithConcurrency(preparedCards, 3, async (prepared) => {
      const { card, language, cacheKey, resultKey } = prepared;

      const cached = cachedByKey.get(cacheKey)!;
      if (cached.state === "fresh") {
        return [
          resultKey,
          withMarketCacheMetadata(cached.entry.value, cacheKey, "fresh", cached.entry, cached.backend),
        ] as const;
      }
      const refreshLease = cached.state === "stale"
        ? await beginSharedMarketRefreshV276(cacheKey)
        : null;
      const ownsRefresh = refreshLease?.acquired ?? true;
      if (!ownsRefresh && cached.state === "stale") {
        return [
          resultKey,
          withMarketCacheMetadata(
            cached.entry.value,
            cacheKey,
            "stale-fallback",
            cached.entry,
      ...
      // Une indisponibilité fournisseur ne doit jamais effacer le dernier prix
      // exploitable. On sert alors le snapshot périmé en le signalant clairement.
      if (
        cached.state === "stale" &&
        hasUsableMarketPayload(cached.entry.value) &&
        !hasUsableMarketPayload(value)
      ) {
        if (refreshLease) await endSharedMarketRefreshV276(cacheKey, refreshLease);
        return [
          resultKey,
          withMarketCacheMetadata(
            cached.entry.value,
      ...
      const historyPoint = marketHistoryPoint(value, card, language);
      if (historyPoint) {
        const recorded = await appendMarketHistoryV277({ key: cacheKey, point: historyPoint });
        value.history = recorded.history;
        value.historyBackend = recorded.backend;
      }

      const stored = await writeSharedMarketCacheV276({
        key: cacheKey,
        value,
        freshForMs: hasUsableMarketPayload(value)
          ? MARKET_CACHE_POSITIVE_FRESH_MS
          : MARKET_CACHE_NEGATIVE_FRESH_MS,
      });
      if (refreshLease) await endSharedMarketRefreshV276(cacheKey, refreshLease);
      return [
        resultKey,
        withMarketCacheMetadata(value, cacheKey, "network", stored.entry, stored.backend),
      ] as const;
    });

    return NextResponse.json({
      success: true,
      version: "price-engine-v112-multilingual-scanner",
