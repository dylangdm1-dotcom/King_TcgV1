...
async function fromEbay(card: InputCard): Promise<MarketPayload> {
  ...
  const compactEnglishQuery = [englishIdentity.name, cleanNumber(card.number), card.setId, languageKeyword]
    .filter(Boolean)
    .join(" ");
  const setNumberQuery = [card.setId, cleanNumber(card.number), languageKeyword, "Pokemon"]
    .filter(Boolean)
    .join(" ");
  const simpleEnglishQuery = [englishIdentity.name, cleanNumber(card.number), languageKeyword, "Pokemon"]
    .filter(Boolean)
    .join(" ");
  ...
  const queries = Array.from(
    new Set(
      (card.language === "ja" || card.language === "zh-tw"
        ? [compactEnglishQuery, simpleEnglishQuery, setNumberQuery, englishQuery, localQuery]
        : [localQuery, englishQuery]
      ).filter(Boolean)
    )
  ).slice(0, card.language === "ja" || card.language === "zh-tw" ? 5 : 2);
  ...
  if (card.language === "zh-tw") {
    ...
    const candidateSummaries = listings
      ...
      const hasExactSetCode = Boolean(
        normalizedSetId &&
        normalizedTitle.includes(normalizedSetId)
      );
      const hasPokemonName = searchNames.some((name) => titleMatchesPokemon(title, String(name)));
  
      // For JP/CN, set code + exact card number + explicit language is a strong
      // identity even when TCGdex has no English translation for the card name.
      if (!hasPokemonName) {
        if (
          !(card.language === "ja" || card.language === "zh-tw") ||
      ...
      const structured = ebayStructuredIdentity(listing, card, searchNames);
      const exactNumberInText = titleHasExactNumber(identityText, card.number);
      ...
      const normalizedSetNames = searchSets.map((value) => normalizedText(String(value))).filter((value) => value.length >= 4);
      const normalizedSetId = normalizedSet(card.setId);
      const hasExactSetName = normalizedSetNames.some((setName) => normalizedIdentity.includes(setName));
      const hasExactSetCode = Boolean(
        normalizedSetId &&
        normalizedIdentity.includes(normalizedSetId)
      );
      const hasPokemonName =
        searchNames.some((name) => titleMatchesPokemon(identityText, String(name))) ||
        structured.matchingName;
  
      // V48 CN/JP: structured eBay aspects can identify a variation even when the
      // parent listing title is generic (e.g. "Pokemon SV8a T-Chinese Language").
      if (!hasPokemonName) {
        if (
          !(card.language === "ja" || card.language === "zh-tw") ||
          !(structured.exactNumber && (structured.matchingLanguage || language === card.language)) ||
      ...
      // Unknown title language is acceptable when eBay structured aspects confirm
      // the requested language, or when the exact local set name/code is present.
      if (language !== card.language) {
        if (
          language !== "unknown" ||
    ...
    const conditionText = `${String(listing?.title ?? "")} ${ebayAspectText(listing)} ${String(listing?.condition ?? "")}`;
    if (isNearMintTitle(conditionText) || /near\s*mint|\bnm\b/i.test(conditionText)) exactLanguageNm.push(item);
  }
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v48-2-cn-item-details",
