...
async function fromJustTcg(card: InputCard): Promise<MarketPayload> {
  ...
  if (!card.number) {
    payload.debugJustTcg.stage = "missing_card_number";
    return payload;
  }

  // V1 stable API. For the Free plan we deliberately spend at most one
  // JustTCG request per refreshed card.
  ...
  if (card.variantTcgplayerId) {
    params = new URLSearchParams({
      tcgplayerId: String(card.variantTcgplayerId),
      include_price_history: "false",
      include_statistics: "false",
    });
    lookup = `tcgplayerId:${card.variantTcgplayerId}`;
    payload.debugJustTcg.stage = "lookup_tcgplayer_id";
  } else {
    params = new URLSearchParams({
      game,
      number: cleanNumber(card.number),
    ...
    const q = String(card.setName || card.setId || card.name || "").trim();
    if (q) params.set("q", q);
    lookup = `search:${q || cleanNumber(card.number)}`;
    payload.debugJustTcg.stage = "lookup_single_search";
  }

  payload.debugJustTcg.lookup = lookup;
  ...
  payload.debugJustTcg.candidateCount = list.length;
  ...
  for (const candidate of list) {
    ...
    if (!card.variantTcgplayerId && candidateNumber !== expectedNumber) continue;

    const candidateTcgplayerId = String(candidate?.tcgplayerId ?? "");
    if (
      card.variantTcgplayerId &&
      candidateTcgplayerId &&
      candidateTcgplayerId !== String(card.variantTcgplayerId)
    ) {
      continue;
    }
    ...
    const setFields = rawFields
      .map((v) => normalizedSet(String(v ?? "")))
      .filter(Boolean);
    const textFields = [
      candidate?.set,
      candidate?.set_name,
      ...
      .map((v) => normalizedText(String(v ?? "")))
      .filter(Boolean);

    let score = card.variantTcgplayerId ? 20 : 0;
    if (
      expectedSetId &&
      setFields.some(
        (field) => field === expectedSetId || field.includes(expectedSetId)
      )
    ) score += 6;
    if (
      expectedSetName &&
      textFields.some((field) => field === expectedSetName)
    ) score += 5;
    if (
      expectedSetName &&
      textFields.some(
        (field) =>
          field.includes(expectedSetName) ||
          expectedSetName.includes(field)
      )
    ) score += 3;
  ...
  if (!bestItem || (!card.variantTcgplayerId && bestScore < 5)) {
    payload.debugJustTcg.stage = "no_exact_card_match";
    return payload;
  }

  const variants = (Array.isArray(bestItem.variants) ? bestItem.variants : [])
    .filter((variant: any) => {
      ...
      const wantedLanguage = normalizedText(expectedLanguage);
      if (
        variantLanguage &&
        !variantLanguage.includes(wantedLanguage) &&
        !wantedLanguage.includes(variantLanguage)
      ) return false;
      ...
      if (wantedPrinting) {
        ...
        if (
          printing &&
          printing !== wantedPrinting &&
          !printing.includes(wantedPrinting) &&
          !wantedPrinting.includes(printing)
        ) return false;
      }
      return true;
    });

  payload.debugJustTcg.matchingVariantCount = variants.length;
  payload.debugJustTcg.language = expectedLanguage;
  payload.debugJustTcg.printing = String(card.printingVariant || "Normal");

  if (!variants.length) {
    payload.debugJustTcg.stage = "no_matching_nm_variant";
    return payload;
  }
  ...
  const usdPrice = Number(median(usdValues).toFixed(2));
  const rate = await usdToEur();
  const eurPrice = Number((usdPrice * rate).toFixed(2));

  payload.debugJustTcg.stage = "price_found";
  payload.debugJustTcg.selectedPriceUsd = usdPrice;
  payload.debugJustTcg.selectedPriceEur = eurPrice;

  payload.justtcg = {
    medianNearMint: eurPrice,
    language: expectedLanguage,
    printing: String(variants[0]?.printing ?? card.printingVariant ?? ""),
    sampleSize: variants.length,
    url: bestItem.url,
    updatedAt:
      variants[0]?.lastUpdated ??
      bestItem.updatedAt ??
      bestItem.lastUpdated,
  };

  addQuote(payload, {
    source: "justtcg",
    label: `JustTCG · NM · ${expectedLanguage}`,
    price: eurPrice,
    currency: "EUR",
    language: card.language ?? "en",
    condition: "Near Mint",
...
export async function POST(request: Request) {
  try {
    ...
    const results = await mapWithConcurrency(cards, 3, async (card) => {
      ...
      const cacheKey = [
        language === "zh-tw" ? "price-v99-cn-variant" : "price-v99-variant",
        card.id,
        language,
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v99-justtcg-v1",
