...
async function fromEbay(card: InputCard): Promise<MarketPayload> {
  ...
  for (const listing of listings) {
    ...
    if (isNearMintTitle(identityText) || /near\s*mint|nm/i.test(String(listing?.condition || ""))) exactLanguageNm.push(item);
  }
...
export async function POST(request: Request) {
  try {
    ...
    const results = await mapWithConcurrency(cards, 3, async (card) => {
      ...
      const cacheKey = [
        "price-v48-1-ebay-aspects-cn-buildfix",
        card.id,
        language,
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v48-1-ebay-aspects-cn-buildfix",
