...
function extractCardmarketMarketStats(htmlText: string): {
  ...
  const text = stripHtmlText(htmlText);
  const read = (labels: RegExp): number | undefined => {
    const match = text.match(
      new RegExp(`${labels.source}[^€0-9]{0,80}(?:€\\s*)?([0-9]{1,6}(?:[.,][0-9]{1,2})?)\\s*€?`, "i")
    );
...
async function fromCardmarketAsia(card: InputCard): Promise<MarketPayload> {
  ...
  const identity = cardmarketAsiaIdentity(card);
  if (!identity || identity.length < 4) return emptyPayload();

  const searchUrl =
    `https://www.cardmarket.com/en/Pokemon/Products/Search?searchString=${encodeURIComponent(identity)}`;
  ...
  if (!search.data) {
    ...
    return emptyPayload(sourceStatus(search.status));
  }

  const productUrl = findExactCardmarketProductUrl(search.data, identity);
  if (!productUrl) {
    console.info("[prices][cardmarket-asia]", {
      cardId: card.id,
      identity,
...
async function fromEbay(card: InputCard): Promise<MarketPayload> {
  ...
  if (card.language === "zh-tw") {
    ...
    const candidateSummaries = listings
      .filter((listing: any) => {
        ...
        const normalizedTitle = normalizedText(title);
        return (
          titleHasExactNumber(title, card.number) ||
          Boolean(normalizedSetId && normalizedTitle.includes(normalizedSetId)) ||
  ...
  for (const listing of listingsForMatching) {
    ...
    if (card.language === "ja") {
      ...
      const hasExactSetCode = Boolean(
        normalizedSetId && normalizedIdentity.includes(normalizedSetId)
      );
...
export async function POST(request: Request) {
  try {
    ...
    const results = await mapWithConcurrency(cards, 3, async (card) => {
      ...
      const cacheKey = [
        "price-v49-language-market-routes",
        card.id,
        language,
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v49-language-market-routes",
