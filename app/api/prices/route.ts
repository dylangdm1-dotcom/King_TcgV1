...
async function fromEbay(card: InputCard): Promise<MarketPayload> {
  ...
  const result = await fetchJson(
    `https://api.ebay.com/buy/browse/v1/item_summary/search?${params.toString()}`,
    {
      headers: {
        ...
        "Accept-Language": card.language === "fr" ? "fr-FR" : card.language === "en" ? "en-US" : "en-US",
      },
    }
  );
  ...
  for (const listing of listings) {
    ...
    const normalizedTitle = normalizedText(title);
    const normalizedSetName = normalizedText(card.setName);
    const hasExactSetName = normalizedSetName.length >= 4 && normalizedTitle.includes(normalizedSetName);
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v31-unified",
