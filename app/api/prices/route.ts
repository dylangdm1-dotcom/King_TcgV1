  ...
  try {
    ...
    const results = await Promise.all(
      uniqueCards.map(async (card) => {
        ...
        // Prefer the TCGdex price when the exact card was resolved there.
        // JustTCG remains a fallback because its broad name/number matching can
        // otherwise attach another printing (notably promos) to the card.
        const tcgplayer = tcgdexMarket.tcgplayer ?? justTcg?.tcgplayer;
