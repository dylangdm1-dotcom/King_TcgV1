...
async function fromEbay(card: InputCard): Promise<MarketPayload> {
  ...
  const compactEnglishQueries = marketSetIds.map((marketSetId) =>
    [englishIdentity.name, cleanNumber(card.number), marketSetId, languageKeyword]
      .filter(Boolean)
      .join(" ")
  );
  const setNumberQueries = marketSetIds.map((marketSetId) =>
    [marketSetId, cleanNumber(card.number), languageKeyword, "Pokemon"]
      .filter(Boolean)
      .join(" ")
  );
...
export async function POST(request: Request) {
  try {
    ...
    const results = await mapWithConcurrency(cards, 3, async (card) => {
      ...
      const cacheKey = [
        "price-v50-asian-market-identity",
        card.id,
        language,
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v50-asian-market-identity",
