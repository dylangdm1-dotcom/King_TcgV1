...
async function fromEbay(card: InputCard): Promise<MarketPayload> {
  ...
  addQuote(payload, {
    ...
    compatible: exactNm || ((card.language === "ja" || card.language === "zh-tw") && selected.length >= 2),
    confidence:
...
function calculateEstimate(payload: MarketPayload, language: CardLanguage): MarketPayload["estimate"] {
  ...
  if (compatible.length) {
    ...
    // JP/CN: exact-language eBay/JustTCG remain the primary signal. An exact
    // Cardmarket value attached to the same TCGdex card is a useful European
    // reference, but it receives only 15% weight and is bounded to avoid bad
    // cross-market mappings from distorting the local quote.
    ...
    if ((language === "ja" || language === "zh-tw") && regional?.price) {
      const boundedRegional = Math.min(localCenter * 1.4, Math.max(localCenter * 0.6, regional.price));
      finalPrice = localCenter * 0.85 + boundedRegional * 0.15;
      includedSources.push(`${regional.label} · référence 15 %`);
    }
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v42-market-languages",
