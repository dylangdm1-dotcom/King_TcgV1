...
async function fromCardmarketFrance(
  ...
  let resolvedProductUrl =
    (typeof card.directCardmarketUrl === "string" && card.directCardmarketUrl.includes("cardmarket.com")
...
export async function POST(request: Request) {
  try {
    ...
    const results = await mapWithConcurrency(cards, 3, async (card) => {
      ...
      const cacheKey = [
        language === "zh-tw" ? "price-v95-cn-variant" : "price-v95-variant",
        card.id,
        language,
    ...
    return NextResponse.json({
      success: true,
      version: "price-engine-v95-cardmarket-direct-url",
