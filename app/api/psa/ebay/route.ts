...
function looksLikeNonSingleCard(title: string): boolean {
  return /\b(lot|bundle|booster|display|box|proxy|custom|fan\s*art|digital|code|online|video\s*game|jeu\s*video|console|nintendo\s*(?:ds|3ds|switch|64)|game\s*boy|gameboy|wii|playstation|xbox|amiibo|dvd|blu\s*ray)\b/i.test(title);
}

function looksLikePokemonCardListing(title: string): boolean {
  // The eBay query already contains Pokémon, but Browse can still broaden results.
  // Require explicit Pokémon/card evidence before a PSA item reaches the UI.
  return /pok[eé]mon/i.test(title) && /\b(?:card|carte|tcg|ccg)\b/i.test(title);
}
...
export async function GET(request: Request) {
  ...
  const candidates = Array.from(byId.values())
    .map((item: any) => {
      ...
      if (!looksLikePokemonCardListing(title)) return null;
