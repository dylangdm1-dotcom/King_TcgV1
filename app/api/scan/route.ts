...
const MAX_SCAN_REQUEST_BYTES = 7_000_000;
const MAX_SCAN_BASE64_LENGTH = 6_500_000;
...
- confidence mesure uniquement la lisibilité de l'identité de la carte, pas la certitude du prix ni de l'édition finale.
- Sans numéro ET sans extension lisibles, confidence ne doit pas dépasser 50.
- Avec seulement deux signaux parmi nom, numéro et extension, confidence ne doit pas dépasser 75.
- Même avec les trois signaux lisibles, confidence ne doit jamais dépasser 95.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const parsedBody = await readJsonBodyWithLimit(req, MAX_SCAN_REQUEST_BYTES);
    ...
    if (!mimeType || !SUPPORTED_SCAN_MIME_TYPES.has(mimeType)) {
      return apiError("Format d’image non pris en charge.", 415, "unsupported_image_type");
    }
    ...
    for (const modelName of models) {
      try {
        ...
        // les signaux d'identité réellement retournés. Un modèle ne peut donc
        // pas annoncer 95 % avec un simple nom ou sans numéro d'extension.
        const identityConfidenceCap =
          identitySignals >= 3 ? 95 : identitySignals === 2 ? 75 : 50;
        const boundedConfidence = Math.min(data.confidence, identityConfidenceCap);
        const responseData = {
          ...data,
          confidence: boundedConfidence,
          needsReview: boundedConfidence < 65 || identitySignals < 2,
        };
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Erreur technique lors de l'analyse de l'image.",
        code: "scan_failed",
