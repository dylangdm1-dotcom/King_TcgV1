  ...
  useEffect(() => {
    ...
    fetch("/api/auth/session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ accessToken, refreshToken, expiresIn }) })
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(data?.error || "Impossible de créer la session King_TCG.");
        window.history.replaceState({}, "", "/auth/callback");
        router.replace("/parametres/compte?connected=1");
        router.refresh();
      })
      .catch((error) => {
        setMessage(error instanceof Error ? error.message : "Impossible de créer la session King_TCG.");
        window.setTimeout(() => router.replace("/parametres/compte?error=session"), 5000);
      });
  }, [router]);
