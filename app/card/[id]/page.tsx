...
export default function CardPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        void enrichAndCacheCards([result])
          .then((enriched) => {
            if (isMounted && enriched[0]) setCard(enriched[0]);
          })
          .catch((priceError) => {
            console.warn("Synchronisation prix fiche indisponible :", priceError);
          });
        ...
        if (isMounted) {
          ...
          if (typeof predictPrice === "function") {
            try {
              const pred = predictPrice(marketHistory, s, result);
              if (pred) {
                setPrediction({
  ...
  // Réaligner tous les modules de la fiche dès que les prix asynchrones arrivent.
  useEffect(() => {
    if (!card) return;
    try {
      const market = getMarketData(card);
      const history = getMarketHistory(card.id) || [];
      const s = typeof getInvestmentScore === "function" ? getInvestmentScore(card, history) : 5;
      setScore(s);
      const current = Number(market.average || 0);
      const marketValues = [current, Number(market.minimum || 0), Number(market.maximum || 0), Number(market.cardmarket || 0), Number(market.cardmarketEurope || 0), Number(market.tcgplayer || 0), Number(market.justtcg || 0), Number(market.ebay || 0)].filter((value) => Number.isFinite(value) && value > 0);
      setPriceInfo((previous) => ({ ...previous, current, lowest: marketValues.length ? Math.min(...marketValues) : current, highest: marketValues.length ? Math.max(...marketValues) : current }));
      if (typeof predictPrice === "function") {
        const pred = predictPrice(history, s, card);
        setPrediction({ predictedPrice30d: Number(pred?.predictedPrice30d || current || 0), roi30d: Number(pred?.roi30d || 0), confidence: Number(pred?.confidence || 0) });
      }
      try { trackCardPrice(card, true); } catch {}
    } catch (recalculationError) {
      console.warn("[King_TCG] Recalcul fiche indisponible :", recalculationError);
    }
  }, [card?.id, card?.computedPrice, card?.marketEstimate?.price, card?.cardmarket?.prices?.trendPrice, card?.cardmarket?.prices?.avg30, card?.cardmarket?.prices?.avg7, card?.cardmarket?.prices?.avg1, card?.justtcg?.medianNearMint]);
  ...
  const emptyMarket = { cardmarket: 0, cardmarketEurope: 0, ebay: 0, tcgplayer: 0, justtcg: 0, average: 0, priceTrend7d: 0, priceTrend30d: 0, validSourceCount: 0 };
  let market = emptyMarket;
  let spread = 0;
  try {
    market = typeof getMarketData === "function" ? getMarketData(card) : emptyMarket;
    spread = typeof getMarketSpread === "function" ? getMarketSpread(card) : 0;
  } catch (marketError) {
    console.warn("Données marché invalides ignorées :", marketError);
  }
        ...
        <div className="relative mx-auto max-w-7xl space-y-5 px-4 py-5 sm:space-y-6 sm:px-6 sm:py-6 lg:px-8">
          ...
          <section className="rounded-2xl border border-white/[0.08] bg-[#18202a] px-4 py-3.5 sm:px-5">
            <p className="text-[11px] font-medium leading-5 text-zinc-400">
              ...
              les cotations exactes, indicatives et comparables restent séparées. Pour les cartes françaises sans cote locale exacte, l’Estimation marché FR utilise temporairement la statistique Cardmarket la plus élevée hors prix bas ; Cardmarket Europe affiche la deuxième plus élevée.
            </p>
          </section>
