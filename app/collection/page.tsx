...
*
* Rôle :
* * Vue globale de la collection physique
* * Valorisation marché
* * Watchlist
* * KPIs portfolio
*
* La logique de prix reste centralisée dans :
* calculateRealMarketPrices()
*
* IMPORTANT :
* Cette page ne doit pas recréer sa propre logique de prix.
  */

/**

* Carte collection avec quantité stockée.
  */
...
* Résultat marché mémorisé pour éviter de recalculer
* plusieurs fois le prix d'une même carte.
  */
  type MarketPriceCache = Record<string, number>;

/**

* Extraction sécurisée de la quantité.
*
* Le stockage historique peut contenir :
* * directement un nombre
* * un objet { quantity: number }
*
* Toute valeur invalide retombe à 1.
  */
  function getCardQuantity(value: unknown): number {
...
return Math.max(1, value.quantity);
}
...
try {
  ...
  const [collectionResults, favoriteResults] = await Promise.all([
    Promise.all(
...
* ======================================================
*
* Un seul calcul par carte.
*
* Toutes les statistiques utilisent ensuite ce cache.
  */
  const collectionPriceCache = useMemo<MarketPriceCache>(() => {
...
for (const card of collectionCards) {
  ...
  cache[card.id] = Number.isFinite(market.average)
    ? market.average ?? 0
    : 0;
}
...
* Cache prix Watchlist.
*
* Séparé de la collection pour éviter les recalculs.
  */
  const favoritePriceCache = useMemo<MarketPriceCache>(() => {
  const cache: MarketPriceCache = {};
...
let topCard: CollectionCardType | null = null;
...
for (const card of collectionCards) {
  ...
  const cardPrice = collectionPriceCache[card.id] ?? 0;

  if (cardPrice > 0) {
    pricedCount++;
  }

  value += cardPrice * quantity;
...
const ratio =
  collectionCards.length > 0
    ? Math.round(
        (pricedCount / collectionCards.length) * 100
      )
    : 0;
...
return {
  ...
  topCardPrice: maxPrice,
  liquidityRatio: ratio,
  lastCollection: collectionCards.slice(0, 6),
  lastFavorites: favoriteCards.slice(0, 6),
};

}, [collectionCards, favoriteCards, collectionPriceCache]);

/**

* Nombre d'actifs Watchlist disposant actuellement
* d'une valorisation marché.
  */
  const monitoredFavoritesCount = useMemo(() => {
  return favoriteCards.filter(
  (card) => (favoritePriceCache[card.id] ?? 0) > 0
  ).length;
  }, [favoriteCards, favoritePriceCache]);
  ...
  <main className="min-h-screen bg-black text-white px-4 sm:px-8 py-6 pb-24">
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <section className="space-y-4 pt-2">
            ...
            {lastCollection.map((card) => {
              ...
              return (
                ...
                >
                  ...
                  {quantity > 1 && (
                    <div className="absolute right-3 top-3 rounded-lg bg-black/80 border border-cyan-500/40 px-2 py-0.5 text-[10px] font-black text-cyan-400 shadow-xl z-10 backdrop-blur-md">
                      x{quantity}
                    </div>
                  )}
                </div>
        ...
        {/* RÉSUMÉ INVENTAIRE */}
        <div className="rounded-2xl border border-zinc-900 bg-neutral-900/40 p-4 text-xs text-zinc-300 flex flex-col sm:flex-row justify-between gap-3 shadow-xl">
      ...
      <section className="space-y-4 pt-4">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-zinc-900 bg-neutral-900/40 p-12 text-center text-xs text-zinc-500 italic shadow-xl">
            Aucun actif surveillé.
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {lastFavorites.map((card) => (
              <CardResult
                key={card.id}
                card={card}
              />
      ...
      <section>
        <div className="rounded-2xl border border-zinc-900 bg-neutral-900/40 p-4 text-[11px] text-zinc-400 shadow-xl">
          ...
          {monitoredFavoritesCount} actifs sous surveillance bénéficient
          actuellement d'une estimation marché.
        </div>
      </section>
    </div>
  </main>
