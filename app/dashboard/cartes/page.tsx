        ...
        <div className="kt-page max-w-4xl space-y-5">
          <section className="overflow-hidden rounded-[24px] border border-sky-300/20 bg-[linear-gradient(145deg,rgba(14,165,233,.10),rgba(20,27,36,.96)_45%,rgba(17,24,33,.98))] p-5 shadow-[0_24px_70px_rgba(2,132,199,.10)] sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-sky-300/20 bg-sky-400/[0.10] px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-sky-200"><WalletCards className="h-3.5 w-3.5" /> Détail du portefeuille</div>
                <h1 className="text-xl font-black tracking-tight text-white">Retrouvez toutes les cartes prises en compte dans votre portefeuille.</h1>
                <p className="mt-1 max-w-2xl text-[11px] leading-5 text-zinc-100">Consultez les prix d’achat, valeurs actuelles, états et plus-values sans quitter l’espace Dashboard.</p>
              </div>
            ...
            <div className="mt-5 grid grid-cols-3 gap-2.5 border-t border-white/[0.08] pt-4">
              <div className="rounded-2xl border border-sky-300/15 bg-sky-400/[0.07] p-3"><p className="text-[10px] font-bold uppercase tracking-[0.11em] text-sky-300/70">Valeur</p><p className="mt-1 text-sm font-black text-sky-100">{euro(totals.value)} €</p></div>
              <div className="kt-premium-card-soft p-3"><p className="text-[10px] font-bold uppercase tracking-[0.11em] text-zinc-200">Investi</p><p className="mt-1 text-sm font-black text-zinc-200">{euro(totals.buy)} €</p></div>
              <div className="kt-premium-card-soft p-3"><p className="text-[10px] font-bold uppercase tracking-[0.11em] text-zinc-200">Plus-value</p><p className={`mt-1 text-sm font-black ${totals.value - totals.buy >= 0 ? "text-emerald-200" : "text-rose-300"}`}>{totals.value - totals.buy >= 0 ? "+" : ""}{euro(totals.value - totals.buy)} €</p></div>
            </div>
          </section>
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 grid grid-cols-3 gap-2">
                          <div><p className="text-[10px] font-bold uppercase tracking-[0.11em] text-zinc-600">Achat</p><p className="text-[10px] font-black text-zinc-300">{euro(totalBuy)} €</p></div>
                          <div><p className="text-[10px] font-bold uppercase tracking-[0.11em] text-zinc-600">Actuelle</p><p className="text-[10px] font-black text-sky-100">{euro(totalValue)} €</p></div>
                          <div className="text-right"><p className="text-[10px] font-bold uppercase tracking-[0.11em] text-zinc-600">Plus-value</p><p className={`text-[10px] font-black ${profit >= 0 ? "text-emerald-200" : "text-rose-300"}`}>{profit >= 0 ? "+" : ""}{euro(profit)} €</p></div>
                        </div>
                      </div>
                    </div>
                    <div className="mt-1 flex items-center justify-between rounded-xl border border-white/[0.07] bg-[#1a222c] px-3 py-2">
          ...
          {!loading && visibleCards.length === 0 ? <div className="kt-premium-card p-10 text-center text-[11px] text-zinc-200"><SlidersHorizontal className="mx-auto mb-3 h-6 w-6" />Aucune carte ne correspond aux filtres.</div> : null}
        </div>
      </main>
    </>
  );
