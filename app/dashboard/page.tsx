  ...
  const loadDashboardData = async () => {
    try {
      ...
      valid.forEach((card) => {
        ...
        const adjustedPrice = getAdjustedPriceByCondition(
          market.average,
          card.condition
        );

        invested += getBuyPrice(card.id) * card.qty;
        currentPortfolio += adjustedPrice * card.qty;
      });
                        ...
                        <div className="grid grid-cols-3 gap-2 text-[10px]">
                          ...
                          <div className="bg-black/60 rounded-lg p-2.5 border border-zinc-800/60 text-center">
                            ...
                            <span className="text-white font-bold mt-1 block tabular-nums">
                              {(currentPrice * card.qty).toFixed(2)} €
                            </span>
                          </div>
