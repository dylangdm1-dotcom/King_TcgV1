    ...
    <div className="kt-chart-shell overflow-hidden rounded-[18px] border">
      ...
      <div className="relative h-52 px-2 pt-3 sm:h-60 sm:px-4">
        ...
        <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" className="h-full w-full" role="img" aria-label="Projection de la valeur sur sept jours">
          ...
          {points.map((point, index) => (
            <text key={`label-${index}`} x={point.x} y={height - 10} textAnchor="middle" fill="#8290a3" className="text-[10px] font-bold">{labels[index]}</text>
          ))}

          <text x={paddingX + 4} y={paddingTop - 8} fill="#8290a3" className="text-[9px] font-bold">
            {formatEuro(maxValue)} €
          </text>
          <text x={paddingX + 4} y={baseline - 7} fill="#8290a3" className="text-[9px] font-bold">
            {formatEuro(minValue)} €
          </text>
        </svg>
      </div>
    ...
    <>
      ...
      <main className="kt-premium-shell min-h-screen pb-32 text-white selection:bg-cyan-500/20">
        <div className="mx-auto max-w-6xl space-y-5 px-4 py-6 sm:px-6">
          ...
          {/* MAIN HEADER */}
          <section className="kt-rise-in kt-hero-surface relative overflow-hidden rounded-[22px] border p-5 sm:p-6">
