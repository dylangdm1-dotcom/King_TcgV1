...
function getQuantity(entry: unknown): number {
...
function getPurchasePrice(entry: unknown): number {
  if (
    entry &&
    typeof entry === "object" &&
    "purchasePrice" in entry
  ) {
    const value = Number(
      (entry as { purchasePrice?: unknown }).purchasePrice
    );
...
function getCondition(entry: unknown): string {
  if (
    entry &&
    typeof entry === "object" &&
    "condition" in entry
  ) {
    const value = (entry as { condition?: unknown }).condition;
      ...
      const loadedCards = await Promise.all(
        ids.map(async (id): Promise<DashboardCard | null> => {
          try {
            ...
            /*
             * IMPORTANT :
             * Le Dashboard utilise exactement le même moteur
            ...
            console.error(
              `[King_TCG] Impossible de charger la carte ${id}:`,
              error
            );
  ...
  const importData = (file: File) => {
    ...
    reader.onload = () => {
      try {
        ...
        /*
         * L'import reste volontairement séparé
         * de la collection réelle.
         */
        if (
          Array.isArray(
            (parsed as { cards?: unknown })
              .cards
          )
        ) {
