...
function SevenDayChart({ values }: { values: number[] }) {
  ...
  return (
    <div className="overflow-hidden rounded-[18px] border border-white/[0.1] bg-[#151c25] shadow-[0_18px_45px_rgba(0,0,0,.24)]">
      <div className="grid grid-cols-3 border-b border-white/[0.07] bg-[#1a222d]">
        <div className="p-3.5">
          <p className="text-[10px] font-black uppercase tracking-[0.14em] text-zinc-500">Valeur actuelle</p>
        ...
        <div className="border-x border-white/[0.07] p-3.5 text-center">
          <p className="text-[10px] font-black uppercase tracking-[0.14em] text-zinc-500">Variation</p>
        ...
        <div className="p-3.5 text-right">
          <p className="text-[10px] font-black uppercase tracking-[0.14em] text-zinc-500">Projection J+6</p>
      ...
      <div className="relative h-52 px-2 pt-3 sm:h-60 sm:px-4">
        ...
        <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" className="h-full w-full" role="img" aria-label="Projection de la valeur sur sept jours">
          ...
          {points.map((point, index) => (
            <text key={`label-${index}`} x={point.x} y={height - 10} textAnchor="middle" className="fill-zinc-600 text-[10px] font-bold">{labels[index]}</text>
          ))}
        </svg>
      </div>
    </div>
      ...
      <main className="kt-app-shell pb-32 selection:bg-cyan-500/20">
        <div className="kt-page max-w-6xl space-y-5">
          ...
          <section className="grid grid-cols-2 gap-2 lg:grid-cols-4">
            <div className="kt-premium-card kt-interactive-card flex min-h-[72px] flex-col justify-between rounded-[14px] p-3">
              <div className="flex justify-between items-start">
                <span className="text-zinc-500 text-[10px] font-bold uppercase tracking-[0.11em]">
                  Valeur Actuelle
                </span>
          ...
          <div className="space-y-4 pt-2">
            <h2 className="text-[10px] font-bold uppercase tracking-[0.11em] text-zinc-500 px-1 flex items-center gap-1.5">
            ...
            <section className="kt-premium-card overflow-hidden rounded-[18px] p-4 sm:p-5 lg:p-6">
              <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.11em] text-zinc-500">
              ...
              <p className="mt-4 text-[10px] text-zinc-700 text-center uppercase tracking-wider">
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-[10px] text-zinc-500">Tendances du marché et répartition de votre collection.</p>
                </div>
              </div>
                    ...
                    <div className="space-y-2">
                      {pokemonMarketTrends.length ? pokemonMarketTrends.map((item) => (
                        <div key={item.name} className="rounded-xl border border-white/[0.06] bg-black/20 p-2.5">
                          ...
                          <p className="mt-1 text-[10px] text-zinc-500">{item.rising}/{item.total} cartes en hausse · {Math.abs(item.average) >= 3 ? "tendance forte à surveiller" : "mouvement collectif détecté"}</p>
                        </div>
                    ...
                    <div className="space-y-2">
                      {extensionMarketTrends.length ? extensionMarketTrends.map((item) => (
                        <div key={item.name} className="rounded-xl border border-white/[0.06] bg-black/20 p-2.5">
                          ...
                          <p className="mt-1 text-[10px] text-zinc-500">{Math.round((item.rising / item.total) * 100)} % des cartes en hausse · {item.total} analysées</p>
                        </div>
