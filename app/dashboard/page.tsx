...
function SevenDayChart({ values }: { values: number[] }) {
  ...
  const points = values.map((value, index) => ({
    ...
    y: paddingTop + (1 - (value - minValue) / range) * (height - paddingTop - paddingBottom),
  }));
    ...
    <div className="overflow-hidden rounded-[20px] border border-white/[0.1] bg-[#151c25] shadow-[0_18px_45px_rgba(0,0,0,.24)]">
      <div className="grid grid-cols-3 border-b border-white/[0.07] bg-[#1a222d]">
        ...
        <div className="border-x border-white/[0.07] p-3.5 text-center">
          ...
          <p className={`mt-1 text-sm font-black tabular-nums ${positive ? "text-emerald-200" : "text-rose-300"}`}>
      ...
      <div className="relative h-52 px-2 pt-3 sm:h-60 sm:px-4">
        <div className={`pointer-events-none absolute inset-x-0 top-0 h-24 ${positive ? "bg-emerald-300/[0.035]" : "bg-rose-300/[0.035]"}`} />
        <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" className="h-full w-full" role="img" aria-label="Projection de la valeur sur sept jours">
          <defs>
            <linearGradient id="dashboardArea" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={positive ? "rgb(167 243 208)" : "rgb(253 164 175)"} stopOpacity="0.28" />
              <stop offset="75%" stopColor={positive ? "rgb(52 211 153)" : "rgb(244 63 94)"} stopOpacity="0.035" />
          ...
          <path d={linePath} fill="none" stroke={positive ? "rgb(167 243 208)" : "rgb(253 164 175)"} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" filter="url(#dashboardGlow)" />

          {points.map((point, index) => index === points.length - 1 || index === 0 ? (
            <g key={`p-${index}`}>
              <circle cx={point.x} cy={point.y} r="7" fill={positive ? "rgba(167,243,208,.15)" : "rgba(253,164,175,.15)"} />
              <circle cx={point.x} cy={point.y} r="3.2" fill="#111827" stroke={positive ? "rgb(167 243 208)" : "rgb(253 164 175)"} strokeWidth="2" />
            </g>
          ) : null)}
    ...
    <>
      ...
      <main className="kt-app-shell pb-32 selection:bg-cyan-500/20">
        <div className="kt-page max-w-xl space-y-5">
                    ...
                    (
                      ...
                      return (
                        <div
                          ...
                          className="flex items-center justify-between rounded-lg border border-white/[0.08]/60 bg-neutral-950/50 p-3"
                        >
          ...
          <section className="kt-premium-card rounded-[20px] p-3.5 sm:p-4">
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-[10px] leading-4 text-zinc-500">Répartition de la valeur estimée selon l’état enregistré pour chaque carte.</p>
              </div>
