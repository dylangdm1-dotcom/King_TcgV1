...
//
// Correction V5 :
// - Near Mint utilisé par défaut
// - prix marché réel le moins cher disponible
// - aucun fallback arbitraire
// - calcul des statistiques basé sur le même prix affiché
// - chargement sécurisé
...
* Retourne le prix marché réellement exploitable.
*
* Priorité :
* 1. prix Near Mint / marché réel fourni par priceTracker
* 2. fallback sur market si disponible
* 3. 0 si aucun prix réel disponible
*
* IMPORTANT :
* On ne crée aucun prix fictif.
  */
  function getRealMarketPrice(card: PokemonCard): number {
  try {
  ...
  // Le moteur V5 doit privilégier le prix réellement disponible
  // le plus bas plutôt qu'une moyenne artificielle.
  return Math.min(...candidates);
  } catch (error) {
  console.error(
  `[King_TCG V5.0] Erreur calcul prix marché ${card.id} :`,
  error
  );
...
// ======================================================
// Cache prix marché V5
//
// Un seul calcul par carte.
// Le prix utilisé est celui réellement disponible
// et non une moyenne artificielle.
// ======================================================
...
cards.forEach((card) => {
  cache[card.id] = getRealMarketPrice(card);
});
...
// ======================================================
// Valeur totale de la Watchlist
//
// IMPORTANT :
// basée sur les cartes réellement affichées après
// recherche + filtre.
// ======================================================
...
// ======================================================
// Nombre de cartes réellement indexées
// ======================================================

const activeMonitoredCount = useMemo(() => {
return cards.filter(
(card) => (priceCache[card.id] ?? 0) > 0
).length;
}, [cards, priceCache]);
...
// Valeur moyenne
//
// Calculée uniquement sur les cartes ayant un prix réel.
// ======================================================
...
const total = pricedCards.reduce(
  (sum, card) =>
    sum + (priceCache[card.id] ?? 0),
  0
);

return total / pricedCards.length;
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          ...
          {processedCards.map((card) => {
            const marketPrice =
              priceCache[card.id] ?? 0;

            /**
             * CardResult reçoit le prix marché calculé
             * afin que l'affichage soit cohérent avec
             * les statistiques de la Watchlist.
             */
            const cardWithMarketPrice = {
              ...card,
              computedPrice: marketPrice,
            };

            return (
              <CardResult
                key={card.id}
                card={cardWithMarketPrice}
              />
            );
          })}
