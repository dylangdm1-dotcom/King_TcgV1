...
import { getSetDisplayCatalog, getSetDisplayMeta, type SetDisplayMeta } from "../../lib/setDisplayCatalog";
...
type SetItem = {
  ...
  displayMeta?: SetDisplayMeta;
};


function localizedSetName(set: SetItem, lang: LanguageCode): string {
  return getSetDisplayMeta(lang, set.id, set.name)?.name || set.name;
}

function displaySetCode(set: SetItem, lang: LanguageCode): string {
  return getSetDisplayMeta(lang, set.id, set.name)?.code || localizedSetCode(set.id, lang);
}

function displaySetYear(set: SetItem, lang: LanguageCode): string {
  const curatedYear = getSetDisplayMeta(lang, set.id, set.name)?.year;
  return curatedYear ? String(curatedYear) : yearLabel(effectiveSetReleaseDate(set.id, set.releaseDate));
}

function displaySetEra(set: SetItem, lang: LanguageCode, fallback: string): string {
...
function getGeneration(set: SetItem, lang: LanguageCode = "fr"): string {
  ...
  if (era) {
    ...
    if (era === "Scarlet & Violet" || era === "Écarlate et Violet") return "Écarlate & Violet";
    if (era === "Sword & Shield" || era === "Épée et Bouclier") return "Épée & Bouclier";
    if (era === "Sun & Moon" || era === "Soleil et Lune") return "Soleil & Lune";
    if (era === "Black & White") return "Noir & Blanc";
    if (era === "Diamond & Pearl") return "Diamant & Perle";
    if (era === "Original" || era === "Neo" || era === "e-Series" || era === "ADV" || era === "PCG" || era === "Platinum") return "Séries classiques";
    if (era === "Simplified Chinese") return "Chinois simplifié";
    return era;
  }
...
function buildCuratedVisibleSets(sets: SetItem[], lang: LanguageCode): SetItem[] {
  ...
  if (!catalog.length) return [...sets].sort(compareSetsNewestFirst);

  const codeCounts = new Map<string, number>();
  catalog.forEach((meta) => {
    const key = normalizeSetId(meta.code);
    codeCounts.set(key, (codeCounts.get(key) || 0) + 1);
  });

  const providerKey = (set: SetItem) =>
    String(set.providerSetId || `${set.id}:${normalizeCatalogText(set.name)}`);
  const usedProviders = new Set<string>();
  const result: SetItem[] = [];

  for (const meta of catalog) {
    const sourceIds = new Set((meta.sourceIds || []).map((id) => normalizeSetId(id)));
    const names = new Set([meta.name, ...(meta.aliases || [])].map(normalizeCatalogText));
    const code = normalizeSetId(meta.code);
    const codeIsUnique = (codeCounts.get(code) || 0) === 1;

    // Name matching has priority. This is essential for CN products sharing
    // the same public code (for example several Collect 151 releases).
    let matches = sets.filter((set) => {
      if (usedProviders.has(providerKey(set))) return false;
      return names.has(normalizeCatalogText(set.name));
    });

    if (!matches.length && sourceIds.size) {
      matches = sets.filter((set) => {
        if (usedProviders.has(providerKey(set))) return false;
        return sourceIds.has(normalizeSetId(set.id));
      });
    }

    if (!matches.length && codeIsUnique) {
      matches = sets.filter((set) => {
        if (usedProviders.has(providerKey(set))) return false;
        return normalizeSetId(set.id) === code;
      });
    }

    const providerIds = matches.map((set) => String(set.providerSetId || set.id));
    const base = matches[0];
    if (base) {
      matches.forEach((set) => usedProviders.add(providerKey(set)));
      result.push({
        ...base,
        id: base.id,
        name: meta.name,
        series: meta.era,
        releaseDate: base.releaseDate || (meta.year ? `${meta.year}-01-01` : undefined),
        providerIds,
        displayMeta: meta,
      });
      continue;
    ...
    const fallbackId = meta.sourceIds?.[0] || meta.code;
    result.push({
      id: fallbackId,
      name: meta.name,
      series: meta.era,
      ...
      images: {},
      providerIds: meta.sourceIds?.length ? [...meta.sourceIds] : [fallbackId],
      displayMeta: meta,
    });
  }
  ...
  if (lang === "fr") {
    const extras = sets
      .filter((set) => !usedProviders.has(providerKey(set)))
      .filter((set) => !getSetDisplayMeta("fr", set.id, set.name))
...
export default function Recherche() {
  ...
  useEffect(() => {
    ...
    async function loadSets() {
      ...
      try {
        ...
        setAllSetsList(buildCuratedVisibleSets(validSets, selectedLanguage));
    ...
    const entries = availableGenerations
      ...
      .map((generation) => {
        const sets = groupedSets[generation].filter((set) => {
          ...
          return `${localizedSetName(set, selectedLanguage)} ${set.name} ${displaySetCode(set, selectedLanguage)} ${set.id} ${set.series || ""} ${set.releaseDate || ""}`.toLowerCase().includes(normalized);
        });
  ...
  async function handleSetSelect(setId: string) {
    ...
    try {
      const providerIds = chosenSet.providerIds?.length ? chosenSet.providerIds : [chosenSet.id];
      const batches = await Promise.all(
        providerIds.map((providerId) => searchCardsBySetId(providerId, selectedLanguage, chosenSet.name))
      );
      const byCardId = new Map<string, PokemonCard>();
      batches.flat().forEach((card) => {
        const key = String(card.id || `${card.set?.id || ""}:${card.number || ""}:${card.name || ""}`);
        if (!byCardId.has(key)) byCardId.set(key, card);
      });
      setCards(Array.from(byCardId.values()));
      setVisible(PAGE_SIZE);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (selected) {
      setSelectedGeneration(getGeneration(selected, selectedLanguage));
      setExpandedGeneration(getGeneration(selected, selectedLanguage));
    }
                              ...
                              >
                                ...
                                <span className="min-w-0 overflow-hidden">
                                  ...
                                  <span className="mt-1 flex min-w-0 items-center gap-1.5 overflow-hidden text-[7px] font-semibold uppercase tracking-wide text-zinc-500">
                                    <span className="inline-flex shrink-0 items-center gap-1"><CalendarDays className="h-2.5 w-2.5 text-amber-300" /> {displaySetYear(set, selectedLanguage)}</span>
                                    <span className="truncate">{displaySetEra(set, selectedLanguage, generation)}</span>
                                  </span>
                                </span>
                                ...
                                <span className="shrink-0 pl-1 text-right">
                                  ...
                                  <span className="mt-0.5 block max-w-[54px] truncate text-[6px] font-black uppercase text-violet-300">{displaySetCode(set, selectedLanguage)}</span>
                                </span>
                              </button>
                            ))}
                          </div>
