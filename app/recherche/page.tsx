...
function buildCuratedVisibleSets(sets: SetItem[], lang: LanguageCode): SetItem[] {
  ...
  // FR keeps the curated visual layer used by the validated catalogue.
  if (lang === "fr") {
    const catalogOrder = new Map(catalog.map((meta, index) => [meta, index]));
    return [...sets]
      .map((set) => ({ ...set, displayMeta: getSetDisplayMeta(lang, set.id, set.name) }))
      .sort((a, b) => {
        const ai = a.displayMeta ? (catalogOrder.get(a.displayMeta) ?? Number.MAX_SAFE_INTEGER) : Number.MAX_SAFE_INTEGER;
        const bi = b.displayMeta ? (catalogOrder.get(b.displayMeta) ?? Number.MAX_SAFE_INTEGER) : Number.MAX_SAFE_INTEGER;
