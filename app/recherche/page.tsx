...
function SearchResultCard({ card, compact = false }: { card: PokemonCard; compact?: boolean }) {
  ...
  const [cachedMarket, setCachedMarket] = useState<ReturnType<typeof getCachedMarketCardV275>>(null);

  useEffect(() => {
    const syncQuickActions = () => {
      setQuantity(getCardQuantity(card.id));
      setFavorite(isFavorite(card.id));
    };

    syncQuickActions();
    window.addEventListener("king_tcg_update", syncQuickActions);
    return () => window.removeEventListener("king_tcg_update", syncQuickActions);
  }, [card.id]);

  useEffect(() => {
    const syncCachedMarket = () => setCachedMarket(getCachedMarketCardV275(card));
    syncCachedMarket();
    window.addEventListener("king_tcg_market_price_update", syncCachedMarket);
    return () => window.removeEventListener("king_tcg_market_price_update", syncCachedMarket);
  }, [card]);

  const handleQuickCollection = () => {
    const updated = addToCollection(card.id);
    const entry = updated[card.id] as any;
    setQuantity(typeof entry === "number" ? entry : (entry?.quantity || 0));
  };

  const handleQuickFavorite = () => {
    const updated = toggleFavorite(card.id);
    setFavorite(updated.includes(card.id));
  };
  ...
  const typeLabel =
    ...
    card.supertype ||
    "Carte Pokémon";
  const cachedPrice = cachedMarket ? getMarketData(cachedMarket.card).average : 0;

  if (compact) {
    return (
      <article className="kt-card-frame group relative flex h-full min-w-0 flex-col overflow-hidden rounded-[14px] bg-[#0a1118] p-1.5">
        <Link href={`/card/${card.id}`} className="relative block">
          <div className="relative aspect-[0.72] overflow-hidden rounded-[10px] bg-[#0c151e]">
            {card.dataLanguage ? (
              <span className="absolute left-1 top-1 z-20 rounded-full border border-cyan-400/25 bg-[#061016]/90 px-1.5 py-0.5 text-[7px] font-black uppercase text-cyan-200">
            ...
            {!imageFailed && imageSrc ? (
              ...
              <div className="flex h-full items-center justify-center text-zinc-500"><ImageOff className="h-5 w-5" /></div>
            )}
          </div>
        </Link>
        ...
        <div className="flex min-w-0 flex-1 flex-col px-0.5 pb-0.5 pt-1.5">
          <Link href={`/card/${card.id}`} className="min-w-0">
            <h2 className="line-clamp-1 text-[9px] font-black leading-tight text-white">{card.name}</h2>
            <p className="mt-0.5 truncate text-[7px] font-semibold text-zinc-400">{setCode || card.set?.name} · #{card.number}</p>
          </Link>
          <p className="mt-1 truncate text-[8px] font-black text-cyan-200">
            {cachedPrice > 0 ? `${cachedPrice.toFixed(2)} €` : "Cote —"}
          </p>
          <div className="mt-auto grid grid-cols-2 gap-1 pt-1.5">
            <button
              type="button"
              onClick={handleQuickCollection}
              title={quantity > 0 ? `${quantity} en collection` : "Ajouter à la collection"}
              aria-label={`Ajouter ${card.name} à la collection`}
              className={`flex h-6 items-center justify-center rounded-md border ${quantity > 0 ? "border-cyan-300/35 bg-cyan-400/[0.10] text-cyan-100" : "border-cyan-400/18 text-cyan-300"}`}
            >
              {quantity > 0 ? <Check className="h-3 w-3" /> : <Plus className="h-3 w-3" />}
            </button>
            <button
              type="button"
              onClick={handleQuickFavorite}
              title={favorite ? "Retirer des favoris" : "Ajouter aux favoris"}
              aria-label={`${favorite ? "Retirer" : "Ajouter"} ${card.name} ${favorite ? "des" : "aux"} favoris`}
              className={`flex h-6 items-center justify-center rounded-md border ${favorite ? "border-rose-400/35 bg-rose-400/[0.10] text-rose-200" : "border-rose-400/18 text-rose-300"}`}
            >
    ...
    <article className="kt-card-frame group relative flex h-full flex-col overflow-hidden rounded-[20px] bg-[#0a1118] p-2.5 transition duration-200 hover:-translate-y-1 hover:shadow-[0_20px_48px_rgba(0,0,0,.30),0_0_24px_rgba(34,211,238,.055)]">
      ...
      <div className="relative flex flex-1 flex-col px-1 pb-1 pt-3">
        ...
        <div className="mt-2.5 flex items-center justify-between gap-2 border-t border-white/[0.06] pt-2.5">
          <span className="min-w-0 text-[10px] font-semibold text-zinc-500">
            {cachedPrice > 0 ? (
              <span className="flex flex-col">
                <span className="font-black text-cyan-200">Cote {cachedPrice.toFixed(2)} €</span>
                <span className={cachedMarket?.state === "fresh" ? "text-emerald-300" : "text-amber-300"}>
                  {cachedMarket?.state === "fresh" ? "Actualisée" : "À actualiser"}
                </span>
              </span>
            ) : "Cote disponible dans la fiche"}
          </span>
          <Link
            href={`/card/${card.id}`}
        ...
        <div className="mt-1.5 grid grid-cols-2 gap-1.5">
          <button
            ...
            onClick={handleQuickCollection}
            aria-label={`Ajouter ${card.name} à la collection`}
            className={`inline-flex min-w-0 items-center justify-center gap-1 rounded-[9px] border px-1.5 py-1.5 text-[9px] font-black transition active:scale-95 ${
              quantity > 0
                ? "border-cyan-300/35 bg-cyan-400/[0.10] text-cyan-100"
                : "border-cyan-400/18 bg-cyan-400/[0.04] text-cyan-200 hover:border-cyan-300/35"
            }`}
          >
            {quantity > 0 ? <Check className="h-3 w-3 shrink-0" /> : <Plus className="h-3 w-3 shrink-0" />}
            <span className="truncate">{quantity > 0 ? `Collection · ${quantity}` : "Collection"}</span>
          </button>
          <button
            type="button"
            onClick={handleQuickFavorite}
            ...
            className={`inline-flex min-w-0 items-center justify-center gap-1 rounded-[9px] border px-1.5 py-1.5 text-[9px] font-black transition active:scale-95 ${
              favorite
                ? "border-rose-400/35 bg-rose-400/[0.10] text-rose-200"
                : "border-rose-400/18 bg-rose-400/[0.04] text-rose-300 hover:border-rose-300/35"
            }`}
          >
            <Heart className={`h-3 w-3 shrink-0 ${favorite ? "fill-current" : ""}`} />
            <span className="truncate">{favorite ? "Favori" : "Favoris"}</span>
          </button>
        </div>
      </div>
    </article>
...
export default function Recherche() {
  ...
  const metadataSetCount = useMemo(
    () => allSetsList.filter((set) => set.availability === "metadata_only").length,
    [allSetsList]
  );
        ...
        <div className="kt-page-wrap space-y-5">
            ...
            <div className="flex flex-col justify-between gap-4 border-b border-white/[0.06] pb-5 md:flex-row md:items-center">
              <div className="min-w-0 flex-1">
                ...
                <p className="kt-search-intro-note mt-2">
                  Recherchez par nom ou parcourez le catalogue local par série et extension. Les cartes FR/EN disponibles sont chargées localement ; JP/CN indiquent clairement ce qui reste à synchroniser.
                </p>
              </div>
          ...
          <section className="kt-section-surface flex items-center justify-between gap-4 rounded-[16px] border p-3">
            ...
            <div className="flex gap-1 rounded-xl border border-cyan-400/14 bg-[#0c141c] p-1">
              <button title="3 cartes par ligne" aria-label="Afficher 3 cartes par ligne" onClick={() => selectViewMode("compact3")} className={`rounded-lg border px-3 py-1.5 text-[10px] font-black uppercase transition ${viewMode === "compact3" ? "border-cyan-300/40 bg-cyan-400/[0.10] text-cyan-200" : "border-transparent text-zinc-300"}`}>
                <Columns3 className="h-3.5 w-3.5" />
              </button>
              <button title="Grille standard" aria-label="Afficher la grille standard" onClick={() => selectViewMode("grid")} className={`rounded-lg border px-3 py-1.5 text-[10px] font-black uppercase transition ${viewMode === "grid" ? "border-cyan-300/40 bg-cyan-400/[0.10] text-cyan-200" : "border-transparent text-zinc-300"}`}>
                <LayoutGrid className="h-3.5 w-3.5" />
              </button>
              <button title="Vue large" aria-label="Afficher les cartes en grand" onClick={() => selectViewMode("large")} className={`rounded-lg border px-3 py-1.5 text-[10px] font-black uppercase transition ${viewMode === "large" ? "border-cyan-300/40 bg-cyan-400/[0.10] text-cyan-200" : "border-transparent text-zinc-300"}`}>
          ...
          <AnimatePresence mode="wait">
            {loading ? (
              <motion.div
                ...
                exit={{ opacity: 0, y: -6 }}
                className={viewMode === "compact3" ? "grid grid-cols-3 gap-2" : "grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6"}
              >
          ...
          {!loading ? (
            <motion.div
              layout
              className={viewMode === "compact3" ? "grid grid-cols-3 gap-2 sm:gap-3" : viewMode === "grid" ? "grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6" : "flex flex-col items-center gap-4"}
            >
