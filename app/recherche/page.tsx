  ...
  async function handleSetSelect(setId: string) {
    ...
    try {
      const results = await searchCardsBySetId(
        chosenSet.sourceId || chosenSet.id,
        selectedLanguage
      );
      setCards(results);
      setVisible(PAGE_SIZE);

      // Même pipeline prix que la recherche par nom, lancé explicitement sur
      // la première page visible. L'affichage de l'extension reste immédiat.
      const firstVisibleBatch = results.slice(0, PAGE_SIZE);
      if (firstVisibleBatch.length) {
        const generation = priceSyncGenerationRef.current;
        firstVisibleBatch.forEach((card) => {
          attemptedPriceIdsRef.current.add(card.id);
          priceAttemptCountRef.current.set(card.id, 1);
        });
        void enrichAndCacheCards(firstVisibleBatch)
          .then((enrichedCards) => {
          ...
          .catch((error) => console.error("[King_TCG] Prix extension :", error));
      }
    } catch (error) {
