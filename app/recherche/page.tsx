    ...
    <>
      ...
      <main className="kt-app-shell selection:bg-cyan-500/20">
        <div className="kt-page space-y-6">
          <section className="kt-rise-in kt-hero-surface relative space-y-5 overflow-hidden rounded-[22px] border p-4 sm:p-6">
              ...
              <div className="flex self-start items-center gap-1.5 rounded-2xl border border-cyan-400/15 bg-[#0d151e] p-1.5 shadow-[0_0_22px_rgba(34,211,238,.035)] md:self-auto">
                ...
                {[
                  ...
                  <button
                    ...
                    className={`flex h-8 w-9 items-center justify-center rounded-lg border text-base transition ${selectedLanguage === lang.code ? "border-cyan-300/45 bg-cyan-400/[0.12] text-white shadow-[0_0_16px_rgba(34,211,238,.08)]" : "border-transparent text-zinc-300 hover:border-white/[0.06] hover:bg-white/[0.04] hover:text-white"}`}
                  >
                    {lang.label}
                  </button>
                ))}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {visibleSetGroups.map(([generation, generationSets]) => {
                    ...
                    return (
                      <section key={generation} className="kt-section-surface min-w-0 max-w-full overflow-hidden rounded-[18px] border">
                        <button
                          type="button"
                          ...
                          <div className="grid min-w-0 gap-1.5 border-t border-white/[0.06] p-2 sm:grid-cols-2 xl:grid-cols-3">
                            {generationSets.map((set) => (
                              <button
                                ...
                                className={`group grid min-h-[56px] min-w-0 max-w-full grid-cols-[38px_minmax(0,1fr)_auto] items-center gap-2.5 overflow-hidden rounded-xl border px-2.5 py-2 text-left transition ${selectedSetId === set.id ? "border-violet-300/40 bg-violet-300/[0.08]" : "kt-data-row hover:bg-[#1d2530]"}`}
                              >
                                ...
                                <span className="shrink-0 pl-1 text-right">
                                  ...
                                  <span className="block text-[10px] font-bold uppercase tracking-wide text-zinc-600">cartes</span>
          ...
          {!loading && query.length >= 2 && filteredCards.length === 0 ? (
            <div className="kt-empty-state rounded-2xl p-10 text-center">
              ...
              <p className="mx-auto mt-1 max-w-sm text-[11px] text-zinc-200">Vérifiez l’orthographe, la langue ou sélectionnez directement une extension.</p>
            </div>
          ) : null}
