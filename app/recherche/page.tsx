      ...
      <main className="kt-app-shell selection:bg-cyan-500/20">
        <div className="kt-page space-y-6">
                ...
                {showUpcoming ? (
                  <div className="grid gap-2 border-t border-amber-300/10 p-2.5 sm:grid-cols-2">
                    {upcomingReleases.map((release) => (
                      <article key={release.id} className="rounded-xl border border-white/[0.06] bg-[#171d25] p-3">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {visibleSetGroups.map(([generation, generationSets]) => {
                    ...
                    return (
                      <section key={generation} className="min-w-0 max-w-full overflow-hidden rounded-[18px] border border-white/[0.06] bg-[#151b23]">
                        <button
                          type="button"
                          ...
                          <div className="grid min-w-0 gap-1.5 border-t border-white/[0.06] p-2 sm:grid-cols-2 xl:grid-cols-3">
                            {generationSets.map((set) => (
                              <button
                                ...
                                className={`group grid min-h-[56px] min-w-0 max-w-full grid-cols-[38px_minmax(0,1fr)_auto] items-center gap-2.5 overflow-hidden rounded-xl border px-2.5 py-2 text-left transition ${selectedSetId === set.id ? "border-violet-300/40 bg-violet-300/[0.08]" : "border-white/[0.055] bg-[#1a212b] hover:border-white/[0.14] hover:bg-[#1d2530]"}`}
                              >
          ...
          {!loading && query.length >= 2 && filteredCards.length === 0 ? (
            <div className="rounded-2xl border border-white/[0.06] bg-[#171d25] p-10 text-center">
