    ...
    const entries = availableGenerations
      ...
      .map((generation) => {
        const sets = groupedSets[generation].filter((set) => {
          ...
          return `${set.name} ${set.id} ${set.series || ""} ${set.releaseDate || ""}`.toLowerCase().includes(normalized);
        });
        ...
        <div className="kt-page space-y-6">
          <section className="kt-premium-card kt-rise-in space-y-5 p-4 sm:p-6">
            <div className="flex flex-col justify-between gap-4 border-b border-white/[0.06] pb-5 md:flex-row md:items-center">
              <div>
                <div className="kt-eyebrow mb-2"><Sparkles className="h-3 w-3" /> Base cartes & marché</div>
              ...
              <div className="flex self-start items-center gap-1 rounded-2xl border border-white/[0.07] bg-[#171d25] p-1.5 md:self-auto">
                <Globe className="mx-1 h-3.5 w-3.5 text-zinc-500" />
                {[
                  { code: "fr", label: "🇫🇷 FR" },
                  { code: "en", label: "🇺🇸 EN" },
                  { code: "ja", label: "🇯🇵 JP" },
                  ...
                  <button
                    ...
                    className={`rounded-xl px-2.5 py-2 text-[9px] font-black transition ${selectedLanguage === lang.code ? "bg-white text-[#0a1017] shadow" : "text-zinc-500 hover:text-white"}`}
                  >
                    {lang.label}
                  </button>
                ))}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {visibleSetGroups.map(([generation, generationSets]) => {
                    ...
                    const shownSets = expanded ? generationSets : generationSets.slice(0, 4);
                    ...
                    return (
                      <section key={generation} className="overflow-hidden rounded-[20px] border border-white/[0.08] bg-[#151b23]">
                        ...
                        <div className="grid grid-cols-2 gap-2 border-t border-white/[0.06] p-3 sm:grid-cols-3 lg:grid-cols-4">
                          {shownSets.map((set) => (
                            <button
                              type="button"
                              key={set.id}
                              onClick={() => handleSetSelect(set.id)}
                              className={`group flex min-h-[112px] flex-col justify-between rounded-2xl border p-3 text-left transition ${selectedSetId === set.id ? "border-violet-300/40 bg-violet-300/[0.08]" : "border-white/[0.07] bg-[#1a212b] hover:border-white/[0.14] hover:bg-[#1d2530]"}`}
                            >
                              <div className="flex items-start justify-between gap-2">
                                <span className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl border border-white/[0.08] bg-black/20">
                                ...
                                <span className="rounded-lg bg-black/20 px-2 py-1 text-[8px] font-black text-amber-300">{yearLabel(set.releaseDate)}</span>
                              </div>
          ...
          {loading ? (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
              {Array.from({ length: 12 }).map((_, index) => <div key={index} className="kt-skeleton aspect-[0.72] rounded-2xl border border-white/[0.08]" />)}
            </div>
          ) : null}
