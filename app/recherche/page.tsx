...
useState<"grid" | "large">("grid");

const [searchMode, setSearchMode] =
useState<"text" | "set">("text");

// Français sélectionné par défaut
const [selectedLanguage, setSelectedLanguage] =
useState<LanguageCode>("fr");

const [allSetsList, setAllSetsList] =
useState<any[]>([]);
...
setLoading(true);
  ...
  try {
    const setsData =
      await getAllSets(selectedLanguage);
    ...
    console.error(
      "[King_TCG] Erreur chargement des séries :",
      error
    );
  } finally {
...
try {
  ...
  console.error(
    "[King_TCG] Erreur recherche extension :",
    error
  );
...
// =====================================================
// EXTENSIONS PRESENTES DANS LES RESULTATS
// =====================================================
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-5 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-800/80 pb-4">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[11px] text-zinc-400">
              Index TCG intelligent,
              recherche multilingue et
              prix marché réels V5.
            </p>
          </div>
      ...
      <section className="flex items-center justify-between gap-4 border-b border-zinc-900 pb-3">
        ...
        <div className="rounded-xl border border-zinc-800/80 bg-neutral-900/40 px-3.5 py-2 text-[10px] font-black uppercase tracking-wider text-zinc-300 flex items-center gap-2">
          ...
          <span>
            {filteredCards.length} actif(s)
          </span>

          {query && (
            <span className="text-cyan-400 font-normal truncate max-w-[150px] sm:max-w-xs">
              pour "{query}"
            </span>
          )}
