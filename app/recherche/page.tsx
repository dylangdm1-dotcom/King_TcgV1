...
function buildCuratedVisibleSets(sets: SetItem[], lang: LanguageCode): SetItem[] {
  ...
  for (const meta of catalog) {
    ...
    const matches = sets.filter((set) => {
      ...
      if (normalizeSetId(meta.code) === id) return true;
      return names.has(normalizeCatalogText(set.name));
    });

    const providerIds = matches.map((set) => set.id);
    const base = matches[0];
    if (base) {
      matches.forEach((set) => usedProviderIds.add(normalizeSetId(set.id)));
      result.push({
        ...base,
        id: base.id,
        name: meta.name,
    ...
    // Keep the curated row visible even when an upstream /sets index is late.
    // Selection still uses the normal provider endpoint and therefore never invents cards.
  ...
  // FR has a long retro catalogue that was not supplied code-by-code in the
  // curated reference. Preserve those existing FR rows after the curated modern list.
  if (lang === "fr") {
    const curatedKeys = new Set(
      result.map((set) => {
        const meta = set.displayMeta || getSetDisplayMeta("fr", set.id, set.name);
        return meta ? `${normalizeCatalogText(meta.code)}:${normalizeCatalogText(meta.name)}` : "";
      }).filter(Boolean)
    );
    const extras = sets
      .filter((set) => !usedProviderIds.has(normalizeSetId(set.id)))
      .filter((set) => {
        const meta = getSetDisplayMeta("fr", set.id, set.name);
        if (!meta) return true;
        return !curatedKeys.has(`${normalizeCatalogText(meta.code)}:${normalizeCatalogText(meta.name)}`);
      })
  ...
  async function handleSetSelect(setId: string) {
    ...
    try {
      ...
      const batches = await Promise.all(
        providerIds.map((providerId) => searchCardsBySetId(providerId, selectedLanguage))
      );
