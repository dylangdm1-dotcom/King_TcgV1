...
export default function ScannerPage() {
  ...
  const [needsRetry, setNeedsRetry] =
    useState(false);
  ...
  function buildScanResult(
    ...
    const cardName =
      typeof source.cardName === "string" &&
      source.cardName.trim()
        ? source.cardName.trim()
        : null;

    const pokemonName =
      typeof source.pokemonName === "string" &&
      source.pokemonName.trim()
        ? source.pokemonName.trim()
        : null;

    const possibleNames =
      Array.isArray(
        source.possibleNames
      )
        ? source.possibleNames.filter(
            (
              name: unknown
            ): name is string =>
              typeof name === "string" &&
    ...
    const cardType =
      source.cardType === "Pokemon" ||
      source.cardType === "Trainer" ||
      source.cardType === "Energy" ||
      source.cardType === "Unknown"
        ? source.cardType
        : null;

    const variant =
      source.variant === "Normal" ||
      source.variant === "Full Art" ||
      source.variant === "Alt Art" ||
      source.variant === "Rainbow" ||
      source.variant === "Gold" ||
      source.variant === "Shiny" ||
    ...
    return {
      ...
      setName:
        typeof source.setName === "string"
          ? source.setName
          : null,

      setSymbol:
        typeof source.setSymbol === "string"
          ? source.setSymbol
          : null,
      ...
      isFullArt:
        typeof source.isFullArt === "boolean"
          ? source.isFullArt
          : false,

      isSecretRare:
        typeof source.isSecretRare === "boolean"
          ? source.isSecretRare
          : false,
      ...
      needsSecondPass:
        typeof source.needsSecondPass ===
        "boolean"
          ? source.needsSecondPass
          : confidence < 0.6,
    };
  }
