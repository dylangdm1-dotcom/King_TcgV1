...
export default function CardActions({ cardId }: Props) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <button
        onClick={addCard}
        className="rounded-xl border border-zinc-800 bg-[#171e28]/85 hover:bg-neutral-900 px-4 py-2 text-xs font-bold text-white transition-all duration-200 flex items-center gap-2 active:scale-95 cursor-pointer"
      >
      ...
      {quantity > 0 && (
        <button
          onClick={removeCard}
          className="rounded-xl border border-white/[0.08] bg-[#111821]/85 hover:bg-neutral-950 px-4 py-2 text-xs font-bold text-zinc-400 hover:text-rose-400 transition-all duration-200 flex items-center gap-2 active:scale-95 cursor-pointer"
        >
      ...
      <button
        ...
        className={`rounded-xl px-4 py-2 text-xs font-bold transition-all duration-200 flex items-center gap-2 active:scale-95 cursor-pointer ${
          favorite
            ...
            : "border border-white/[0.08] bg-neutral-950/20 text-zinc-500 hover:border-zinc-800 hover:text-zinc-300"
        }`}
      >
        {favorite ? (
          <>
