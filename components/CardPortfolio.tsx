...
import type { CardCondition, CardPrintVariantKey, PokemonCard } from "../lib/types";
...
export default function CardPortfolio({ card, currentValue, onPrintingVariantChange, onConditionChange }: Props) {
  currentValue = Number.isFinite(Number(currentValue)) ? Number(currentValue) : 0;
  const [quantity, setQuantity] = useState(0);
  const [buyPrice, setBuyPriceState] = useState(0);
  const [condition, setConditionState] = useState<CardCondition>(card.condition || "Near Mint");
  const availableVariants = useMemo(
    () => card.availablePrintVariants?.length
  ...
  useEffect(() => {
    const refresh = () => {
      ...
      setConditionState(getCondition(card.id) as CardCondition);
  ...
  }, [card.id, card.selectedPrintVariant, availableVariants]);
  ...
  const updateCondition = (value: CardCondition) => {
    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-3 grid-cols-1 sm:grid-cols-3 pt-2 border-t border-white/[0.08]">
        ...
        <div>
          ...
          <select
            value={condition}
            onChange={(e) => updateCondition(e.target.value as CardCondition)}
