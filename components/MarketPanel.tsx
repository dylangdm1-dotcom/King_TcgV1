...
export default function MarketPanel({
  ...
  const languageNames: Record<MarketQuote["language"], string> = {
    fr: "français",
    en: "anglais",
    ja: "japonais",
    "zh-tw": "chinois",
  ...
  const preferredQuote = (source: MarketQuote["source"]) =>
    quotes
      .filter((quote) => quote.source === source && quote.price > 0)
      .sort((left, right) => {
        const priority = (quote: MarketQuote) => {
          if (quote.language === language && quote.classification === "exact" && quote.compatible) return 6;
          if (quote.language === language && quote.compatible) return 5;
          if (quote.language === "multi" && quote.compatible) return 4;
          if (quote.classification === "comparable" && quote.compatible) return 3;
          if (quote.language === language) return 2;
          return 1;
        };
        return priority(right) - priority(left);
      })[0];
  const quoteScope = (quote?: MarketQuote) => {
    if (!quote) return `Aucune cotation ${targetLanguageName} compatible trouvée`;
    const sourceLanguage = languageNames[quote.language];
    if (quote.language === language && quote.classification === "exact") {
      return `Cotation ${targetLanguageName} exacte · ${quote.condition}`;
    }
    if (quote.language === language) {
      return `Cotation ${targetLanguageName} compatible · ${quote.condition}`;
    }
    if (quote.language === "multi") {
      return `Référence multilingue ${quote.compatible ? "incluse" : "indicative"} · ${quote.condition}`;
    }
    return `Comparaison ${sourceLanguage} · ${quote.compatible ? "incluse" : "hors cote"}`;
  };
  const tcgplayerQuote = preferredQuote("tcgplayer");
  const justTcgQuote = preferredQuote("justtcg");
  const ebayQuote = preferredQuote("ebay");
  ...
  const sources = [
    {
      title: language === "fr"
        ...
        : language === "ja" || language === "zh-tw"
          ? `Cardmarket · impression ${targetLanguageName}`
      ...
      subtitle: language === "fr"
        ...
        : language === "ja" || language === "zh-tw"
          ? "Référence occidentale du produit exact · pas un prix local"
    ...
    },
    {
      title: language === "fr"
        ...
        : language === "ja" || language === "zh-tw"
          ? "Cardmarket Europe · repère 7 jours"
          : "Cardmarket Europe",
      subtitle: language === "fr"
        ? "Prix moyen du marché européen sur 7 jours"
        : language === "ja" || language === "zh-tw"
          ? "Référence occidentale indicative · pas un prix local"
    ...
    {
      title: tcgplayerQuote?.language === language
        ? `TCGPlayer · marché ${targetLanguageName}`
        : tcgplayerQuote && tcgplayerQuote.language !== "multi"
          ? `TCGPlayer · comparaison ${languageNames[tcgplayerQuote.language]}`
          : `TCGPlayer · cible ${targetLanguageName}`,
      subtitle: quoteScope(tcgplayerQuote),
      source: "tcgplayer" as MarketSource,
      value: tcgplayer ?? 0,
    },
    {
      title: justTcgQuote?.language === language
        ? `JustTCG · marché ${targetLanguageName}`
        : "JustTCG · référence comparable",
      subtitle: quoteScope(justTcgQuote),
      source: "justtcg" as MarketSource,
      value: justtcg ?? 0,
    },
    {
      title: ebayQuote?.language === language
        ? `eBay · marché ${targetLanguageName}`
        : "eBay · annonces comparables",
      subtitle: ebayQuote?.condition === "Near Mint"
        ? `${ebayQuote.sampleSize ?? 0} annonces actives · ${quoteScope(ebayQuote)}`
        : ebayQuote
