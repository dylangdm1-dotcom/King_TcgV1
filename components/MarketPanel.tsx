...
export default function MarketPanel({
  ...
  const hasExactLocalQuote = quotes.some(
    (quote) => quote.compatible && quote.classification === "exact" && quote.condition === "Near Mint" && quote.price > 0
  );
  const usingEuropeReference = !hasExactLocalQuote && Number(cardmarketEurope) > 0 && Number(average) > 0;

  const sources = [
    {
      title: "Cardmarket FR",
      subtitle: "Prix FR vérifié · Near Mint",
      source: "cardmarket" as MarketSource,
      value: cardmarket ?? 0,
    },
    {
      title: "Cardmarket Europe",
      subtitle: "Tendance européenne · toutes langues",
      source: "cardmarket" as MarketSource,
  ...
  return (
    <div className="space-y-2.5">
      ...
      {quotes.length ? (
        <details className="group rounded-[14px] border border-white/[0.07] bg-[#171e27] px-3 py-2.5">
          <summary className="cursor-pointer list-none text-[9px] font-black uppercase tracking-[0.15em] text-zinc-500 transition hover:text-zinc-300">
            Voir les détails marché · {quotes.length} cotation{quotes.length > 1 ? "s" : ""}
          </summary>
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-[8px] text-zinc-600">
                    {quote.condition} · {quote.language.toUpperCase()} · {quote.metric}
                    {" · "}
          ...
          <div className="relative flex items-start justify-between gap-3">
            <div>
              <p className="text-[9px] font-black uppercase tracking-[0.16em] text-cyan-200/80">
                {usingEuropeReference ? "Référence marché" : "Cote King_TCG"}
              </p>
              <p className="mt-1 text-[11px] font-medium text-zinc-400">
                {usingEuropeReference
                  ? "Cardmarket Europe · valeur réelle indicative, pas une cote locale NM."
                  : "Médiane des cotations Near Mint compatibles avec la langue de la carte."}
              </p>
            </div>
