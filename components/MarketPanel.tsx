...
export default function MarketPanel({
  ...
  const pokeWalletCardmarket = quotes.find(
    (quote) => quote.source === "pokewallet" && quote.label.includes("Cardmarket")
  );
  const pokeWalletTcgplayer = quotes.find(
    (quote) => quote.source === "pokewallet" && quote.label.includes("TCGPlayer")
  );

  const sources = [
    {
      title: "Cardmarket",
      subtitle: pokeWalletCardmarket
        ? "Prix exact via PokéWallet"
        : "Référence européenne disponible",
      source: "cardmarket" as MarketSource,
      value: cardmarket ?? 0,
    },
    {
      title: "TCGPlayer",
      subtitle: pokeWalletTcgplayer
        ? "Market via PokéWallet"
        : "Marché anglais",
    ...
    {
      title: "PokéWallet",
      subtitle: pokeWalletCardmarket || pokeWalletTcgplayer
        ? "Source gratuite complémentaire"
        : "Clé optionnelle non configurée",
      source: "pokewallet" as MarketSource,
      value: pokeWalletCardmarket?.price ?? pokeWalletTcgplayer?.price ?? 0,
    },
    {
      title: "eBay",
      subtitle: "Aucune source de ventes réelle branchée",
  ...
  return (
    <div className="space-y-2.5">
      ...
      {quotes.length ? (
        <div className="rounded-[14px] border border-white/[0.07] bg-[#171e27] px-3 py-2.5">
          ...
          <div className="relative flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-[11px] font-medium text-zinc-400">
                Médiane des cotations compatibles avec la langue de la carte.
              </p>
            </div>
