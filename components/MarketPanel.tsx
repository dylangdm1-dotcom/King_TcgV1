...
export default function MarketPanel({
  ...
  const sources = [
    ...
    { title: "Annonces eBay", subtitle: "Médiane des annonces actives comparables", source: "ebay" as MarketSource, value: ebay ?? 0 },
  ];
  ...
  return (
    <div className="space-y-3.5">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        {sources.map((item) => (
          <div key={item.title} className="rounded-[16px] border border-white/[0.09] bg-[#1b232e] p-3 shadow-[inset_0_1px_0_rgba(255,255,255,.035)] transition duration-200 hover:-translate-y-0.5 hover:border-white/[0.14]">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-[10px] font-black uppercase tracking-[0.14em] text-zinc-300">{item.title}</p>
                <p className="mt-1 min-h-0 text-[9px] font-medium leading-4 text-zinc-500">{item.subtitle}</p>
              </div>
              <MarketSourceBadge source={item.source} compact />
            </div>
            <p className="mt-2.5 text-lg font-black tracking-tight text-white tabular-nums">{euro(item.value)}</p>
          </div>
        ))}
      </div>

      <div className="grid gap-3 sm:grid-cols-[1.35fr_.65fr]">
        <div className="relative overflow-hidden rounded-[17px] border border-cyan-300/18 bg-[linear-gradient(135deg,rgba(34,211,238,.10),rgba(21,29,39,.96)_48%)] p-4">
          ...
          <p className="relative mt-3 text-2xl font-black tracking-[-0.035em] text-white tabular-nums">{euro(average ?? 0)}</p>
        </div>

        <div className="rounded-[20px] border border-white/[0.09] bg-[#181f29] p-4">
          ...
          <p className="mt-3 text-xl font-black text-white tabular-nums">{(spread ?? 0) > 0 ? "+" : ""}{(spread ?? 0).toFixed(2)} €</p>
        </div>
      </div>
    </div>
  );
