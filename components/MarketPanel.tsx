...
export default function MarketPanel({
  ...
  const cardmarketEurope1dQuote = quotes.find(
    (quote) =>
  ...
  const sources = [
    ...
    {
      title: "JustTCG",
      subtitle: "Prix variante exacte · langue + état + impression",
    ...
    <div className="space-y-2.5">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-xl text-[10px] font-medium leading-4 text-zinc-400">
            {language === "fr"
              ? "Cote FR provisoire : moyenne Cardmarket 1 j + moyenne Europe 7 j + eBay compatible."
              : "Chaque valeur garde sa vraie source. Une cotation d’une autre langue peut être visible, mais elle n’entre pas dans la cote King_TCG."}
          </p>
        </div>
        {onRefresh ? (
          <button
      ...
      {language === "fr" ? (
        <div className="rounded-[12px] border border-amber-400/25 bg-amber-400/[0.05] px-3 py-2 text-[10px] leading-4 text-amber-100">
          ...
          {debugCardmarketFr ? (
            <>
              étape {debugCardmarketFr.stage || "?"} · statut {debugCardmarketFr.fetchStatus || "?"} ·
              produit {debugCardmarketFr.cardmarketProductId || "aucun"} ·
              identité {debugCardmarketFr.identitySource || "aucune"} ·
              {debugCardmarketFr.articleRows} articleRow · FR/NM [{debugCardmarketFr.frNmPrices.join(", ") || "aucun"}] ·
              HTML 210€ {debugCardmarketFr.htmlHas210 ? "oui" : "non"} ·
              HTML 278,99€ {debugCardmarketFr.htmlHas27899 ? "oui" : "non"}
            </>
          ) : (
            <>aucun diagnostic reçu depuis l’API prix</>
          )}
        </div>
      ) : null}

      {debugJustTcg ? (
        <div className="rounded-[12px] border border-cyan-400/20 bg-cyan-400/[0.04] px-3 py-2 text-[10px] leading-4 text-cyan-100">
          <span className="font-black">Diagnostic temporaire JustTCG :</span>{" "}
          clé {debugJustTcg.keyConfigured ? "configurée" : "absente"} ·
          étape {debugJustTcg.stage} · statut {debugJustTcg.status || "—"} ·
          lookup {debugJustTcg.lookup || "—"} · candidats {debugJustTcg.candidateCount ?? 0} ·
          variantes exactes {debugJustTcg.matchingVariantCount ?? 0}
          {debugJustTcg.condition ? ` · ${debugJustTcg.condition}` : ""}
          {debugJustTcg.printing ? ` · ${debugJustTcg.printing}` : ""}
          {debugJustTcg.selectedPriceEur
            ? ` · prix ${debugJustTcg.selectedPriceEur.toFixed(2)} €`
            : ""}
          {debugJustTcg.priceChange7d != null
            ? ` · 7j ${debugJustTcg.priceChange7d >= 0 ? "+" : ""}${debugJustTcg.priceChange7d}%`
            : ""}
        </div>
      ) : null}
          ...
          <div className="relative flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-[11px] font-medium text-zinc-400">
                {language === "fr"
                  ? "Cote provisoire : Cardmarket 1 j + Europe 7 j + eBay compatible, avec garde anti-outlier."
