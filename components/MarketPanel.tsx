...
function MarketCard({ title, subtitle, icon, source, value, featured = false }: MarketCardProps) {
  return (
    <div className={`kt-market-source-card ${featured ? "kt-market-source-card-featured" : ""}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[10px] font-medium leading-4 text-zinc-500">
            {subtitle}
          </p>
        </div>
        {source ? (
          <MarketSourceBadge source={source} compact />
        ) : (
          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] text-zinc-300">
            {icon}
          </span>
        )}
      </div>
