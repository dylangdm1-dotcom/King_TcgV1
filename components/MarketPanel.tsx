...
const euro = (value: unknown) => { const n = Number(value); return Number.isFinite(n) && n > 0 ? `${n.toFixed(2)} €` : "—"; };
...
export default function MarketPanel({
  ...
  const sources = [
    ...
    { title: "TCGPlayer", subtitle: "Marché anglais", source: "tcgplayer" as MarketSource, value: tcgplayer ?? 0 },
    { title: "JustTCG", subtitle: "Médiane Near Mint · langue exacte", source: "justtcg" as MarketSource, value: justtcg ?? 0 },
    ...
    <div className="space-y-2.5">
      ...
      <div className="grid gap-3 sm:grid-cols-[1.35fr_.65fr]">
        ...
        <div className="rounded-[20px] border border-white/[0.09] bg-[#181f29] p-3">
          ...
          <p className="mt-2 text-lg font-black text-white tabular-nums">{Number(spread) > 0 ? "+" : ""}{Number.isFinite(Number(spread)) ? Number(spread).toFixed(2) : "0.00"} €</p>
        </div>
      </div>
    </div>
  );
