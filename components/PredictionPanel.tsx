...
export default function PredictionPanel({ predictedPrice30d = 0, roi30d = 0, confidence = 0, rangeLow = 0, rangeHigh = 0, qualityLabel = "Insuffisante", evidence = [] }: Props) {
    ...
    <div className="space-y-2.5">
      ...
      <div className="grid gap-3 lg:grid-cols-[1.35fr_.65fr]">
        <div className="relative overflow-hidden rounded-[17px] border border-violet-300/15 bg-[linear-gradient(135deg,rgba(139,92,246,.13),rgba(21,29,39,.97)_52%)] p-3.5">
          ...
          <p className="relative text-[10px] font-black uppercase tracking-[0.18em] text-violet-200/70">Scénario central</p>
          ...
          <div className="relative mt-2.5 flex items-center justify-between gap-4 border-t border-white/[0.07] pt-2.5">
            <span className="text-[10px] font-medium text-zinc-100">Variation du scénario</span>
            <span className={`text-lg font-black tabular-nums ${roi30d >= 0 ? "text-emerald-200" : "text-rose-300"}`}>{roi30d >= 0 ? "+" : ""}{roi30d.toFixed(2)} %</span>
          </div>
        </div>

        <div className="kt-metric-cell rounded-[17px] border p-3.5">
          <div className="flex items-center gap-2">
            <ShieldCheck className="h-4 w-4 text-emerald-200" />
            <span className="text-[10px] font-black uppercase tracking-[0.15em] text-zinc-200">Couverture des données</span>
          </div>
          ...
          <div className="mt-2 flex items-center gap-2 text-[10px] text-zinc-200">
            ...
            {qualityLabel} · {rangeLow.toFixed(2)} € à {rangeHigh.toFixed(2)} €
          </div>
        </div>
      </div>
      <p className="text-[10px] leading-4 text-zinc-400">Base : {evidence.length ? evidence.join(" · ") : "aucune donnée exploitable"}. La courbe reconstruite n’augmente jamais cette couverture.</p>
    </div>
  );
}
