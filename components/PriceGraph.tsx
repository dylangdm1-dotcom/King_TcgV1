...
export default function PriceGraph({ cardId }: Props) {
  ...
  const history: PricePoint[] = useMemo(
    () => (cardId ? getMarketHistoryDays(cardId, range) : []),
    [cardId, range]
  );
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="space-y-1">
          <h2 className="flex items-center gap-2 text-sm font-bold uppercase tracking-widest text-zinc-400">
            <BarChart3 className="h-4 w-4 text-cyan-400" /> Analyse de marché globale
          </h2>
        ...
        <div className="flex h-[280px] w-full items-center justify-center">
          {!mounted ? (
            <div className="flex items-center gap-2 text-[11px] font-bold text-zinc-600">
              <Activity className="h-3.5 w-3.5 animate-pulse text-cyan-500" /> Indexation...
            </div>
          ) : data.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-2 text-center text-zinc-500">
              ...
              <p className="text-xs font-medium">
                Aucun historique enregistré pour cette période.
              </p>
              <p className="text-[10px] text-zinc-600">
                L'historique se construira automatiquement lors de vos prochaines visites.
              </p>
            </div>
          ) : (
