...
export default function PriceStats({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="kt-section-surface rounded-[18px] border p-5">
      ...
      <div className="rounded-[18px] border border-violet-300/14 bg-violet-300/[0.045] p-4">
        <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.16em] text-violet-200">
        ...
        {frenchMode ? (
          ...
          <p className="mt-2 text-xs font-medium leading-5 text-zinc-200">{opportunity || "Analyse des écarts et de l’historique en cours…"}</p>
        )}
      </div>
    </div>
  );
...
function Metric({ label, value, tone = "neutral" }: { label: string; value: string; tone?: "low" | "high" | "neutral" }) {
  const styles = tone === "low"
    ...
    : "kt-metric-cell text-zinc-200";

  return (
    <div className={`min-w-0 rounded-xl border px-3 py-2.5 text-center ${styles}`}>
      <span className="block text-[10px] font-black uppercase tracking-[0.14em] opacity-60">{label}</span>
