...
import { ArrowRight, Hash, ImageOff, Loader2, TrendingUp } from 'lucide-react';
...
export default function CardResult({ card, isPriceLoading = false }: Props) {
  ...
  return (
    <article className="kt-search-card group flex h-full flex-col">
      ...
      <div className="flex min-h-[160px] flex-1 flex-col pt-3">
        <div>
        ...
        <div className="mt-3 border-t border-white/[0.08] pt-3">
          <div className="flex items-end justify-between gap-2">
            ...
            {priceAvailable && price > 0 ? (
              <div className="flex items-center gap-1 rounded-full border border-emerald-200/15 bg-emerald-300/[0.08] px-2 py-1 text-[8px] font-black text-emerald-200"><TrendingUp className="h-3 w-3" /> Marché</div>
            ) : null}
          </div>
        </div>
