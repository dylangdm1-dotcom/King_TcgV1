...
export default function ConditionValueBars({ data }: { data: ConditionValueDatum[] }) {
  ...
  if (!visible.length) {
    return <div className="kt-empty-state-rich min-h-[110px]"><p className="text-[11px] font-black text-white">Valeur par état indisponible</p><p className="text-[10px]">Renseignez l’état de vos cartes pour comparer le volume et la valeur de chaque catégorie.</p></div>;
  }
  ...
  return (
    <div className="grid gap-x-5 gap-y-3 sm:grid-cols-2">
      {visible.map((item) => {
        ...
        return (
          <div key={item.condition} className="kt-data-row rounded-xl border px-3 py-2.5">
            ...
            <p className="mt-1.5 text-[9px] font-semibold text-zinc-400">{item.count} carte{item.count > 1 ? "s" : ""}</p>
          </div>
        );
      })}
    </div>
