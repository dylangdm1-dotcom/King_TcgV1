...
function DonutTooltip({ active, payload }: any) {
  ...
  return (
    <div className="rounded-xl border border-white/[0.14] bg-[#151b23] px-3 py-2 shadow-2xl">
...
export default function RarityDonut({ data }: { data: RarityDatum[] }) {
  ...
  if (!total) {
    return <div className="flex min-h-[140px] items-center justify-center rounded-2xl border border-white/[0.08] bg-[#1a212b] px-5 text-center text-[10px] font-semibold text-zinc-500">La répartition apparaîtra dès que la collection contient des cartes enrichies.</div>;
  }
  ...
  return (
    <div className="grid grid-cols-[128px_1fr] items-center gap-4 sm:grid-cols-[145px_1fr]">
      ...
      <div className="space-y-1.5">
        {filtered.map((item) => (
          <div key={item.name} className="flex items-center justify-between gap-2 rounded-xl border border-white/[0.07] bg-[#1a212b] px-2.5 py-2">
