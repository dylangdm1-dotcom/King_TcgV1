...
export default function CollectionCard({ card }: Props) {
  ...
  return (
    <Link href={`/card/${card.id}`} className="group block">
      <div className="relative overflow-hidden rounded-xl border border-white/[0.08] bg-[#111821]/85 p-4 transition-all duration-200 hover:border-cyan-500/20 hover:bg-neutral-950/60">
        
        {/* Conteneur Image Épuré avec Badge de Volume */}
        <div className="relative overflow-hidden rounded-lg bg-neutral-950 border border-white/[0.08]/50 aspect-[4/3] flex items-center justify-center p-3">
          <img
          ...
          />
          <span className="absolute bottom-2 right-2 rounded bg-neutral-950/90 border border-white/[0.08] px-2 py-0.5 text-[11px] font-black tracking-tight text-zinc-300 tabular-nums">
            x{card.qty}
          </span>
        </div>
        ...
        <div className="mt-3 space-y-2 border-t border-white/[0.08]/80 pt-2.5 text-[11px]">
          ...
          <div className="flex justify-between items-center border-t border-white/[0.08] pt-2 mt-1">
            <span className="text-[9px] font-black uppercase tracking-wider text-zinc-500">Allocation totale</span>
