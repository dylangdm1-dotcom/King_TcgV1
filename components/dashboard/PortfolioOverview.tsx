...
export default function PortfolioOverview({ cards }: Props) {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener("king_tcg_update", update);
    };
  }, []);

  // Force le recalcul après événement King_TCG
  void refresh;
  ...
  const bestResult = ownedCards.reduce<{
  ...
  const bestCard = bestResult.card;
  const bestProfit = bestResult.profit;
  ...
  return (
    <section className="rounded-xl border border-white/[0.08] bg-[#111821]/85 p-4 sm:p-5">
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-6">
        <div className="rounded-lg border border-white/[0.08] bg-neutral-950/60 p-3">
        ...
        <div className="rounded-lg border border-white/[0.08] bg-neutral-950/60 p-3">
          ...
          <p className="mt-1 text-base font-black text-zinc-300 tabular-nums">
            {invested.toFixed(2)} €
          </p>
        </div>
        ...
        <div className="rounded-lg border border-white/[0.08] bg-neutral-950/60 p-3">
          ...
          <p className="mt-1 text-base font-black text-cyan-400 tabular-nums">
            {value.toFixed(2)} €
          </p>
        </div>
      ...
      {bestCard && (
        <div className="mt-4 rounded-lg border border-white/[0.08] bg-neutral-950/60 p-3 flex items-center justify-between">
