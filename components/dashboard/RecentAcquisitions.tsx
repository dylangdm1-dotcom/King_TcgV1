...
export default function RecentAcquisitions({
  cards,
}: Props) {
  ...
  const recent = cards
    ...
    .sort(
      (a, b) =>
        new Date(getPurchaseDate(b.id)).getTime() -
        new Date(getPurchaseDate(a.id)).getTime()
    )
    .slice(0, 5);
    ...
    <section className="rounded-xl border border-zinc-900 bg-neutral-950/40 p-4 sm:p-5">
      ...
      <div className="space-y-2">
        {recent.length === 0 ? (
          ...
          recent.map((card) => {
            const purchaseDate = getPurchaseDate(card.id);
