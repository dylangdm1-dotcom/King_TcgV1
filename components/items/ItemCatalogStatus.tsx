...
export default function ItemCatalogStatus({ manifest, offline = false }: { manifest: ItemCatalogManifest | null; offline?: boolean }) {
  return (
    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div className="kt-subpanel flex items-center gap-3 px-3 py-2.5"><ImageIcon className="h-4 w-4 text-cyan-300" /><div><p className="text-[8px] font-black uppercase tracking-[0.08em] text-zinc-500">Visuels disponibles</p><p className="text-[10px] font-black text-white">{manifest?.imageCount ?? 0} visuel{manifest?.imageCount === 1 ? "" : "s"} fournisseur</p></div></div>
      <div className="kt-subpanel flex items-center gap-3 px-3 py-2.5"><Languages className="h-4 w-4 text-amber-300" /><div><p className="text-[8px] font-black uppercase tracking-[0.08em] text-zinc-500">Catalogue français</p><p className="text-[10px] font-black text-white">{manifest?.languageStatus?.fr?.itemCount ? `${manifest.languageStatus.fr.itemCount} items · ${manifest.languageStatus.fr.imageCount} visuels` : "FR propre · en préparation"}</p></div></div>
