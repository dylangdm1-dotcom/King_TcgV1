      ...
      <section className="kt-item-detail-grid grid gap-5 lg:grid-cols-[minmax(0,420px)_1fr]">
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <div className="kt-subpanel p-3"><p className="text-[8px] font-black uppercase tracking-[0.08em] text-zinc-500">Cote actuelle</p><p className={`mt-2 text-lg font-black ${pricing.currentMarket ? "text-emerald-300" : "text-zinc-500"}`}>{formatItemMoney(pricing.currentMarket)}</p><p className="mt-1 text-[9px] text-zinc-500">{pricing.currentMarket ? "Marché TCGplayer EN/US · jamais converti en cote FR." : "Jamais remplacée par un prix magasin."}</p></div>
            <div className="kt-subpanel p-3"><p className="text-[8px] font-black uppercase tracking-[0.08em] text-zinc-500">Prix de sortie officiel</p><p className={`mt-2 text-lg font-black ${pricing.officialRetail ? "text-amber-300" : "text-zinc-500"}`}>{formatItemMoney(pricing.officialRetail)}</p><p className="mt-1 text-[9px] text-zinc-500">Conservé comme référence distincte.</p></div>
          </div>
