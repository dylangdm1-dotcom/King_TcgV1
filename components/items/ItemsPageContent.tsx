...
import { Bookmark, Crown, FileSpreadsheet, Library, PackageOpen, Plus, Settings2, ShieldCheck, Store } from "lucide-react";
...
export default function ItemsPageContent() {
  ...
  useEffect(() => {
    ...
    fetchItemSourceStatus(controller.signal).then((result) => setCardTraderReady(result.cardtrader?.configured ?? null));
  ...
  return (
    <div className="kt-page-wrap space-y-5">
      <header className="kt-page-header kt-hero-surface relative overflow-hidden border">
        ...
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="mb-2 flex flex-wrap items-center gap-2"><ItemAccessBadge /><span className="rounded-full border border-cyan-300/15 bg-cyan-300/[0.05] px-2 py-1 text-[8px] font-black uppercase tracking-[0.08em] text-cyan-300">V295 · Bêta</span></div>
      ...
      <section className="grid gap-2 sm:grid-cols-2">
        <div className="rounded-[15px] border border-amber-300/[0.16] bg-amber-300/[0.035] px-4 py-3"><p className="text-[9px] font-black uppercase tracking-[0.1em] text-amber-300">FR · En préparation</p><p className="mt-1 text-[10px] leading-5 text-zinc-300">{cardTraderReady === true ? "Connecteur CardTrader actif. La console propriétaire V295 permet maintenant de vérifier puis exporter les candidats FR avant publication." : cardTraderReady === false ? "CardTrader attend sa variable serveur sur cet environnement. Aucun produit anglais ne remplit artificiellement le catalogue français." : "Vérification du connecteur CardTrader en cours. Aucun produit anglais ne remplit artificiellement le catalogue français."}</p>{cardTraderReady === true ? <Link href="/parametres/items-cardtrader" className="mt-2 inline-flex items-center gap-2 text-[8px] font-black uppercase tracking-[0.08em] text-amber-200"><Settings2 className="h-3.5 w-3.5" /> Ouvrir le contrôle propriétaire</Link> : null}</div>
