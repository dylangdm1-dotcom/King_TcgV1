...
const sourceMap: Record<MarketSource, SourceItem> = {
  ...
  justtcg: {
    ...
    logoClassName: "h-5 w-auto max-w-[82px] object-contain",
    className: "bg-[#f4f6f8] text-zinc-900",
  },
...
export default function MarketSourceBadge({ source, compact = false }: { source: MarketSource; compact?: boolean }) {
  ...
  return (
    <span
      className={`kt-logo-tile inline-flex min-h-8 items-center justify-center gap-2 rounded-lg px-2.5 py-1.5 ${item.className}`}
