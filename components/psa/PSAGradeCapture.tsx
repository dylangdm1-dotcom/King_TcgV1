...
export default function PSAGradeCapture() {
  ...
  return (
    <div className="space-y-5">
              ...
              return (
                ...
                >
                  <div className="relative aspect-[63/88] w-full overflow-hidden bg-black/25">
                    ...
                    <span className="absolute left-1.5 top-1.5 flex h-5 w-5 items-center justify-center rounded-full border border-white/10 bg-black/70 text-[8px] font-black text-white">
                      {index + 1}
                    </span>
                    {photo ? (
                  ...
                  <div className="px-2 py-2">
                    <p className="truncate text-[9px] font-black text-white">{step.shortTitle}</p>
                    <p className="mt-0.5 text-[8px] text-zinc-600">{photo ? "Reprendre" : "À capturer"}</p>
                  </div>
                </button>
              );
            })}
      ...
      {analysisError ? (
        <section className="rounded-[20px] border border-rose-400/20 bg-rose-400/[0.06] p-4">
          <div className="flex items-start gap-3">
      ...
      {analysis ? (
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
            {CRITERIA.map((criterion) => {
              ...
              return (
                <article
                  key={criterion.key}
                  className={`rounded-[20px] border p-4 ${scoreTone(value.score)}`}
                >
                  <div className="flex items-end justify-between gap-2">
                    <div>
                      <p className="text-[9px] font-black uppercase tracking-wider opacity-75">
                        {criterion.label}
                      </p>
                  ...
                  {value.observations[0] ? (
                    <p className="mt-2 text-[9px] leading-4 text-zinc-400">{value.observations[0]}</p>
                  ) : null}
                </article>
              );
            })}
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <article className="kt-premium-panel rounded-[22px] p-5">
                  ...
                  {analysis.defects.map((defect, index) => (
                    <div key={`${defect.description}-${index}`} className="rounded-2xl border border-white/[0.07] bg-black/20 px-3 py-2.5">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-[9px] font-black uppercase tracking-wider text-zinc-400">{defect.area}</span>
          ...
          <p className="px-2 text-center text-[9px] leading-4 text-zinc-600">
            {analysis.disclaimer}
          </p>
        </section>
      ) : null}
          ...
          <div className="relative mx-auto h-full w-full max-w-xl overflow-hidden bg-black">
            ...
            <div className="absolute inset-x-0 top-0 z-20 flex items-start justify-between gap-3 p-4 pt-[max(1rem,env(safe-area-inset-top))]">
              ...
              <div className="min-w-0 flex-1 rounded-2xl border border-white/12 bg-black/55 px-4 py-3 text-center backdrop-blur-xl">
                ...
                <p className="mt-1 text-[9px] leading-4 text-zinc-300">{activeStep.description}</p>
              </div>
            ...
            {!reviewDataUrl ? (
              <div className="pointer-events-none absolute left-1/2 top-1/2 z-10 aspect-[63/88] w-[72%] max-w-[330px] -translate-x-1/2 -translate-y-1/2">
                ...
                <span className="absolute -bottom-12 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full border border-white/12 bg-black/60 px-3 py-2 text-[9px] font-black uppercase tracking-wider text-white backdrop-blur-xl">
            ...
            {cameraError ? (
              <div className="absolute left-4 right-4 top-1/2 z-30 -translate-y-1/2 rounded-[22px] border border-rose-300/25 bg-[#15171c]/95 p-5 text-center backdrop-blur-xl">
            ...
            <div className="absolute inset-x-0 bottom-0 z-20 px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))]">
              <div className="mb-4 rounded-2xl border border-white/10 bg-black/55 px-4 py-3 text-center backdrop-blur-xl">
                <div className="flex items-start justify-center gap-2">
                  ...
                  <p className="text-[9px] leading-4 text-zinc-300">{activeStep.hint}</p>
                </div>
              </div>
