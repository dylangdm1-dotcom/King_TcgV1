...
export default function PSAGradeCapture() {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="kt-premium-panel kt-psa-camera-panel overflow-hidden rounded-[24px]">
        <div className="border-b border-cyan-300/[0.26] bg-[linear-gradient(135deg,rgba(14,165,233,0.22),rgba(34,211,238,0.10),rgba(17,24,39,0.42))] p-5 sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex items-start gap-3">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-cyan-300/[0.42] bg-cyan-400/[0.12] text-cyan-200">
            ...
            <div className="grid grid-cols-2 gap-2 self-start">
              <div className="flex items-center gap-2 rounded-xl border border-cyan-300/[0.46] bg-cyan-400/[0.09] px-3 py-2">
                <span className="text-xs font-black text-white">{completedCount}/4</span>
                <span className="text-[9px] font-bold uppercase tracking-[0.09em] text-zinc-200">vues validées</span>
              </div>
              <div className="flex items-center justify-center gap-1.5 rounded-xl border border-[#f5c451]/35 bg-[#f5c451]/[0.07] px-3 py-2 text-[#f5c451]">
                <Sparkles className="h-3.5 w-3.5" />
                <span className="text-[9px] font-black uppercase tracking-[0.09em]">Premium</span>
              </div>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-5">
          <div className="grid grid-cols-4 gap-2">
            {CAPTURE_STEPS.map((step, index) => {
              ...
              return (
                ...
                >
                  <div className="relative aspect-[63/88] w-full overflow-hidden bg-white/[0.04]">
                    {photo ? (
                      <img
                        src={photo.dataUrl}
                        alt={step.title}
          ...
          <div className="mt-3 flex items-start gap-2 rounded-2xl border border-amber-300/12 bg-amber-400/[0.05] px-3 py-2.5">
            ...
            <p className="text-[10px] leading-4 text-zinc-100">
        ...
        <section className="rounded-[18px] border border-rose-400/20 bg-rose-400/[0.06] p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[10px] leading-4 text-zinc-100">{analysisError}</p>
              <button
                type="button"
                onClick={analyzePhotos}
        ...
        <section className="space-y-4">
            ...
            <div className="p-5 sm:p-6">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-2 max-w-xl text-[11px] leading-5 text-zinc-100">
                    {analysis.summary}
                  </p>
                  {isRefined ? (
                ...
                <div className="min-w-[150px] rounded-[18px] border border-white/[0.08] bg-white/[0.04] p-4 text-center">
                  <p className="text-[10px] font-bold uppercase tracking-[0.11em] text-zinc-200">Confiance</p>
              ...
              {!analysis.photoQuality.acceptable ? (
                <div className="mt-4 rounded-2xl border border-amber-300/18 bg-amber-400/[0.06] p-3">
                  ...
                  <ul className="mt-2 space-y-1 text-[10px] leading-4 text-zinc-100">
              ...
              return (
                ...
                >
                  ...
                  {value.observations[0] ? (
                    <p className="mt-2 text-[10px] leading-4 text-zinc-100">{value.observations[0]}</p>
                  ) : null}
                </article>
              );
            })}
              ...
              {analysis.defects.length ? (
                <div className="mt-3 space-y-2">
                  {analysis.defects.map((defect, index) => (
                    <div key={`${defect.description}-${index}`} className="rounded-2xl border border-white/[0.07] bg-white/[0.035] px-3 py-2.5">
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-[10px] font-bold uppercase tracking-[0.11em] text-zinc-100">{defect.area}</span>
                      ...
                      <p className="mt-1 text-[10px] leading-4 text-zinc-100">{defect.description}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="mt-3 text-[10px] leading-4 text-zinc-200">Aucun défaut évident signalé sur les photographies fournies.</p>
              )}
            </article>
              ...
              <div className="mt-3 space-y-2">
                {analysis.recommendations.length ? (
                  analysis.recommendations.map((recommendation) => (
                    <div key={recommendation} className="flex items-start gap-2 text-[10px] leading-4 text-zinc-100">
                      <ChevronRight className="mt-0.5 h-3.5 w-3.5 shrink-0 text-blue-300" />
                      <span>{recommendation}</span>
                    </div>
                  ))
                ) : (
                  <p className="text-[10px] leading-4 text-zinc-200">Aucune recommandation complémentaire.</p>
                )}
              </div>
            </article>
          </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            <button
              type="button"
              onClick={resetAll}
              className="flex flex-1 items-center justify-center gap-2 rounded-2xl border border-white/[0.09] bg-white/[0.035] px-4 py-3 text-xs font-black text-zinc-300 transition hover:text-white"
            >
          ...
          <div className="relative mx-auto h-full w-full max-w-xl overflow-hidden bg-black">
            ...
            {cameraError ? (
              <div className="absolute left-4 right-4 top-1/2 z-30 -translate-y-1/2 rounded-[18px] border border-rose-300/25 bg-[#15171c]/95 p-5 text-center backdrop-blur-xl">
                ...
                <p className="mt-2 text-[10px] leading-4 text-zinc-100">{cameraError}</p>
                <button
                  type="button"
                  onClick={() => fileInputRef.current?.click()}
...
function ManualSelect({ label, value, options, onChange }: { label: string; value: string; options: [string, string][]; onChange: (value: string) => void }) {
  return (
    <label className="rounded-2xl border border-white/[0.07] bg-white/[0.035] p-3">
      <span className="block text-[10px] font-bold uppercase tracking-[0.11em] text-zinc-200">{label}</span>
...
function ManualToggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return (
    <button type="button" onClick={() => onChange(!checked)} className={`flex min-h-[48px] items-center justify-between gap-3 rounded-2xl border px-3 py-2.5 text-left transition ${checked ? "border-rose-300/25 bg-rose-400/[0.08] text-rose-100" : "border-white/[0.07] bg-white/[0.035] text-zinc-300"}`}>
