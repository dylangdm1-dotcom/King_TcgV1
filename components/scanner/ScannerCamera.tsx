      ...
      <div className="relative h-full w-full overflow-hidden">
        ...
        <div className="absolute top-4 right-4 z-20">
          <button
            ...
            className="rounded-2xl border border-cyan-400/20 bg-black/65 px-3 py-2 text-[10px] font-black uppercase tracking-wide text-cyan-300 shadow-xl backdrop-blur-xl transition hover:border-cyan-300/35 hover:bg-black/80 active:scale-95 flex items-center gap-1.5"
          >
            <Grid className="w-3.5 h-3.5 text-cyan-300 shrink-0" />
            <span>Scan groupé · 4 cartes</span>
          </button>
        </div>

        {isQuadOpen && (
          <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="kt-premium-panel w-full max-w-lg rounded-[24px] overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                ...
                {quadSession && (
                  <div className="grid grid-cols-2 gap-3">
                    {quadSession.slots.map((slot) => (
                      <div key={slot.slot} className="rounded-2xl border border-white/[0.07] bg-[#111821] p-3 flex flex-col items-center gap-2">
                        ...
                        <div className="flex items-center gap-1 text-[10px] font-bold">
                          ...
                          {slot.status === "empty" && <span className="text-zinc-500">En attente</span>}
                        </div>
                      </div>
                    ))}
                  </div>
