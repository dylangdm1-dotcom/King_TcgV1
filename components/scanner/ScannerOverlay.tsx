  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-10 select-none overflow-hidden">
      {mode === "quad" ? (
        <>
          ...
          {QUAD_FRAMES.map((frame, index) => (
            <div
              key={frame.slot}
              className={`absolute overflow-hidden rounded-[16px] border bg-black/5 transition-all duration-300 ${stateColor}`}
              style={{
                left: `${frame.x * 100}%`,
                top: `${frame.y * 100}%`,
            ...
            >
              {corners.map((corner) => (
                <span
                  key={corner}
                  className={`absolute h-5 w-5 ${corner} ${stateColor}`}
                />
              ))}
              <span className="absolute bottom-1.5 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-black/65 px-2 py-1 text-[8px] font-black uppercase tracking-widest text-zinc-200 backdrop-blur-md">
                Carte {index + 1}
              </span>
              {scanning && !hasResult && (
          ...
          <div className="absolute left-1/2 top-[10.5%] -translate-x-1/2 rounded-full border border-violet-300/20 bg-black/55 px-3 py-1.5 text-[8px] font-black uppercase tracking-[0.16em] text-violet-200 backdrop-blur-md">
      ...
      <div className="absolute bottom-[1.5%] inset-x-0 flex justify-center px-4">
        <div
          className={`flex max-w-[92%] items-center gap-2.5 rounded-xl border px-3 py-2 text-center text-[10px] font-black uppercase tracking-widest shadow-2xl backdrop-blur-xl transition-all duration-300 ${
            hasResult
