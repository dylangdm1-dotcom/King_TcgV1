...
export function getCardMarketPrice(card?: PokemonCard | null): number {
  // FR: use the exact live seller quote when available. Until the official
  // Cardmarket articles API is connected, fall back provisionally to the
  // Cardmarket 1-day aggregate exposed by TCGdex.
  if (card?.dataLanguage === "fr") {
    ...
    const oneDayProxy = card?.marketQuotes?.find(
      (item) =>
        item.source === "cardmarket" &&
        item.language === "multi" &&
...
export function getCardmarketEuropePrice(
  ...
  // FR: the dedicated European reference is the Cardmarket 7-day average.
  if (card?.dataLanguage === "fr") {
    const europe7d =
      card?.marketQuotes?.find(
        (item) =>
    ...
    return Number(n(europe7d?.price).toFixed(2));
  }
...
function frenchKingTcgPrice(card?: PokemonCard | null): number {
  const oneDay = card?.marketQuotes?.find(
    (quote) =>
  ...
  const sevenDay = card?.marketQuotes?.find(
    (quote) =>
      quote.source === "cardmarket" &&
      quote.language === "multi" &&
  ...
  const inputs = [
    n(oneDay?.price) || n(card?.cardmarket?.prices?.avg1),
    n(sevenDay?.price) || n(card?.cardmarket?.prices?.avg7),
    n(ebayQuote?.price) || getEbayPrice(card),
  ].filter((value) => value > 0);

  if (!inputs.length) return 0;

  // Provisional FR quote: simple mean of the three independent references
  // requested for the current production phase.
  return Number(
    (inputs.reduce((sum, value) => sum + value, 0) / inputs.length).toFixed(2)
  );
}
