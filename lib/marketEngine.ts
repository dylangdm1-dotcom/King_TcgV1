...
function frenchKingTcgPrice(card?: PokemonCard | null): number {
  ...
  if (cardmarket > 0 && ebay > 0) {
    ...
    const weight = sample >= 5 ? 0.25 : sample >= 3 ? 0.2 : 0.12;
    const boundedEbay = Math.min(cardmarket * 1.35, Math.max(cardmarket * 0.65, ebay));
...
export function getAverageMarketPrice(card?: PokemonCard | null): number {
  ...
  // V42 FR: Cardmarket remains the anchor but exact compatible eBay listings
