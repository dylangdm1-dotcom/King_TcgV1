...
export type MarketPrices = {
  ...
  ebay: number;         // Médiane eBay active ou dernière vente si disponible
  tcgplayer: number;    // Valeur de marché TCGPlayer en EUR
  justtcg: number;      // Médiane Near Mint JustTCG en EUR
...
export function getJustTcgPrice(card?: PokemonCard | null): number {
  return Number(safeNumber(card?.justtcg?.price).toFixed(2));
}

export function getEbayPrice(card?: PokemonCard | null): number {
  return Number(safeNumber(card?.ebay?.price).toFixed(2));
}
...
export function getMarketData(card?: PokemonCard | null): MarketPrices {
  ...
  const justtcg = getJustTcgPrice(card);
