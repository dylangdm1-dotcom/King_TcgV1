...
function compatibleQuote(
  ...
  const quote = card?.marketQuotes?.find((item) =>
    Boolean(item) &&
    Boolean(item?.compatible) &&
    predicate(String(item?.source || ""), String(item?.label || ""))
  );
...
export function getCardMarketPrice(card?: PokemonCard | null): number {
  // Règle temporaire choisie pour le marché français :
  // prendre le plus élevé des indicateurs Cardmarket disponibles
  // (trend, moyenne, 30 j, 7 j, 1 j), sans utiliser lowPrice.
  if (card?.dataLanguage === "fr") {
    return Number(estimatedFrenchMarketPrice(card).toFixed(2));
  }

  // JP/CN : même principe temporaire que le FR, mais uniquement sur les
  // statistiques du produit exact déjà résolu pour cette carte/langue.
...
export function getAverageMarketPrice(card?: PokemonCard | null): number {
  ...
  // Une véritable cote locale déjà résolue reste prioritaire.
  if (
    estimate > 0 &&
    card?.marketEstimate?.confidence === "high"
  ) {
    return Number(estimate.toFixed(2));
  }

  // Pour les cartes FR sans cote locale fiable, utiliser l'estimation
  // temporaire choisie : plus haute statistique Cardmarket hors lowPrice.
  if (card?.dataLanguage === "fr") {
    const frenchEstimate = getCardMarketPrice(card);
    if (frenchEstimate > 0) {
      return Number(frenchEstimate.toFixed(2));
    }
  }

  if (card?.dataLanguage === "ja" || card?.dataLanguage === "zh-tw") {
    const localizedEstimate = getCardMarketPrice(card);
    if (localizedEstimate > 0) {
      return Number(localizedEstimate.toFixed(2));
    }
  }
