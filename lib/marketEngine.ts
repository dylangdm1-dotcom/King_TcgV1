  ...
  if (card?.dataLanguage === "fr") {
    ...
    return Number(
      (
        ...
        n(card?.cardmarket?.prices?.frenchNmLow) ||
        frenchCardmarketOneDayProxy(card)
      ).toFixed(2)
    );
  }
...
export function getCardmarketEuropePrice(
  ...
  // FR: keep the real European aggregate separate from the French NM listing.
  // Prefer Cardmarket's average sell price, then trend/7d/30d if unavailable.
  if (card?.dataLanguage === "fr") {
    const europe =
      card?.marketQuotes?.find(
        (item) =>
          item.language === "multi" &&
          item.metric === "average_europe" &&
          n(item.price) > 0
      ) ??
      card?.marketQuotes?.find(
        (item) =>
      ...
      card?.marketQuotes?.find(
        (item) =>
          item.language === "multi" &&
          item.metric === "average_30d_europe" &&
          n(item.price) > 0
      );

    return Number(n(europe?.price).toFixed(2));
  }
...
function frenchCardmarketOneDayProxy(card?: PokemonCard | null): number {
  ...
  return Number((n(quote?.price) || n(card?.cardmarket?.prices?.avg1)).toFixed(2));
}

function frenchCardmarketSevenDayReference(card?: PokemonCard | null): number {
  const quote = card?.marketQuotes?.find(
    (item) =>
      item.source === "cardmarket" &&
      item.language === "multi" &&
  ...
  return Number((n(quote?.price) || n(card?.cardmarket?.prices?.avg7)).toFixed(2));
}

function frenchKingTcgPrice(card?: PokemonCard | null): number {
  const oneDay = frenchCardmarketOneDayProxy(card);
  const sevenDay = frenchCardmarketSevenDayReference(card);
  ...
  const cardmarketValues = [oneDay, sevenDay].filter((value) => value > 0);
  const anchor = cardmarketValues.length
    ? cardmarketValues.reduce((sum, value) => sum + value, 0) / cardmarketValues.length
    : 0;

  const signals: number[] = [];
  if (oneDay > 0) signals.push(oneDay);
  if (sevenDay > 0) signals.push(sevenDay);
  if (ebay > 0) {
    const boundedEbay = anchor > 0
      ? Math.min(anchor * 1.6, Math.max(anchor * 0.55, ebay))
      : ebay;
    signals.push(boundedEbay);
  }

  if (!signals.length) return 0;
  return Number(
    (signals.reduce((sum, value) => sum + value, 0) / signals.length).toFixed(2)
  );
}
