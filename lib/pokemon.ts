...
function normalize(card: any): PokemonCard {
  ...
  const rawCardmarket = Array.isArray(sourceCardmarket)
    ? {
  ...
  const rawTcgplayer = Array.isArray(sourceTcgplayer)
...
export async function searchCardsBySetId(
  ...
  const cacheKey = `set_v12_cn_private_direct_jp_tcgdex_${cleanId}_${lang}`;
