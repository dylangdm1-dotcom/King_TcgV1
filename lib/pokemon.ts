...
function buildSearchNameCandidates(value: string): string[] {
  ...
  return Array.from(new Set([value, ...explicit, english, french].filter((item): item is string => Boolean(item && item.trim()))));
}
...
export async function searchCards(
  ...
  try {
    ...
    const summaries = Array.from(new Map(responses.flat().map((card: any) => [String(card?.id || `${card?.name}-${card?.localId}`), card])).values());
    if (summaries.length) tcgdexCards = await hydrateTCGdexCards(summaries.slice(0, 120), lang, undefined, 36);
  } catch (error) {
