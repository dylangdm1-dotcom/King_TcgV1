...
const CACHE_KEY = "king_tcg_cards_cache_final_data_v1";
...
function tcgdexImageCandidates(card: any, lang: LanguageCode, setId: string, localId: string): string[] {
  ...
  if (card.image) {
    ...
    // `card.image` TCGdex est une BASE d'asset, pas une image directement chargeable.
    // Documentation TCGdex: /{quality}.{extension} doit être ajouté.
    add(`${base}/high.webp`);
    add(`${base}/high.png`);
    add(`${base}/low.webp`);
    add(`${base}/low.png`);
  ...
  // Ne pas fabriquer une URL assets TCGdex à partir de setId/localId :
  // le chemin réel contient aussi la série. Quand `card.image` est présent,
  // TCGdex nous fournit déjà la base correcte. Sinon on passe directement
  // au resolver serveur PokéWallet ci-dessous.

  // Fallback serveur PokéWallet : utile surtout pour les cartes JP/CN ou
  // Cardmarket-only dont TCGdex ne possède pas encore le visuel. La clé API
  // reste côté serveur et n'est jamais exposée au navigateur.
  if (setId && localId) {
    add(`/api/card-image?setId=${encodeURIComponent(setId)}&number=${encodeURIComponent(localId)}&lang=${encodeURIComponent(lang)}&name=${encodeURIComponent(String(card?.name ?? ""))}`);
  }
...
function applySetMetadataAndRecentSort(cards: PokemonCard[]): PokemonCard[] {
  cards.forEach((card) => {
    if (!card.set) return;
    const lang = card.dataLanguage || "fr";
    const meta = setMetadataCache.get(`${lang}:${normalizeSetId(card.set.id)}`);
    if (!meta) return;
    card.set = {
      ...card.set,
...
export async function searchCardsBySetId(
  ...
  const cacheKey = `set_final_v1_${lang}_${cleanId}`;
  ...
  if (resolvedSetId !== cleanId) {
    searchCache.set(
      `set_final_v1_${lang}_${normalizeSetId(resolvedSetId)}`,
      cards
    );
  }
  return cards;
...
export async function getCardById(id: string): Promise<PokemonCard | null> {
  ...
  if (cached) {
    return ensureDetailedMarketPrices(cached);
  }
  ...
  if (saved) {
    ...
    return ensureDetailedMarketPrices(saved);
  }

  const targetId = decodedId.startsWith("tcgdex-") ? decodedId : id;
  if (targetId.startsWith("tcgdex-")) {
    // Ne jamais parser les locales avec split("-") : zh-tw contient lui-même
    // un tiret et produisait des IDs invalides / fiches en erreur.
    const match = targetId.match(/^tcgdex-(zh-tw|fr|en|ja)-(.+)$/i);
    if (!match) return null;
    const lang = match[1].toLowerCase() as LanguageCode;
    const rawCardId = match[2];

    try {
      const response = await fetch(`${TCGDEX_URL}/${lang}/cards/${encodeURIComponent(rawCardId)}`);
      if (!response.ok) return null;
      ...
      return ensureDetailedMarketPrices(card);
    } catch (error) {
      return null;
    }
  }
