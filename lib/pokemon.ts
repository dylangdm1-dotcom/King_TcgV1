...
const CACHE_KEY = "king_tcg_cards_cache_v15_variants_images";
...
export async function searchCardsBySetId(
  ...
  const cacheKey = `set_v6_${cleanId}_${lang}`;
  ...
  if (resolvedSetId !== cleanId) {
    searchCache.set(`set_v6_${normalizeSetId(resolvedSetId)}_${lang}`, cards);
  }
  return cards;
}
