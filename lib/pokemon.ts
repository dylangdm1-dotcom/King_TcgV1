...
export async function searchCards(
  ...
  const cacheKey = `search_v4_${lang}_${normalizedKey}`;
  ...
  // Charger le catalogue de la langue avant de trier les résultats.
  // Les résumés TCGdex omettent souvent la date mais conservent l'identifiant du set.
  try { await getAllSets(lang); } catch {}
...
export async function searchCardsBySetId(
  ...
  if (resolvedSetId !== cleanId) {
    searchCache.set(`set_v4_${normalizeSetId(resolvedSetId)}_${lang}`, cards);
  }
  return cards;
}
...
export async function getAllSets(lang: LanguageCode = "fr"): Promise<any[]> {
  ...
  let mergedSets = Array.from(merged.values());
  if (targetLang === "zh-tw") {
    mergedSets = mergedSets.filter((set: any) => {
      const id = normalizeSetId(set.id);
      return !/^(?:m|me)\d+/.test(id);
    });
  }
