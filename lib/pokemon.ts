...
function normalizeTCGdexCard(card: any, lang: LanguageCode, parentSet?: any): PokemonCard {
  ...
  return normalize({
    ...
    set: {
      ...
      series: card.set?.series?.name || parentSet?.series?.name || undefined,
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (Array.isArray(data)) {
        tcgdexCards = await hydrateTCGdexCards(data.slice(0, 100), lang, undefined, 48);
      }
    }
  } catch (error) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (Array.isArray(setData?.cards)) {
        cards = await hydrateTCGdexCards(setData.cards, lang, setData, 60);
      }
    }
  } catch (error) {
