...
const REGIONAL_SET_CACHE_VERSION = "v19";
const REGIONAL_SET_CACHE_MAX_AGE = 180 * 24 * 60 * 60 * 1000;

function regionalSetCacheKey(lang: "ja" | "zh-tw"): string {
  return `king_tcg_regional_sets_${REGIONAL_SET_CACHE_VERSION}_${lang}`;
}

function loadRegionalSetsCache(lang: "ja" | "zh-tw"): any[] {
  if (typeof window === "undefined") return [];
  try {
    const raw = localStorage.getItem(regionalSetCacheKey(lang));
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    const savedAt = Number(parsed?.savedAt || 0);
    const sets = Array.isArray(parsed?.sets) ? parsed.sets : [];
    if (!savedAt || Date.now() - savedAt > REGIONAL_SET_CACHE_MAX_AGE) return [];
    return sets;
  } catch {
    return [];
  }
...
function saveRegionalSetsCache(lang: "ja" | "zh-tw", sets: any[]) {
  if (typeof window === "undefined" || !sets.length) return;
  try {
    localStorage.setItem(
      regionalSetCacheKey(lang),
      JSON.stringify({ savedAt: Date.now(), sets })
    );
  } catch {
    // Catalogue cache is best-effort. The curated manifest remains the fallback.
  }
}

function mergeRegionalCatalogSnapshots(
  older: any[],
  fresher: any[],
  lang: "ja" | "zh-tw"
): any[] {
  ...
  for (const set of fresher) {
    ...
    byCode.set(code, {
      ...
      printedTotal:
        Number(set?.printedTotal || 0) || Number(previous?.printedTotal || 0),
      releaseDate: set?.releaseDate || previous?.releaseDate || "",
      images: { ...(previous?.images || {}), ...(set?.images || {}) },
      // Mainland CN curated sets are already released. A temporary provider
      // response without a count must never downgrade them to "announced".
      availability:
  ...
  return dedupeRegionalSets(Array.from(byCode.values()), lang);
}

async function fetchRegionalSets(lang: "ja" | "zh-tw"): Promise<any[]> {
  const cached = loadRegionalSetsCache(lang);
  ...
  try {
    ...
    const merged = mergeRegionalCatalogSnapshots(cached, fresh, lang);

    // Never replace a healthy catalogue with a suspiciously partial response.
    const chosen =
      cached.length > 0 && merged.length < cached.length
        ? mergeRegionalCatalogSnapshots(merged, cached, lang)
        : merged;

    if (chosen.length) saveRegionalSetsCache(lang, chosen);
    return chosen;
  } catch (error) {
