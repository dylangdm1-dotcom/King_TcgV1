...
async function fetchRegionalSetCards(setId: string, lang: "ja" | "zh-tw", setName?: string): Promise<PokemonCard[]> {
  try {
    const response = await fetch(
      `/api/catalog/sets?lang=${encodeURIComponent(lang)}&set=${encodeURIComponent(setId)}${setName ? `&name=${encodeURIComponent(setName)}` : ""}`,
      { cache: "no-store" }
    );
    if (!response.ok) return [];
    const json = await response.json();
...
function dedupeRegionalSets(sets: any[], lang: "ja" | "zh-tw"): any[] {
  ...
  for (const set of sets) {
    ...
    const providerId = String(set?.providerSetId || "").trim();
    const name = String(set?.name || "").trim();
    if (!code) continue;

    // Mainland Chinese has multiple products that can share a public code.
    // Keep the provider set_id/name as part of identity instead of collapsing
    // the whole catalogue by set_code.
    const key = lang === "zh-tw"
      ? `${providerId || code}:${normalizeText(name)}`
      : code;
    ...
    byKey.set(key, {
      ...set,
      id: set.id,
      name: set.name || meta?.name || set.id,
      series: set.series || meta?.era || "Pokémon TCG",
      providerSetId: providerId || undefined,
    });
  }

  // Keep new curated rows visible when an upstream index lags. They are
  // placeholders only; selecting them still resolves through provider APIs.
  for (const meta of curated) {
    const code = normalizeSetId(meta.code);
    const alreadyPresent = Array.from(byKey.values()).some(
      (set) => normalizeSetId(set?.id) === code || normalizeText(set?.name) === normalizeText(meta.name)
    );
    if (!code || alreadyPresent) continue;
    byKey.set(`curated:${code}:${normalizeText(meta.name)}`, {
      id: meta.code,
      name: meta.name,
      series: meta.era,
      total: 0,
...
export async function searchCardsBySetId(
  ...
  if (lang === "zh-tw") {
    cards = await fetchRegionalSetCards(String(setId).trim(), "zh-tw", setName);
  }
  ...
  if (!cards.length && lang === "ja") {
    cards = await fetchRegionalSetCards(String(setId).trim(), "ja", setName);
  }
