...
function normalize(card: any): PokemonCard {
  ...
  const mapTcgVariant = (variant: any) => {
    ...
    return {
      ...(low > 0 ? { low: Number((low * 0.92).toFixed(2)) } : {}),
      ...(mid > 0 ? { mid: Number((mid * 0.92).toFixed(2)) } : {}),
      ...(high > 0 ? { high: Number((high * 0.92).toFixed(2)) } : {}),
      ...(market > 0 ? { market: Number((market * 0.92).toFixed(2)) } : {}),
      ...(directLow > 0
        ? { directLow: Number((directLow * 0.92).toFixed(2)) }
        : {}),
    };
  };
