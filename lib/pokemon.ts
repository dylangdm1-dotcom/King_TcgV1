...
import { enrichCardsWithMarketPrices, getCachedMarketCardV275 } from "./priceClient";
import { findClosestPokemon } from "./levenshtein";
import {
  compareSetsNewestFirst,
  effectiveSetReleaseDate,
  isPhysicalBrowsableSet,
  normalizeSetId,
  setCodeRecency,
  setIdAliases,
} from "./setCatalog";
import {
  loadSearchCatalogSetCardsV278,
  loadSearchCatalogSetsV278,
...
const CACHE_KEY = "king_tcg_cards_cache_v17_stable_details";
...
if (typeof window !== "undefined") {
  try {
    const oldKeys = [
      ...
      "king_tcg_cards_cache_v16_regional_catalogs",
    ];
...
function normalize(card: any): PokemonCard {
  const sourceCardmarket = card.cardmarket?.prices || card.pricing?.cardmarket || {};
  const sourceTcgplayer = card.tcgplayer?.prices || card.pricing?.tcgplayer || {};
  const isSimplifiedChinese = card?.dataLanguage === "zh-tw";

  // PokéWallet set/card endpoints expose price variants as arrays while the
  // historical King_TCG normalizer expects keyed objects. Normalize those
  // arrays here so CN cards can display the Cardmarket/TCGPlayer prices that
  // were already returned with the set, without one extra API call per card.
  const cardmarketVariants = Array.isArray(sourceCardmarket) ? sourceCardmarket : [];
  const normalCm = cardmarketVariants.find((price: any) =>
    ["normal", "holo", "holographic"].includes(String(price?.variant_type || "").toLowerCase())
  ) || cardmarketVariants[0];
  const reverseCm = cardmarketVariants.find((price: any) =>
    /reverse/i.test(String(price?.variant_type || ""))
  );
  const rawCardmarket = isSimplifiedChinese && Array.isArray(sourceCardmarket)
    ? {
        averageSellPrice: normalCm?.avg,
        lowPrice: normalCm?.low,
        trendPrice: normalCm?.trend,
        avg1: normalCm?.avg1,
        avg7: normalCm?.avg7,
        avg30: normalCm?.avg30,
        reverseHoloSell: reverseCm?.avg,
        reverseHoloLow: reverseCm?.low,
        reverseHoloTrend: reverseCm?.trend,
      }
    : sourceCardmarket;

  const rawTcgplayer = isSimplifiedChinese && Array.isArray(sourceTcgplayer)
    ? Object.fromEntries(sourceTcgplayer.map((variant: any) => {
        const label = String(variant?.sub_type_name || "Normal");
        const key = /reverse/i.test(label)
          ? "reverseHolofoil"
          : /1st.*holo/i.test(label)
            ...
            : /1st/i.test(label)
              ...
              : /holo/i.test(label)
                ...
                : "normal";
        return [key, {
          low: variant?.low_price,
          mid: variant?.mid_price,
          high: variant?.high_price,
          market: variant?.market_price,
          directLow: variant?.direct_low_price,
        }];
      }))
    : sourceTcgplayer;
  ...
  const cmData = hasCardmarket
    ? {
        url: card.cardmarket?.url || card.cardmarket?.product_url || "",
        updatedAt:
...
function saveBrowserCache(cards: PokemonCard[]) {
  ...
  try {
    ...
    // Keep the browser cache bounded. An unbounded catalogue cache can exceed
    // localStorage quota, silently preventing older set summaries from being
    // saved before navigation to the detail page.
    const values = Array.from(map.values());
    const bounded = values.slice(-600);
    try {
      localStorage.setItem(CACHE_KEY, JSON.stringify(bounded));
    } catch {
      // Last-resort smaller cache instead of failing silently.
      localStorage.setItem(CACHE_KEY, JSON.stringify(bounded.slice(-200)));
    }
  } catch {}
}
...
/**
 * Données déjà connues du navigateur pour les analyses agrégées.
 * Lecture locale uniquement : ne déclenche aucun appel API.
 */
export function getCachedCardsForAnalytics(): PokemonCard[] {
  return loadBrowserCache().map((card) => getCachedMarketCardV275(card)?.card ?? card);
}

function hydrateMarketFromSharedCache(card: PokemonCard): PokemonCard {
  return getCachedMarketCardV275(card)?.card ?? card;
}
...
const tcgdexParentSetRequests = new Map<string, Promise<any | null>>();

function hasTcgdexImage(card: any): boolean {
  return Boolean(String(card?.image || "").trim());
}

async function loadTcgdexParentSet(lang: LanguageCode, setId: string): Promise<any | null> {
  const key = `${lang}:${setId}`;
  const existing = tcgdexParentSetRequests.get(key);
  if (existing) return existing;

  const request = (async () => {
    try {
      const response = await fetch(`${TCGDEX_URL}/${lang}/sets/${encodeURIComponent(setId)}`);
      return response.ok ? await response.json() : null;
    } catch {
      return null;
    }
  })();
  tcgdexParentSetRequests.set(key, request);
  return request;
}

async function recoverFrenchCardImage(card: any, fetchCardDetail = true): Promise<any> {
  if (hasTcgdexImage(card)) return card;

  let source = card;
  const cardId = String(card?.id || "").trim();
  if (fetchCardDetail && cardId) {
    try {
      const response = await fetch(`${TCGDEX_URL}/fr/cards/${encodeURIComponent(cardId)}`);
      if (response.ok) {
        const detail = await response.json();
        source = { ...card, ...detail, set: detail?.set || card?.set };
        if (hasTcgdexImage(source)) return source;
      }
    } catch {}
  }

  const setId = deriveTcgdexSetId(source);
  if (!setId) return source;
  const parentSet = await loadTcgdexParentSet("fr", setId);
  const rows = Array.isArray(parentSet?.cards) ? parentSet.cards : [];
  const localId = String(source?.localId || source?.number || "").trim();
  const summary = rows.find((row: any) =>
    String(row?.id || "") === cardId || String(row?.localId || "") === localId
  );

  return hasTcgdexImage(summary)
    ? { ...source, image: summary.image, set: source?.set || parentSet }
    : source;
}

async function recoverMissingFrenchImages(cards: any[]): Promise<any[]> {
  ...
  for (let index = 0; index < missing.length; index += 6) {
    ...
    const results = await Promise.all(
      batch.map(({ card }) => recoverFrenchCardImage(card, true))
    );
    results.forEach((card, resultIndex) => {
      recovered[batch[resultIndex].index] = card;
    });
  }
  return recovered;
}
...
function tcgdexPrintVariants(card: any): CardPrintVariant[] {
  ...
  for (const raw of detailed) {
    ...
    const rawPricing = raw?.pricing && typeof raw.pricing === "object"
      ? {
          ...(raw.pricing.cardmarket && typeof raw.pricing.cardmarket === "object"
            ? { cardmarket: raw.pricing.cardmarket as Record<string, unknown> }
            : {}),
          ...(raw.pricing.tcgplayer && typeof raw.pricing.tcgplayer === "object"
            ? { tcgplayer: raw.pricing.tcgplayer as Record<string, unknown> }
            : {}),
        }
      : undefined;
    add(key, {
      ...
      ...(rawPricing && Object.keys(rawPricing).length ? { pricing: rawPricing } : {}),
    });
  }
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(tcgdexData) && tcgdexData.length > 0) {
            const recovered = await recoverMissingFrenchImages(tcgdexData.slice(0, 30));
            const formatted = recovered.map((c) => normalizeTCGdexCard(c, "fr"));
...
export async function searchCards(
  ...
  try {
    ...
    if (summaries.length) {
      ...
      const imageReadySummaries = lang === "fr"
        ? await recoverMissingFrenchImages(summaries as any[])
        : summaries;
      tcgdexCards = applySetMetadataAndRecentSort(
        imageReadySummaries.map((card: any) => normalizeTCGdexCard(card, lang))
      );
    }
...
async function fetchPublicChineseCardsClient(setId: string): Promise<PokemonCard[]> {
  try {
    const code = String(setId || "").trim().toUpperCase();
    if (!code) return [];
    const response = await fetch(
      `https://www.pokewallet.io/api/catalog/sets/${encodeURIComponent(code)}/cards?game=pokemon`,
      { cache: "no-store", mode: "cors", headers: { Accept: "application/json" } }
    );
    if (!response.ok) return [];
    const json = await response.json();
    const rows = Array.isArray(json?.data) ? json.data : [];
    if (!json?.success || rows.length === 0) return [];
    return rows.map((raw: any, index: number) => normalizeRegionalProxyCard({
      id: `pokewallet-public-client-zh-tw-${code}-${String(raw?.card_number || index + 1)}-${index}`,
      name: String(raw?.card_name || `Carte ${raw?.card_number || index + 1}`),
      number: String(raw?.card_number || ""),
      set: {
        id: code,
        name: code,
    ...
    logger.warn("API", `[Public CN browser fallback ${setId}]`, error);
    return [];
  }
}


function cachedChineseSetCards(setId: string): PokemonCard[] {
  if (typeof window === "undefined") return [];
  const wanted = normalizeSetId(setId);
  const cachedCards = loadBrowserCache().filter((card) => {
    if (card.dataLanguage !== "zh-tw") return false;
    if (normalizeSetId(card.set?.id) !== wanted) return false;
    return Boolean(card.images?.large || card.images?.small || card.imageCandidates?.length);
  });
  return cachedCards.sort((a, b) => {
...
async function fetchRegionalSetCards(setId: string, lang: "ja" | "zh-tw"): Promise<PokemonCard[]> {
  // CN images/provider ids are expensive to recover. Reuse a previously
  // successful image-rich set before spending quota or accepting a degraded
  // public fallback without images.
  if (lang === "zh-tw") {
    const cached = cachedChineseSetCards(setId);
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (rows.length > 0) return rows.map((raw: any) => normalizeRegionalProxyCard(raw, lang));
    }

    // V77: if Vercel cannot reach PokéWallet's public catalogue, let the
    // browser try the same public endpoint directly. This path is CN-only and
    // never changes JP/FR/EN behaviour.
    if (lang === "zh-tw") return fetchPublicChineseCardsClient(setId);
    return [];
  } catch (error) {
...
const REGIONAL_SET_LIST_CACHE_VERSION = "v20";
const REGIONAL_SET_LIST_CACHE_MAX_AGE = 180 * 24 * 60 * 60 * 1000;

function regionalSetListCacheKey(lang: "ja" | "zh-tw") {
  return `king_tcg_regional_set_list_${REGIONAL_SET_LIST_CACHE_VERSION}_${lang}`;
}

function readRegionalSetListCache(lang: "ja" | "zh-tw"): any[] {
  if (typeof window === "undefined") return [];
  try {
    const raw = localStorage.getItem(regionalSetListCacheKey(lang));
    if (!raw) return [];
    const parsed = JSON.parse(raw);
    if (!parsed?.savedAt || Date.now() - parsed.savedAt > REGIONAL_SET_LIST_CACHE_MAX_AGE) return [];
...
function writeRegionalSetListCache(lang: "ja" | "zh-tw", sets: any[]) {
  if (typeof window === "undefined" || !sets.length) return;
  try {
    localStorage.setItem(
      regionalSetListCacheKey(lang),
      JSON.stringify({ savedAt: Date.now(), sets })
    );
  } catch {}
}

function mergeRegionalSetLists(cached: any[], fresh: any[]): any[] {
  const byId = new Map<string, any>();
  for (const set of cached) {
    const key = normalizeSetId(set?.id);
    if (key) byId.set(key, set);
  }
  for (const set of fresh) {
    ...
    byId.set(key, {
      ...previous,
      ...set,
      total: Number(set?.total || 0) || Number(previous?.total || 0),
      printedTotal: Number(set?.printedTotal || 0) || Number(previous?.printedTotal || 0),
      releaseDate: set?.releaseDate || previous?.releaseDate || "",
      images: { ...(previous?.images || {}), ...(set?.images || {}) },
      availability:
        set?.availability === "available" || previous?.availability === "available"
          ? "available"
          : set?.availability || previous?.availability || "unknown",
    });
  }
...
async function fetchRegionalSets(lang: "ja" | "zh-tw"): Promise<any[]> {
  const cached = readRegionalSetListCache(lang);
  try {
    const response = await fetch(
    ...
    const json = await response.json();
    const fresh = Array.isArray(json?.sets) ? json.sets : [];
    const merged = mergeRegionalSetLists(cached, fresh);
    if (merged.length) writeRegionalSetListCache(lang, merged);
    return merged;
  } catch (error) {
...
function dedupeRegionalSets(sets: any[], lang: "ja" | "zh-tw"): any[] {
  ...
  for (const set of sets) {
    ...
    const candidate = {
      ...
      availability: set?.availability || (total > 0 ? "available" : "unknown"),
    };
    ...
    // Les index régionaux peuvent renvoyer à la fois le code éditorial
    // King_TCG et l'alias fournisseur (CS65C / CS6.5C, par exemple). Si
    // l'entrée vide arrive en premier, toujours conserver ensuite celle qui
    // publie réellement les cartes et ses identifiants de fournisseur.
    const currentTotal = Number(current?.total || current?.printedTotal || 0);
    if (total > currentTotal) {
      byCode.set(code, {
        ...current,
        ...candidate,
        id: meta?.code || current.id || candidate.id,
        name: meta?.name || candidate.name || current.name,
        series: meta?.era || candidate.series || current.series,
        images: { ...(current.images || {}), ...(candidate.images || {}) },
        availability: candidate.availability,
      });
    }
  }

  // V54: the upstream regional list can lag behind the newest JP/CN release.
  // Keep the curated catalogue additive so M6 / CSV10C (and corrected CSV9.5)
  // remain visible even before the provider's /sets index catches up. Opening
  // the set still goes through the normal regional endpoint and never invents cards.
  for (const meta of curated) {
    const code = normalizeSetId(meta.code);
    if (!code || byCode.has(code)) continue;
    byCode.set(code, {
      id: meta.code,
      name: meta.name,
      series: meta.era,
      total: Number(meta.officialCount || 0),
      printedTotal: Number(meta.officialCount || 0),
      releaseDate: effectiveSetReleaseDate(meta.code, meta.releaseDate || ""),
      images: {},
      availability: lang === "zh-tw" ? "metadata_only" : "announced",
    });
  }
...
export async function searchCardsBySetId(
  ...
  const cacheKey = `set_v14_local_catalog_${cleanId}_${lang}`;
  ...
  if (lang === "fr" || lang === "en" || lang === "zh-tw") {
    try {
      ...
      if (
        ...
        localCards.forEach((card) => cache.set(card.id, card));
        saveBrowserCache(localCards);
        searchCache.set(cacheKey, localCards);
        return localCards;
      }
    } catch (error) {
      logger.error("API", `[Catalogue local ${lang}/${setId}]`, error);
    }
  }
  ...
  // V78: JP display is TCGdex-only. Never call the authenticated PokéWallet
  // regional proxy here: a PokéWallet 429 must not turn into a Vercel 502 or
  // interfere with an otherwise healthy Japanese catalogue.
  ...
  if (localSet?.cards.length) {
    const mergedCards = new Map<string, PokemonCard>();
    for (const card of localSet.cards) {
      mergedCards.set(`${normalizeText(card.name)}:${cleanCardNumber(card.number) || card.number}`, card);
    }
    for (const card of cards) {
      const key = `${normalizeText(card.name)}:${cleanCardNumber(card.number) || card.number}`;
      const local = mergedCards.get(key);
      mergedCards.set(key, local ? {
        ...local,
        ...card,
        images: {
          small: card.images?.small || local.images.small,
          large: card.images?.large || local.images.large,
        },
        imageCandidates: Array.from(new Set([
          ...(card.imageCandidates || []),
          ...(local.imageCandidates || []),
        ])),
        availablePrintVariants: card.availablePrintVariants?.length
          ? card.availablePrintVariants
          : local.availablePrintVariants,
      } : card);
    }
  ...
  if (resolvedSetId !== cleanId) {
    searchCache.set(`set_v8_cn_public_${normalizeSetId(resolvedSetId)}_${lang}`, cards);
  }
  return cards;
}
...
export async function getAllSets(lang: LanguageCode = "fr"): Promise<any[]> {
  ...
  try {
    localSets = await loadSearchCatalogSetsV278(targetLang);
  } catch (error) {
  ...
  if (targetLang === "zh-tw") {
    ...
    const mergedRegional = mergeSearchCatalogSetsV278(localSets, regional)
      .sort(compareSetsNewestFirst);
    mergedRegional.forEach((set: any) => {
      setMetadataCache.set(normalizeSetId(set.id), {
        name: set.name,
        releaseDate: set.releaseDate || "",
        series: typeof set.series === "string" ? set.series : set.series?.name,
  ...
  // V78: do not merge PokéWallet's authenticated JP catalogue. TCGdex is the
  // authoritative runtime source for JP; curated additions below stay local.
  ...
  tcgdexSets = (await hydrateRecentSetMetadata(targetLang as LanguageCode, tcgdexSets))
    .filter((set) => isPhysicalBrowsableSet(set));
  ...
  const mergedSets = mergeSearchCatalogSetsV278(localSets, Array.from(merged.values()));
...
export async function getCardById(id: string): Promise<PokemonCard | null> {
  ...
  if (targetId.startsWith("tcgdex-")) {
      ...
      if (data) {
        ...
        const card = saved
          ? (() => {
              ...
              ebayListings: saved.ebayListings ?? rawCard.ebayListings,
              marketQuotes: saved.marketQuotes ?? rawCard.marketQuotes,
              marketEstimate: saved.marketEstimate ?? rawCard.marketEstimate,
          ...
          : rawCard;
        const hydrated = hydrateMarketFromSharedCache(card);
        cache.set(hydrated.id, hydrated);
        cache.set(targetId, hydrated);
        saveBrowserCache([hydrated]);
        return hydrated;
      }
    } catch (error) {
    ...
    if (fallbackCard) {
      const hydrated = hydrateMarketFromSharedCache(fallbackCard);
      cache.set(hydrated.id, hydrated);
      return hydrated;
    }

    // V91: old FR/JP cards can occasionally have an unavailable single-card
    // endpoint while their parent set endpoint still works. Rebuild a safe
    // summary from the set instead of failing the whole detail page.
    const separator = rawCardId.lastIndexOf("-");
    if (separator > 0 && separator < rawCardId.length - 1) {
      const rawSetId = rawCardId.slice(0, separator);
      const rawLocalId = rawCardId.slice(separator + 1);
      for (const locale of tcgdexLocales(lang)) {
        try {
          const setResponse = await fetch(`${TCGDEX_URL}/${locale}/sets/${encodeURIComponent(rawSetId)}`);
          if (!setResponse.ok) continue;
          const parentSet = await setResponse.json();
          const rows = Array.isArray(parentSet?.cards) ? parentSet.cards : [];
          const summary = rows.find((row: any) =>
            String(row?.id || "") === rawCardId || String(row?.localId || "") === rawLocalId
          );
          if (!summary) continue;
          const card = normalizeTCGdexCard(summary, lang, parentSet);
          const hydrated = hydrateMarketFromSharedCache(card);
          cache.set(hydrated.id, hydrated);
          cache.set(targetId, hydrated);
          saveBrowserCache([hydrated]);
