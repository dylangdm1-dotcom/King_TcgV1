...
const CACHE_KEY = "king_tcg_cards_cache_v14_fr_catalogue_complete";
...
export async function searchCardsBySetId(
  ...
  const cacheKey = `set_v5_${cleanId}_${lang}`;
  ...
  for (const locale of tcgdexLocales(lang)) {
    for (const candidateId of providerSetIds.length ? providerSetIds : [String(setId).trim()]) {
      try {
        ...
        // V43 JP: some set endpoints return CardBrief entries without `image`
        // even though the single-card endpoint has the asset. Hydrate ONLY
        // those missing-image summaries, never the whole extension.
        if (lang === "ja") {
  ...
  if (resolvedSetId !== cleanId) {
    searchCache.set(`set_v5_${normalizeSetId(resolvedSetId)}_${lang}`, cards);
  }
  return cards;
}
