...
const CACHE_KEY = "king_tcg_cards_cache_v34_jp_cn_extensions";
...
export async function getAllSets(lang: LanguageCode = "fr"): Promise<any[]> {
  ...
  if (targetLang === "fr" || targetLang === "en") {
    for (const set of pokemonSets) {
      merged.set(String(set.id).toLowerCase(), set);
    }
  }
  ...
  mergedSets.forEach((set: any) => {
    setMetadataCache.set(`${targetLang}:${normalizeSetId(set.id)}`, {
      name: set.name,
