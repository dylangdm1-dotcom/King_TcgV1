...
const CACHE_KEY = "king_tcg_cards_cache_v35_catalog_detail";
...
if (typeof window !== "undefined") {
  try {
    const oldKeys = [
      ...
      "king_tcg_cards_cache_v31",
      "king_tcg_cards_cache_v32_estimation_fr",
      "king_tcg_cards_cache_v33_stable_display",
      "king_tcg_cards_cache_v34_jp_cn_extensions",
    ];
...
function sanitizeCachedQuotes(value: unknown): PokemonCard["marketQuotes"] {
  if (!Array.isArray(value)) return [];
  return value.flatMap((raw: any) => {
    if (!raw || typeof raw !== "object") return [];
    const price = safePrice(raw.price);
    if (!price) return [];
    return [{
      ...raw,
      source: String(raw.source || "cardmarket"),
      label: String(raw.label || "Cotation marché"),
      price,
      currency: "EUR" as const,
      language: ["fr", "en", "ja", "zh-tw", "multi"].includes(raw.language)
        ? raw.language
        : "multi",
...
function sanitizeMarketEstimate(value: unknown): PokemonCard["marketEstimate"] | undefined {
  ...
  return {
    price,
    currency: "EUR",
    language: ["fr", "en", "ja", "zh-tw"].includes(raw.language) ? raw.language : "en",
    condition: "Near Mint",
    confidence: ["high", "medium", "limited", "none"].includes(raw.confidence) ? raw.confidence : "limited",
    includedSources: Array.isArray(raw.includedSources)
      ? raw.includedSources.map((source: unknown) => String(source))
      : [],
    excludedSources: Array.isArray(raw.excludedSources)
      ? raw.excludedSources.flatMap((item: any) =>
          item && typeof item === "object"
            ? [{ source: String(item.source || "unknown"), reason: String(item.reason || "Non précisé") }]
            : []
        )
      : [],
  };
...
function normalize(card: any): PokemonCard {
  ...
  const computedPrice =
    safePrice(card.marketEstimate?.price) ||
  ...
  return {
    ...
    computedPrice,
    marketQuotes: sanitizeCachedQuotes(card.marketQuotes),
    marketEstimate: sanitizeMarketEstimate(card.marketEstimate),
    marketStatus: card.marketStatus || undefined,
    images: {
...
function tcgdexImageCandidates(card: any, lang: LanguageCode, setId: string, localId: string): string[] {
  ...
  if (setId && localId) {
    const fallbackLocales =
      lang === "fr" || lang === "en"
        ? Array.from(new Set([...tcgdexLocales(lang), "en"]))
        : tcgdexLocales(lang);
  ...
  if (card?.id) {
    add(`/api/card-image?locale=${encodeURIComponent(lang)}&id=${encodeURIComponent(String(card.id))}`);
  }
...
function normalizeTCGdexCard(card: any, lang: LanguageCode, parentSet?: any): PokemonCard {
  ...
  const setMeta = setMetadataCache.get(`${lang}:${normalizeSetId(setId)}`);
...
export async function searchCardsBySetId(
  ...
  const aliases = setIdAliases(setId, lang);
  const cleanId = aliases[0] || normalizeSetId(setId);
  const cacheKey = `set_v34_${lang}_${cleanId}`;
  ...
  if (resolvedSetId !== cleanId) {
    searchCache.set(
      `set_v34_${lang}_${normalizeSetId(resolvedSetId)}`,
      cards
    );
  }
  return cards;
...
function chineseDisplaySetId(value?: string): string {
  const raw = String(value || "").trim();
  if (!raw) return raw;
  if (/^c(?:s|sv)/i.test(raw)) return raw;
  if (/^sv\d/i.test(raw)) return `c${raw}`;
  return raw;
}

function setSemanticKey(set: any): string {
  const name = normalizeSearchText(String(set?.name || ""));
  const release = String(set?.releaseDate || "").slice(0, 10);
  const total = Number(set?.total || set?.printedTotal || 0);
  return `${name}:${release}:${total}`;
}
...
export async function getAllSets(lang: LanguageCode = "fr"): Promise<any[]> {
  ...
  try {
    ...
    // Une langue ne doit afficher que son propre catalogue. Pour le chinois,
    // zh-cn n'est utilisé qu'en secours si zh-tw ne renvoie aucun catalogue.
    const locales = tcgdexLocales(targetLang as LanguageCode);
    for (const locale of locales) {
      ...
      for (const set of data) {
        ...
        // Sécurité catalogue : les séries CS/CSV sont chinoises et ne doivent
        // jamais apparaître dans le catalogue japonais, même via un cache/API tiers.
        if (targetLang === "ja" && /^(cs|csv)/i.test(key)) continue;
        // Écarter les entrées fantômes sans identifiant, sans nom ou sans carte.
        if (!key || !set.name || total <= 0 || byId.has(key)) continue;
        const publicId =
          targetLang === "zh-tw"
            ? chineseDisplaySetId(set.id)
            : set.id;

        byId.set(normalizeSetId(publicId), {
          id: publicId,
          sourceId: set.id,
          name: set.name,
  ...
  // Pokémon TCG API couvre principalement les catalogues occidentaux.
  // Ne jamais fusionner sa liste mondiale dans les catalogues JP/CN.
  ...
  // Un seul catalogue actif par langue. Quand TCGdex répond, ne jamais
  // réinjecter le catalogue Pokémon TCG API par-dessus : c'était la cause
  // des séries MEGA dupliquées et de l'ancien chemin à 0 prix.
  ...
  for (const set of catalogueSource) {
    const key = normalizeSetId(set.id);
    if (!key) continue;

    const semantic = setSemanticKey(set);
    const duplicate = Array.from(merged.values()).find(
      (existing: any) => setSemanticKey(existing) === semantic
    );
  ...
  const mergedSets = Array.from(merged.values()).filter((set: any) => {
    const key = normalizeSetId(set?.id);
    if (targetLang === "ja" && /^(cs|csv)/i.test(key)) return false;
    return true;
  });
  mergedSets.forEach((set: any) => {
    ...
    setMetadataCache.set(`${targetLang}:${normalizeSetId(set.id)}`, metadata);
    if (set.sourceId) {
      setMetadataCache.set(`${targetLang}:${normalizeSetId(set.sourceId)}`, metadata);
    }
  });
...
async function ensureDetailedMarketPrices(card: PokemonCard): Promise<PokemonCard> {
  const alreadyEnriched =
    Array.isArray(card.marketQuotes) &&
    card.marketQuotes.length > 0 &&
    Number(card.marketEstimate?.price || 0) > 0;

  if (alreadyEnriched) return card;

  try {
    const enriched = await enrichCardsWithMarketPrices([card]);
    const result = enriched[0] ?? card;
    cache.set(result.id, result);
    saveBrowserCache([result]);
    return result;
  } catch (error) {
    logger.error("API", "[Detail price enrichment]", error);
    return card;
  }
}
...
export async function getCardById(id: string): Promise<PokemonCard | null> {
  ...
  const forceRemoteDetail = /^tcgdex-(?:ja|zh-tw|zh-cn)-/i.test(targetId);

  if (!forceRemoteDetail) {
  ...
  if (targetId.startsWith("tcgdex-")) {
    const match = targetId.match(/^tcgdex-(zh-tw|zh-cn|fr|en|ja)-(.+)$/i);
    if (!match) return null;

    const requestedLocale = match[1].toLowerCase();
    const rawCardId = match[2];
    const locales = requestedLocale.startsWith("zh-")
      ? [requestedLocale, requestedLocale === "zh-tw" ? "zh-cn" : "zh-tw"]
      : [requestedLocale];
    ...
    for (const locale of locales) {
      try {
        ...
        const data = await response.json();
        const appLanguage = (locale.startsWith("zh-") ? "zh-tw" : locale) as LanguageCode;
        const card = normalizeTCGdexCard(data, appLanguage);
