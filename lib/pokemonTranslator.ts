...
/**
 * 🃏 Nettoyage des suffixes cartes Pokémon TCG
 * Exemple : Drattak ex -> Drattak
 */
...
/**
 * 🔄 Traduit un Pokémon français vers anglais (Ex: "Drattak ex" -> "Salamence ex")
 */
export function translatePokemonToEnglish(name: string): string | null {
  ...
  // 2. Si non trouvé, retirer le suffixe (ex: "ex", "gx", "vmax") et traduire le nom de base
  const baseNameFR = cleanTCGSuffix(name);
  const cleanBase = normalizeName(baseNameFR);

  if (cleanBase && pokemonNames[cleanBase]) {
    const translatedBase = pokemonNames[cleanBase];
    // Récupérer le suffixe d'origine s'il existe
    const suffix = name.toLowerCase().trim().slice(baseNameFR.length).trim();
