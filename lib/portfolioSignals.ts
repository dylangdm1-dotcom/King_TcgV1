...
import { getLastPrice, getMarketHistory } from "./priceHistory";

const DAY = 24 * 60 * 60 * 1000;

function trendFromHistory(cardId: string, days: number): number {
...
export function getEffectiveTrend7d(card: PokemonCard): number {
  ...
  const historyTrend = trendFromHistory(card.id, 7);
...
export function getEffectiveTrend30d(card: PokemonCard): number {
  ...
  return trendFromHistory(card.id, 30);
}
...
export function getEffectiveCurrentPrice(card: PokemonCard): number {
  ...
  const last = getLastPrice(card.id)?.average || 0;
...
export function hasUsableMarketSignal(card: PokemonCard): boolean {
  ...
  const history = getMarketHistory(card.id).filter((point) => Number(point?.average) > 0);
