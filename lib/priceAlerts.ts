...
import { getEffectiveMarketHistory } from "./priceHistory";
...
export function analyzeCardAlerts(
  ...
  try {
    ...
    const quality = assessAnalysisQuality(card, getEffectiveMarketHistory(card));
    const qualityFields = {
    ...
    if (change <= -2 && change > -8) {
      return {
        ...
        type: "OPPORTUNITY",
        changePercent: roundedChange,
        message: `💰 ${card.name} recule de ${Math.abs(roundedChange)}% : repli modéré à surveiller.`,
        ...qualityFields,
      };
    }
