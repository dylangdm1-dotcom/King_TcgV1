...
const BATCH = 20;
const PRICE_CACHE_KEY = "king_tcg_market_price_cache_v18_justtcg_stats_cn_catalog";
...
function priceRequestKey(card: PokemonCard): string {
  ...
  return [
    "price-v11-justtcg-stats-cn-catalog",
    card.id,
    language(card),
    card.set?.id,
    card.number,
...
async function batch(cards: PokemonCard[]): Promise<Response | null> {
  ...
  try {
    const response = await fetch("/api/prices", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-King-TCG-Price-Version": "73" },
