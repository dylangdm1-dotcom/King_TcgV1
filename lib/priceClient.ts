import { buildPriceRequestCardV279 } from "./market/request";
...
export function marketCacheKeyForCardV275(card: PokemonCard): string {
  return buildPriceRequestCardV279(card).requestKey;
}
...
async function batch(cards: PokemonCard[]): Promise<Response | null> {
  ...
  try {
    const response = await fetch("/api/prices", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-King-TCG-Price-Version": "79" },
      signal: controller.signal,
      body: JSON.stringify({ cards: cards.map(buildPriceRequestCardV279) }),
    });
