...
const BATCH = 20;
function marketSetId(card: PokemonCard): string | undefined {
  const raw = String(card.set?.id || "").trim();
  if (!raw) return undefined;
  if (language(card) === "zh-tw" && /^sv\d/i.test(raw) && !/^csv/i.test(raw)) return `c${raw}`;
  return raw;
}
...
function merge(card: PokemonCard, payload?: Item): PokemonCard {
  ...
  const next: PokemonCard = {
    ...
    marketQuotes: safeQuotes(payload.quotes).length
      ? safeQuotes(payload.quotes)
      : (Array.isArray(card.marketQuotes) ? card.marketQuotes.filter(Boolean) : []),
...
async function batch(cards: PokemonCard[]): Promise<Response | null> {
  ...
  try {
    const response = await fetch("/api/prices", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-King-TCG-Price-Version": "35" },
      signal: controller.signal,
      body: JSON.stringify({ cards: cards.map((card) => ({
        id: card.id, name: card.name, number: card.number,
        setId: card.set?.id,
        marketSetId: marketSetId(card),
