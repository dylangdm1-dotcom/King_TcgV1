...
const BATCH = 20;
const PRICE_CACHE_KEY = "king_tcg_market_price_cache_v13_cardmarket_productid";
...
async function batch(cards: PokemonCard[]): Promise<Response | null> {
  ...
  try {
    const response = await fetch("/api/prices", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-King-TCG-Price-Version": "68" },
