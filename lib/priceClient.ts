...
const BATCH = 20;
const PRICE_CACHE_KEY = "king_tcg_market_price_cache_v15_justtcg_exact_variant";
...
function priceRequestKey(card: PokemonCard): string {
  ...
  return [
    "price-v8-justtcg-exact-variant",
    card.id,
    language(card),
    card.set?.id,
    card.number,
...
async function batch(cards: PokemonCard[]): Promise<Response | null> {
  ...
  try {
    const response = await fetch("/api/prices", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-King-TCG-Price-Version": "70" },
