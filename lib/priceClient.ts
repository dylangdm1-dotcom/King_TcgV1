...
const BATCH = 20;
const PRICE_CACHE_KEY = "king_tcg_market_price_cache_v10_cardmarket_fr_resolution_diag";
...
async function batch(cards: PokemonCard[]): Promise<Response | null> {
  ...
  try {
    const response = await fetch("/api/prices", {
      method: "POST",
      headers: { "Content-Type": "application/json", "X-King-TCG-Price-Version": "64" },
