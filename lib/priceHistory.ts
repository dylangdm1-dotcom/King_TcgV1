...
/**
 * 🛠️ Génère un historique réaliste reconstitué (Fallback) sur X jours (30j min)
 * basé sur les tendances 7d et 30d Cardmarket/TCGPlayer si l'historique local est insuffisant.
 */
 ...
 * Si l'historique local est vide ou incomplet, reconstitue une courbe réelle à partir de la carte.
 */
export function getMarketHistoryDays(
  ...
  let history = getMarketHistory(cardId);

  // Si moins de 3 points en local et qu'on a l'objet PokemonCard, on génère un historique de 30/90j fiable
