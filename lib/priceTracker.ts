...
export function calculateRealMarketPrices(card?: PokemonCard | null) {
  if (!card) {
    return { cardmarket: 0, tcgplayer: 0, justtcg: 0, ebay: 0, average: 0 };
  }
  ...
  const justtcg = market.justtcg || getJustTcgPrice(card) || 0;
  const ebay = market.ebay || getEbayPrice(card) || 0;

  const activePrices = [cardmarket, tcgplayer, justtcg, ebay].filter(
