...
export async function searchCardsForScan(scan: CardScanResult): Promise<PokemonCard[]> {
  ...
  // V86 : matching strict dans la langue du scan. Une carte d'une autre langue
  // ne doit jamais devenir un faux positif simplement parce que son nom ressemble.
  // Si le catalogue régional ne produit pas de candidat fiable, on préfère un résultat
  // vide / à vérifier plutôt qu'une impression FR/EN/JP/CN incorrecte.
  const ranked = rankCards(candidates, scan, language);
