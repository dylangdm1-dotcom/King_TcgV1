...
const MAX_ENRICHED_CANDIDATES = 10;
...
function scoreCard(card: PokemonCard, scan: CardScanResult, language: LanguageCode) {
  ...
  const cardLanguage = card.dataLanguage ||
    (card.id.startsWith("tcgdex-ja-") ? "ja" :
      card.id.startsWith("tcgdex-zh-") ? "zh-tw" :
      card.id.startsWith("tcgdex-fr-") ? "fr" : "en");
  if (cardLanguage === language) score += 900;
  else if (language === "ja" || language === "zh-tw") score -= 5_000;
 ...
 * les images, le cache et les prix de la Recherche standard. Seul le classement
 * visuel est spécifique au Scanner.
 */
export async function searchCardsForScan(scan: CardScanResult): Promise<PokemonCard[]> {
  ...
  // Secours contrôlé : les éditions asiatiques peuvent avoir un nom traduit dans
  // une source internationale, mais ce fallback ne s'active que sans candidat solide.
  let ranked = rankCards(candidates, scan, language);
  const bestScore = ranked[0]?.score ?? -Infinity;
  if (bestScore < 1_000 && language !== "ja" && language !== "zh-tw") {
    const fallbackLanguages: LanguageCode[] = language === "fr" ? ["en"] : ["fr"];

    for (const fallbackLanguage of fallbackLanguages) {
      const fallback = await searchByNames(scan, fallbackLanguage);
      candidates.push(...fallback);
      ranked = rankCards(candidates, scan, language);
      if ((ranked[0]?.score ?? -Infinity) >= 1_000) break;
    }
  }

  const shortlist = ranked.slice(0, MAX_ENRICHED_CANDIDATES).map(({ card }) => card);
  if (!shortlist.length) return [];

  const enriched = await withTimeout(
    enrichAndCacheCards(shortlist),
    shortlist,
    "Enrichissement marché Scanner"
  );

  // L'enrichissement ne doit jamais modifier le meilleur match identitaire.
  return rankCards(enriched, scan, language).map(({ card }) => card);
}
