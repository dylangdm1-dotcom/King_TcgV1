...
// TCGdex expose aussi Pokémon TCG Pocket dans /sets. King_TCG suit les cartes
// physiques et leurs marchés : ces extensions numériques ne doivent donc pas
// être présentées comme des extensions physiques. Les trois autres identifiants
// sont des index techniques dont la route de série ne publie aucune carte.
const NON_PHYSICAL_OR_NON_BROWSABLE_SET_IDS = new Set([
  "a1", "pa", "a1a", "a2", "a2a", "a2b", "a3", "a3a", "a3b", "a4", "a4a",
  "b1", "b1a", "b2", "b2a",
  "wp", "rc", "jumbo",
]);

export function isPhysicalBrowsableSet(set: SetLike): boolean {
  const id = normalizeSetId(set.id);
  if (!id || NON_PHYSICAL_OR_NON_BROWSABLE_SET_IDS.has(id)) return false;
  const series = typeof set.series === "string" ? set.series : set.series?.name || "";
  return !/tcg\s*pocket|pokémon\s*tcg\s*pocket|pokemon\s*tcg\s*pocket/i.test(series);
}
