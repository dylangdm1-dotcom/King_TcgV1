...
export function setPrintingVariant(id: string, printingVariant: string) {
  const infos = getCollectionInfos();
  ensureCollectionInfo(id);
  infos[id] = {
    ...infos[id],
    printingVariant: printingVariant || "Normal",
  };
  saveCollectionInfos(infos);
  notifyStorageUpdate();
}

export function getPrintingVariant(id: string): string {
  const info = getCollectionInfo(id);
  return info?.printingVariant || "Normal";
}
