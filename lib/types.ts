...
function temporaryLocalizedEstimate(card?: PokemonCard | null): number {
  if (!card || !["fr", "ja", "zh-tw"].includes(String(card.dataLanguage || ""))) return 0;
...
export function getCardPrice(card?: PokemonCard | null): number {
  ...
  const localizedEstimate = temporaryLocalizedEstimate(card);
  if (localizedEstimate > 0) return localizedEstimate;
