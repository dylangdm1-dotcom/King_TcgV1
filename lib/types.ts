...
export function getCardPrice(card?: PokemonCard | null): number {
  ...
  const justTcgMarket = positivePrice(card.justtcg?.price);
  const ebayMarket = positivePrice(card.ebay?.price);

  const values = [cmMarket, tcgMarket, justTcgMarket, ebayMarket].filter(
...
export function getNormalizedMarketSummary(card?: PokemonCard | null): NormalizedMarketSummary {
  ...
  if (card.marketSources?.justtcg || card.justtcg?.price) sources.push("justtcg");
  if (card.marketSources?.ebaySold || card.ebay?.kind === "last_sold") sources.push("ebay_last_sold");
  else if (card.marketSources?.ebayListings || card.ebay?.price) sources.push("ebay_listings");
