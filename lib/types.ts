...
export interface PokemonCard {
  ...
  /** Source JustTCG conservée séparément de TCGPlayer. */
  justtcg?: {
    url?: string;
    updatedAt?: string;
    currency?: "USD" | "EUR";
  ...
  /** Réservé à une future source eBay réelle. Aucune valeur synthétique. */
  ebayListings?: {
    median?: number;
    sampleSize?: number;
    currency?: "EUR" | "USD";
...
export function getCardPrice(card?: PokemonCard | null): number {
  ...
  const justTcgMarket = positivePrice(card.justtcg?.medianNearMint);

  const values = [cmMarket, tcgMarket, justTcgMarket].filter(
