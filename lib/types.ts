...
export function getCardPrice(card?: PokemonCard | null): number {
  ...
  // Si aucune cote locale exacte n'est prouvée, afficher une vraie référence
  // de marché plutôt qu'un faux 0 €. Priorité à la tendance Cardmarket Europe.
  // Cette valeur reste explicitement "indicative" dans la fiche marché.
  const europe = card.marketQuotes?.find(
    (quote) => quote.language === "multi" && quote.metric === "trend_europe" && positivePrice(quote.price) !== undefined
  );
  const europePrice = positivePrice(europe?.price);
  if (europePrice !== undefined) return Number(europePrice.toFixed(2));

  const indicative = card.marketQuotes?.find(
    (quote) => quote.classification === "indicative" && positivePrice(quote.price) !== undefined
  );
  const indicativePrice = positivePrice(indicative?.price);
  return indicativePrice !== undefined ? Number(indicativePrice.toFixed(2)) : 0;
}
